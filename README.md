...
- **#74 — worker-orchestrated swarms** *(contract v1.1 folded; red-first suite landed; suite-fold running)* — a heavyweight coordinator member over cheap flash rows as ONE workflow spec: the truthful steering trail (denied answers record `denied`, never falsified `answered`), the scratchpad read-authorization law, escalation bounds. The two-level dogfood (the **#147** control-surface audit) is staged and driving.
...
- **#61 — worker verdict surface** *(contract v1.1 + suite landed; blue-team running)* — the worker-facing four-field `{gate, check, detail, corrective}` verdict + objectives generated from live truth (never boilerplate).
- **#70 — cross-deployment knowledge** *(suite landed; blue-team folded 5 green-side blockers, suite-fold running)* — one primary KG root per project; promotion primary-only on every path.
...
- **#73 — feedback-forge hardening** *(suite landed; blue-team running)* — `run.feedback` gate-shaped submissions are hub-minted or refused, never caller-authored.
...
- **#144 — LSP support** *(contract v1.1 + suite landed)* — a bounded, honest LSP pool for diagnostic scoping and environmental understanding; clock-free wedged-server trigger; effective-view absence caching.
