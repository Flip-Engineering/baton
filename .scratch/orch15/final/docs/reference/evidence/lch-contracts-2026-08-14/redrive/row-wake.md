# ROW BRIEF — row-wake: the wake contract (package ④ lifecycle-honesty)

Read `foundry-brief.md` first (the frame + laws bind you — Ring-2 contract form: ground truths (cited fresh, NUL discipline on application.mjs + coordination-store.mjs) → decisions → closed refusal vocabulary → red-first acceptance pins at named stages → open questions; no clocks; sorted-key literals ACTUAL order; your [attempt: <salt> <role>] line VERBATIM in the first five lines). Your issue set + the campaign evidence:

- #71 orchestrator wake (the top woken on park/doubt — suite orchestrator-wake-red 6/36 RED exists) + #181 member wake-on-signal (SIX instances today: signal recipients' turns ended before delivery — blue-team-b/suite-c/pm/dsh coordinators + lane-proof) — the park/re-arm + wake-with-message machinery as ONE contract. Evidence: docs/reference/evidence/channel-audit-2026-08-13/ + the lane-proof landing note

**Deliverable:** `docs/reference/evidence/lch-contracts-2026-08-14/redrive/contract-wake.md` ONLY (plus the shared publish — or the recorded refusal). Every acceptance pin RED at HEAD at a named stage, green only for a correct impl (shallow-greenability is a defect). Judgment calls recorded; authority-class ambiguity → DECISION_REQUEST with options.
