# Superseded README (verbatim, 2026-08-13)

This is the **complete, unedited** README as of commit-time 2026-08-13, preserved per the
operator's no-content-loss law when the systematic product README replaced it. Nothing here was
deleted — it was superseded. Section → new-home map:

- "The core question" / "What baton is" → README.md (refreshed product statement).
- "Status" (the 2026-08-04 Ring-2 numbers) → README.md "Status" (current, tiered) +
  docs/PROGRESS.md (the live ledger).
- "Architecture, plainly" → README.md "Architecture" (expanded, with diagrams) + SYSTEM.md.
- "Run it" → README.md "Run it" (the suite-count claims below are stale — see the new README's
  honesty note on red-first suites).
- "Design docs" table → README.md "Documentation map" (same rows, pruned commentary).
