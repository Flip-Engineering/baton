...
test('WAVE-ATTRIBUTION: concurrent missing results never assign one fallback pin to two members', async (t) => {
  const repoRoot = mkdtempSync(join(tmpdir(), 'baton-wave-attribution-'));
  ...
  git('add', '.');
  git('commit', '-qm', 'preserved shared tree');
  const sha = git('rev-parse', 'HEAD');
  git('update-ref', 'refs/baton/results/preserved', sha);
  const facade = { runs: { start: async (_objective, options) => ({
    id: `run-${options.waveRole}`, approve: async () => {},
    status: async () => ({ phase: 'completed' }),
    inspect: async () => ({ section: { items: [] } }),
  }) } };
  const wave = await createWave(facade, { repoRoot, members: ['alpha', 'beta'].map((role) => ({
    ...member(role), report: `${role}.md`,
  })) });
  for (let observation = 0; observation < 2; observation += 1) {
    const outcomes = await wave.settle();
    assert.deepEqual(outcomes.map(({ role, resultSha }) => ({ role, resultSha })), [
      { role: 'alpha', resultSha: sha }, { role: 'beta', resultSha: null },
    ], 'fallback attribution is exclusive and stable across refreshed observations');
  }
});
