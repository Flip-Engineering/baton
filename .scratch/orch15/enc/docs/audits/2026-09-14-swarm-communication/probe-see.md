...
The review was just as visible once it existed — and nothing told me it existed. Polling ~70 s later
showed my contribution accepted (`seq: 2080, "ts": "2026-09-14T04:41:48.958Z"`):
...
- Wakes are **not** generated by tool-call telemetry: a ledger walk stopped at seq 1867 while a view
  taken seconds later reported `cursor: 1884` — 17 intervening ledger rows that matched nothing a
  watch accepts. The filter names the excluded classes explicitly
  (`content.tool_call`, `route.observed`, `resource.*`, `swarm-runtime.mjs:382-384`), and those rows
  advance the head without waking anyone.
