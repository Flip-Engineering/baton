...
const tasks = [
  { taskId: 'phase49-codex-spec-review', harness: 'codex', model: 'gpt-5.6-sol', family: 'openai', target: 'reviews/dogfood/phase49-codex-spec-review.md', focus: 'replay integrity, atomic projection, idempotency, ACI preflight, and max+1 gates', tokens: 180_000, usd: 3 },
  { taskId: 'phase49-glm-spec-review', harness: 'glm', model: 'glm-4.7', family: 'glm', target: 'reviews/dogfood/phase49-glm-spec-review.md', focus: 'closed candidate taxonomy, Scratch poisoning boundaries, causal edges, and non-disclosure', tokens: 120_000, usd: 1.25 },
];
...
function brief(task) {
  return createBrief({
    goal: `Adversarially review committed Phase 49 spec at ${git(['rev-parse', '--short', 'HEAD'])}, focusing on ${task.focus}. Read spec/phase49/cairn-selective-promotion.md, spec/phase11/coordination-knowledge.md, docs/capabilities/coordination-repl.md, impl/src/coordination-store.mjs, impl/src/cairn-run-scorecard.mjs, and impl/src/capability-registry.mjs. Write ${task.target} with exactly the headings "## Verdict", "## P0-P1 findings", and "## Required contract corrections".`,
    constraints: [
      `Edit only ${task.target}.`,
      'Keep the report under 1400 words and use at most 12 repository/tool calls.',
      'Ground confirmed defects in exact source/spec behavior and propose a closed deterministic correction.',
...
function bounded(events) {
  ...
  return events.filter((event) => kinds.has(event.kind)).map((event) => ({
    ...
    payload: event.kind === 'lifecycle.spawned' ? { pid: event.payload?.pid ?? null, modelObserved: event.payload?.modelObserved ?? null }
...
const driver = createDriver({
  ...
  adapters: {
    ...
    glm: new GlmSessionCli({ authTokenFile: GLM_AUTH, authTokenJsonPointer: '/glm_key', model: 'glm-4.7', approvals: false, permissionMode: 'acceptEdits', args: ['--safe-mode', '--no-session-persistence', '--max-budget-usd', '1.25'], ceiling: 1, killGraceMs: 5_000 }),
  },
  runtimeIsolation: { credentialFiles: { codex: [CODEX_AUTH] } }, workerDependencyDirs: dependencies, verifyDependencyDirs: dependencies,
...
try {
  await Promise.all(tasks.map(async (task) => { const handle = await coordinator.spawn(task.harness, brief(task), { taskId: task.taskId, taskType: 'phase49-selective-promotion-spec-review', runId: RUN_ID, model: task.model, effort: 'low', modelPolicy: { allow: [task.model], allowFamilies: [task.family], reasoningEffort: 'low' } }); rows.push({ ...task, workerId: handle.id, handle }); }));
...
const overlap = windows.length === 2 && Math.max(...windows.map((row) => Date.parse(row.start.ts))) <= Math.min(...windows.map((row) => Date.parse(row.terminal.ts)));
