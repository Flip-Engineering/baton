...
## Revision 11: no bookkeeping ledgers in place of function (proposed; law review pending)

| | |
|---|---|
| Statement | A gate decides from observed runs only; no gate, test or check reads a hand-maintained record of expected results (expected-failure list, converged declaration, count or census pin) in place of running the software. A landing blocks exactly when a test fails with the change and does not fail on the target. |
| Enforcement anchor in Baton | `impl/scripts/expected-red-tests.json` and its guard tests, SI6 / `CORPUS_COUNTS`, count literals in tests; removed by 5df1acf5, c66098c1, #579 and #580. AGENTS.md bans the pattern. |
