...
if (git(['status', '--porcelain']) !== '') throw new Error('Phase 55 recursive build target must be clean');
if (!existsSync(GLM_AUTH)) throw new Error('project GLM credential file is missing');
if (!existsSync(join(REPO, 'impl', 'node_modules'))) throw new Error('pre-Phase-55 target lacks the temporary legacy dependency copy');
...
const adapter = new GlmSessionCli({
  ...
  args: ['--safe-mode', '--no-session-persistence', '--max-budget-usd', '4.5'],
...
const taskId = process.env.BATON_TASK_ID ?? 'phase55-glm-build';
const verification = 'node --test impl/test/phase55-toolchain-projection.test.mjs impl/test/worktree.test.mjs';
const brief = createBrief({
  goal: `Implement committed Baton Phase 55 at ${BASE_SHA.slice(0, 12)}. Read spec/phase55/immutable-toolchain-projection.md and make the committed red suite pass with a closed immutable bounded dual-root projection authority wired through worktrees, the driver, session validation, and verification evidence.`,
  constraints: [
    'Edit only impl/src/toolchain-projection.mjs, impl/src/worktree.mjs, impl/src/index.mjs, and impl/src/coordinator.mjs.',
    'Do not weaken, delete, skip, or edit tests or the specification.',
    'Preserve legacy same-root dependency behavior when the new option is absent and reject ambiguous mixed configuration.',
    'Never expose sourceRoot or source absolute paths; never follow links; enforce every independent bound; clean every partial copy/worktree/branch.',
    'Projection bytes must never enter a result commit, including through a worker git add or force-add.',
    'Do not inspect credentials/environment, use network tools, commit, push, deploy, or access homelab/project-manager.',
    'Run the focused verification before finishing. Keep the implementation dependency-free, deterministic, and fail-closed.',
  ],
  pathScope: ['impl/src/toolchain-projection.mjs', 'impl/src/worktree.mjs', 'impl/src/index.mjs', 'impl/src/coordinator.mjs'],
  definitionOfDone: 'All Phase 55 and worktree tests pass without changing tests or the specification.',
  verification: { command: verification, expectExit: 0, timeoutMs: 180_000 },
  budget: { tokens: 260_000, usd: 4.5, wallMin: 40 },
});
...
const responses = []; let pumping = true;
const pump = (async () => {
  ...
  while (pumping) {
    for (const worker of coordinator.list()) {
      ...
      if (requestId && !consumed.has(requestId)) {
        ...
        responses.push({ requestId, ack: await coordinator.respond(requestId, worker.pendingApprovalId ? { decision: 'allow' } : { text: 'Finish only the scoped Phase 55 implementation and run the pinned tests.' }, 'orchestrator') });
      }
    }
    await sleep(25);
  }
...
try {
  handle = await coordinator.spawn('glm', brief, {
    taskId, taskType: 'phase55-toolchain-projection-implementation', runId: 'phase55-toolchain-build',
    model: 'glm-4.7', effort: 'low', modelPolicy: { allow: ['glm-4.7'], allowFamilies: ['glm'], reasoningEffort: 'low' },
  });
  await until(async () => (await coordinator.result(handle.id)).ready, 'Phase 55 GLM result');
...
try {
  ...
  }, 'Phase 55 process and ownership reap', 45_000);
  closed = driver.close();
...
const exactRoute = Array.isArray(tuple) && publicHandle?.harnessRequested === 'glm'
  ...
  && tuple[2] === 'glm-4.7' && tuple[3] === 'low' && tuple[4] === 'glm' && tuple[5] === 'phase55-toolchain-projection-implementation';
