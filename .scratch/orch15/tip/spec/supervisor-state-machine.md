...
- **I1 (Fencing, turn-scoped).** Every control op carries the `(worker, turn_epoch)` fence it was issued against. The supervisor bumps `turn_epoch` on every `turn/start` and rejects any op whose fence < current with `stale_fence{current}`. This is how "human > orchestrator" actually works AND how a steer composed against turn N is prevented from splicing into turn N+1 (red-team `eventloop` A5): a human takeover bumps the fence; a stale orchestrator op or a stale steer is rejected. Idempotency keys dedupe; **fences order**. *(Turn-scoping was supervisor open-question #3; the red-team confirmed the stale-turn steer is real, so it is promoted into I1 now, not later.)* (Codex risk #2; red-team `ownership-concurrency` + `steer-correctness` A6 + `eventloop` A5.)
...
- **I3 (Durable cursors, at-least-once).** A read cursor is persisted BEFORE any data is returned. The durable cursor advances only on the *next* call's arrival (an implicit ack that the previous page was received), and callers dedup the resulting overlap by `seq`. This is deliberately **at-least-once, not at-most-once**: a `fleet_wait` whose waiter died must be able to *re-see* the page it never delivered, or an approval on that page is silently lost (red-team `eventloop` A1). At-most-once (advance-on-send) is the knife-edge bug; at-least-once + seq-dedup is the fix. (Codex risk #1.)
...
- **I6 (Two-phase stop, drain-first).** `interrupt`/`kill` are request→confirm. **Precondition:** drain the worker's outstanding approvals (answer `cancel`, retry until `serverRequest/resolved`) — a worker blocked on an approval when cancelled hangs otherwise (red-team `approval` A1). The op is not "done" until the supervisor observes the worker's authoritative stop event (Codex `turn/completed`-after-interrupt; Claude interrupt receipt reconciled against `still_queued`, with the nudge outbox flushed and any surviving `still_queued` armed with a re-interrupt tripwire). Until then the worker is `stopping`, not `idle`, and its worktree lease is NOT released — `fleet_spawn`/merge into it is fenced out (red-team `interrupt-liveness` A1). (Red-team `interrupt-liveness`.)
- **I7 (Verification is hub-run, not worker-reported).** A worker's result `{command, exit, tail}` is an unverified *claim*; the supervisor **re-executes the brief's verification independently, in the worker's (or a fresh throwaway) sandbox — never on the hub**, and treats only its own observed exit/output as authoritative. Worker prose and self-reported exit codes are non-authoritative; the "untrusted worker output" provenance frame is applied by the hub and is not quotable by the worker (red-team `adversarial-worker` A5; verification-execution-locus, doc 09 §C1/C2).
