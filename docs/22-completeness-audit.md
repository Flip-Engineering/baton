...
one repository-oriented deployment and bound-Run surface. It is still incomplete: readiness and
capacity must be deployment-owned, crash recovery and close/reap must be exact, and recursive
multi-harness live evidence must use that concise surface rather than a phase-specific runner.
