...
| Application scope | Open. Baton's gate is `defaultIntegrationGates` (#580). The model encodes failure identity, unjudged-verdict reporting, invocation accounting, and controls for forged outcomes, empty selection, mismatched failure kinds, missing verdicts and test selection; applying them to the runtime gate, with outcome provenance, selection and the transition composition, remains open. Selection and test-contract changes do not authorize omission of their own failures; the repository test-policy clause is enforced separately. |
