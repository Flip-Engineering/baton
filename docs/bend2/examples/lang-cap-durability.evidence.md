...
The programs and this record were recovered from the preserved worktree of
`bend2-language-lead4`
...
bend docs/bend2/examples/lang-cap-durability.bend --check-only
bend docs/bend2/examples/lang-cap-durability.bend
bend docs/bend2/examples/lang-cap-durability.bend -o <scratch>/lang-cap-durability && <scratch>/lang-cap-durability
bend base File
bend docs/bend2/examples/lang-cap-durability-sync.bend --check-only
bend docs/bend2/examples/lang-cap-durability-stat.bend --check-only
bend docs/bend2/examples/lang-cap-durability-rename.bend --check-only
...
Outputs in order: `All terms check.` (exit 0); `write and close completed; Base returned Unit`
(exit 0) with the 18-byte output file; a 1,128,216-byte executable that exited 0 with the same
line; the same eight `def File.*` lines; and the same three refusals naming `File.sync`,
`File.stat` and `File.rename` as undefined with exit 1 each. The reproduction matches this
record's outputs and the retained executable size exactly. The run's
`lang-cap-durability-output.tmp` artifact is removed after each run so the examples directory
carries no generated file.
