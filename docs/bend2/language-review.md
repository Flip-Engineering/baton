...
## 10. Reconciliation of the preserved draft

An untracked draft of this review (302 lines) was preserved at
`refs/baton/preserve/uncommitted/bend2-language-core-host/20260922T172619Z` (tree `20bb1747`) from
the orphaned lane worktree, and reconciled into this document. Its disposition, for a reader of
this branch:

- Folded in, each claim verified against the pin before landing: LANG-F-30 (theory shape:
  `Type : Type`, no positivity check, the live/dead consistency wall, no traits or macros, closed
  templates); LANG-F-31 (fail-stop conditions outside `Result`, with the WONTFIX numbers); the
  no-ABI rebuild obligation, the proof-boundary sentence and the JavaScript `_need`/`io_park_on`
  clauses in LANG-F-08; the #825 handle-law mechanism in LANG-F-09; the clang-only C output in
  LANG-F-25; the filesystem durability and metadata prerequisites in LANG-CAP-01; the
  linked-list-strings byte-buffer note in LANG-CAP-06; the HTTP/HTTPS/TLS and Unix-socket gaps in
  LANG-CAP-08; and new LANG-CAP-09 (cancellation, deadlines, race/select, supervision) and
  LANG-CAP-10 (hashing, HMAC, secure random bytes, constant-time comparison, signatures).
- Superseded by this document and not folded: the draft's scope-and-evidence table
  (`examples/index.md` carries it); its process-handle wording (LANG-F-28 is the probe-corrected
  form); its fibers/`IO.spawn` paragraph (LANG-F-07, LANG-F-15); its foreign-host-contract bullets
  (LANG-F-08); its concurrency measurement table (LANG-F-11, the same numbers); its modules and
  tooling sections (LANG-F-19, LANG-F-20, LANG-F-23, LANG-F-24); and its capability and
  consequences lists (sections 8 and 9).
- Not folded, unverifiable at the pin: the draft's claim that F32 arithmetic is axiomatic.
  `WONTFIX.txt` carries only the JavaScript-lane NaN-payload entry (#797); the laws lane may
  revisit the claim if float arithmetic enters a law.