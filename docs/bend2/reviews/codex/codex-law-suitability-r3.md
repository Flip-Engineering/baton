...
Governing clarification: [select a minimal set of inviolable behavioral prohibitions](operator-minimal-law-scope.md). The classifications below are an initial assessment of the extracted material. Even a Candidate must still justify inclusion in the smallest necessary set of rules preventing implementation agents from introducing banned behavior.
