...
The production target is a native Bend2 deployment using Base effects and declared C imports.
Phase 7 removes the temporary JavaScript boundary and Node runtime.
...
| `B2-FS-DURABILITY` | `LANG-CAP-01`, `LANG-F-29`: metadata, permissions, safe temporary files, links, atomic publication, file and directory sync, traversal, and crash recovery. Bind durable acknowledgment to the actual write path. | Workspace and Artifacts / Journal; any native acceptance or append, with full storage cutover in Phase 6. |
...
The proof corpus covers the 16 approved laws using the coverage table above. Historical
`CS-*`, `AB-*`, `PM-*`, `WAKE-*`, and `LEDG-*` traces in the design notes locate current behavior
fixtures. Current behavior that violates an approved law receives an explicit expected correction
and a regression case; baseline parity alone cannot certify that behavior.
...
Verification and Landing takes contribution/review state, capture/check eligibility, required-gate
selection, landing plans, and projection of local integration and shared publication. Workspace and
Artifacts executes scoped Git operations; Journal and Projectors records their distinct outcomes.
JavaScript retains Git, scratch checkout, regeneration, runner, local-ref, and publication execution
until Phase 6. Entry requires the F16/F17/F20 architecture decisions, `B2-DESTINATION` planning
and adapter evidence, Phase 3 recovery, and the applicable approved law proofs.
...
It also binds the canonical shared repository and target designated by admitted authority, endpoint
identity evidence, and configuration revision. Revalidate those coordinates at publication dispatch.
An unresolved destination identity produces no publication effect.
