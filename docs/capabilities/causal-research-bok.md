...
- **The BoK is a hub-owned, bi-temporal causal shadow-graph fed by selective promotion from the event ledger, and drained by export into the user's `project-manager` (PM) KG.** Baton does not reimplement PM; it *stages* into a PM-shaped graph and promotes across the boundary only after an audit passes. The sharp line: **baton owns a fast, run-scoped, machine-written staging graph; PM owns the slow, months-scale, curated permanent record.**
...
4. **The write cadence is machine-fast but the value is human-slow.** A fleet emits thousands of events/minute; PM-style findings are hand-curated "200+ char lab reports." Neither harness memory nor PM bridges this: harness memory ingests *everything* (unbounded, unaudited); PM ingests *deliberately* (can't keep up with fleet tempo, assumes a single curator). The missing piece is a **selective, automatic promoter** with a provenance-integral backbone — fast enough for the ledger, disciplined enough to be worth querying, and drained into PM at run boundaries so PM stays curated and baton stays lean.
...
| **project-manager (user's MCP)** — the target | Typed research KG: Phases/Experiments/Findings/Decisions/Hypotheses/Principles/Constraints/Literature; edges `Supports`/`Contradicts`/`Supersedes`/`Informed`/`ProducedBy`/`Contains`; enforced causal backbone; temporal-coherence audit; 0–100 health scorecard (6 axes). Live: volta-renaissance 1522 nodes / 2522 edges / **70/100** | The user's shipping tool; baton's **export target**, not a competitor | The entire epistemic **contract**: causal backbone, temporal integrity, `Supersedes`/`Contradicts`, the 6-axis scorecard. Baton emits into it via `pm_log_finding`/`pm_decision`/`pm_add_edge` | Reimplementing it; its human write cadence (200+char findings), single-curator assumption, topic-similarity 1-hop retrieval as *worker* context |
...
- **`bok_export` is single-flighted** — one writer to the durable PM graph at a time (PM is single-curator-shaped; concurrent exporters would corrupt its narrative).
