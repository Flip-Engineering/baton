...
Grounding paths: `$HOME/Development/Experiments/baton/spec/capability-plane.md` (the contract this dossier must conform to — `card`/`invoke`/`resume`/`cancel`/`reverify`, the ACI envelope, §4 staleness = overlay model, §8 MVP), `$HOME/Development/Experiments/baton/spec/supervisor-state-machine.md` (I7 sandbox-not-hub locus at line 29; MVP overlap), `$HOME/Development/Experiments/baton/docs/05-telemetry-steering.md` (closed kind taxonomy the invented `verify.*`/`attest.*` events violate), `$HOME/Development/Experiments/baton/docs/10-interaction-model.md` (T3/stigmergy basis for the counterexample-corpus and conjecture-market opportunity), `$HOME/Development/Experiments/baton/docs/08-shared-memory-and-pm.md` (task-DAG ready-work + artifact registry the module plugs into).
