...
- `knowledge.reuse_policy_reconciled` `{repoId, expectedPolicyVersion, previousPolicyHash, policy, policyCardDigest, decisionTargets, bindingTargets, findingTargets, guardTargets, constraintId}` (Coordinator/store-derived only; no caller target or policy authority)
...
- The reuse registry has one deployment-card-derived active policy head. The first baseline binds matching pre-head Decisions and closes unknown or mismatching legacy state; later policy changes derive and append their complete bounded target sets inside the store under one exclusive writer lifetime.
...
- Phase 44 adds the first Cairn cross-run learning projection: one immutable verified `RouteStat` per terminal task, linked to that task through `ObservedIn`, under an exact deployment policy. Restart hydration replays ordered observations without double-counting, and bounded `route.advice` reads the same evidence through the sole ACI plane. It is local and deployment-neutral, accepts no caller outcomes, and does not generalize narrow task evidence into broad model-family Findings.
...
| Reuse-decision cache / allowlist | **Fleet** | Append-only Decisions plus monotonic policy heads | Subject binds configured repo/effective tree, normalized need, exact coordinate, Atlas epoch, and policy hash; all authority writes serialize under one exact-token writer lease, policy activation derives bounded complete targets inside the store, and overlapping live writers fail closed; causal edges are local and deployment-neutral |
...
- **MVP rung (smallest useful).** `fleet_orient` served from an **Aider-style tree-sitter+PageRank map**, cached per `(repo, sha)` as a registry artifact — *grounded on installed tools*: `ctags -R` for definitions + `rg` for references + `networkx` PageRank in `python3`, no tree-sitter dependency needed to start. Plus `fleet_reuse`/`fleet_vet` backed by deps.dev `GetVersion`, optional separate `GetProject` Scorecard enrichment, and an OSV exact-version/malicious-package query. The Coordinator-owned immutable decision transaction and deployment-card-derived policy-epoch reconciliation now ship: they freshly reverify exact evidence, project deployment-neutral causal knowledge, close stale authority on policy change, and grant no install, mutation, policy-authoring, or clearance authority. `fleet_orient_worker` ships here too; it's cheap once slices exist and it's the differentiator.
...
1. **Safety closure — shipped Phase 42:** deployment-card-derived policy baseline/change reconciliation, bounded atomic Decision/Finding/read/guard fan-out, legacy binding, stale-but-blocking adverse migration, exclusive writer lifetime, exact replay, and local causal Constraint lineage. It grants no clearance or caller policy authority.
...
Provider push, clearance, internal decisions, plan approval, true function
