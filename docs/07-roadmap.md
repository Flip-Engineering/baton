...
2. Dogfood Baton on Baton continuously: route multiple workers in parallel, select exact
   ...
   refreshed on 2026-07-18 and exact `kimi-code/k3`/high is live under a Phase 81 dogfood Wave;
   Grok remains separately auth-red. No provider-success claim is borrowed from readiness alone.
...
4. Build the common Context Program and Scratch Bench as the context-computation layer for dynamic
   ...
   Next is durable cell admission plus ContextManifest/Atlas/Scratch partition compilation into
   existing WorkItems/Waves/Attempts, followed by model-backed depth-one map/reduce, replay,
   stop/reap, transport parity, and direct-vs-context-vs-RLM evaluation. No ambient shell, host
   `exec`, hidden provider callback, caller route/budget knob, or shared mutable checkout is added.
