...
3. Build durable dynamic workflow composition over the existing Run application: one approved
   ...
   The bounded Plan-v2 vertical is now live-green through Baton itself with two parallel Codex
   Candidates, exact feedback, approval-gated revision, a distinct retained revision commit, and
   zero final ownership. Next is the adverse recovery/stop matrix and one policy-admitted v3 round;
   arbitrary-depth recursion is not yet claimed.
4. Deepen the capability and representation plane from the shipped bounded R1 structural-delta,
...
5. Extend the shipped Cairn causal/temporal primitives into the deployment-neutral shared typed
