...
> close exact Plan-owned harness/model/effort route tuples. Phase 89 has now landed the first
> resident-application vertical: one owner/connected Runs facade, authenticated bounded catalog,
> validating attach, stable progress timing, repository-bound discovery, hardened Web reads, and
> an explicitly advanced loopback host seam. The ordinary private resident host, durable control
> settlement, Run-scoped streams, restart publication/lease fencing, and browser/CLI convergence
> remain acceptance-red. The
...
1. Finish the concise `openBaton`/bound-Run surface, contextual doctor/help, deployment-owned
   ...
   `connectBaton`, stable progress timing, and advanced loopback-host seam are green. Ordinary
   `openBaton().host()` remains deliberately unavailable until Baton owns the private socket,
   credential/session issuance, fenced resident lease and incarnation, atomic publication,
   authenticated self-check, restart recovery, and compare-and-swap cleanup.
...
4. Build the common Context Program and Scratch Bench as the context-computation layer for dynamic
   ...
   vertical. Semantic send/interrupt, durable control settlement, Run-scoped resumable streams,
   ordinary resident hosting, and deeper recursive composition remain next, followed by
