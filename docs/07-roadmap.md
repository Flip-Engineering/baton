...
> zero-assembly `baton serve`. Durable semantic control settlement, Run-scoped streams, opaque
> catalog continuation, persisted progress indexes, and browser control convergence remain
> acceptance-red. The
...
4. Build the common Context Program and Scratch Bench as the context-computation layer for dynamic
   ...
   opaque catalog continuation, persisted progress indexing, and deeper recursive composition
   remain next, followed by
