...
1. Finish the concise `openBaton`/bound-Run surface, contextual doctor/help, deployment-owned
   readiness and capacity, crash recovery, exact stop/reap, and Web/MCP parity.
2. Dogfood Baton on Baton continuously: route multiple workers in parallel, select exact
   ...
   warranted).
3. Deepen the capability and representation plane: Atlas AST/CST, symbol/SCIP, CPG/dataflow/taint,
...
4. Complete the deployment-neutral shared typed causal knowledge graph inspired by
...
5. Run and publish the routing, control-latency, cross-vendor decorrelation, recovery, and
