...
| `swarm.recruit` | `swarmId`, `participantId`, `objective`, `options`, `permissions`, `shareWorkspaceWith`, `resumeFrom`, `idempotencyKey`, `view` | `control`, `observe` | web + mcp | `idempotencyKey`, reconcilable |
...
**Attention kinds (closed set, 13).** Each view row is a condition that needs an act, derived by the
