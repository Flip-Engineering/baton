...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  settlement above and preserves it across restart and Run stop. The provider-result capsule core
  ...
  substituted results fail before usable projection. Durable accepted-child attachment remains the
  next gate: the application must rederive capsule inputs from coordination rather than treat the
  projector's input as acceptance authority. Phase 85 retains the remaining load-bearing order:
  attach accepted provider-result capsules durably, preserve the root semantic role catalog
...
5. **Complete recursive Context and adaptive workstream composition without a second engine:**
   ...
   evidence-only failed-generation settlement, and private retained-commit capsule core are shipped.
   Continue its fixed dependency order—durable accepted-child capsule attachment, root role catalog,
   generic `map | reduce`, workerless
