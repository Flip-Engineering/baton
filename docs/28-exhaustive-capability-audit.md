...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  `recovery_terminalized` for both Codex and GLM while still closing with zero workers. The latest
  counted complete suite is green at 2,107/2,107. Baton dogfood retained exact Codex high/xhigh Candidates and reaped both process
  ...
  observed/closed both process records, and left zero workers. Generic Plan dispatch and reduce
  provider effects remain honestly closed. Phase 85 retains the remaining load-bearing
  order: propose/approve/dispatch the generic successor Plan, add
...
5. **Complete recursive Context and adaptive workstream composition without a second engine:**
   ...
   v3-only derived call source are shipped. Continue its fixed dependency order—sole-authority
   generic admission/reduce, projection-failure
