...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  authority. The combined adjacent matrix is 33/33 and the complete suite is green at 2,093/2,093
  after one earlier isolated process-lifecycle timeout passed its exact and later complete reruns.
  Current map settlements still lack exact result-output lineage, so durable generic admission and
  reduce provider effects remain honestly closed. Phase 85 retains the remaining load-bearing
  order: make the generic envelope the sole new durable call authority, add
...
5. **Complete recursive Context and adaptive workstream composition without a second engine:**
   ...
   generation-1 `map | reduce` identity core are shipped. Continue its fixed dependency order—exact
   result-output lineage and sole-authority generic admission/reduce, projection-failure
   terminalization, workerless
