...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  also ship, together with the authenticated contradiction workspace. Phase 81 now ships the first
  closed stateless Context Program/Bench substrate locally: immutable tree-bound manifests,
  canonical pure-operation ASTs, content-addressed cell output, integrity refusal, and a compact
  ContextSession. Durable ledger cells, Atlas/Scratch partition-to-Wave compilation, model-backed
  depth-one map/reduce, replay/stop union, transport parity, and evaluation remain pending. A later
