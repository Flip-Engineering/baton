...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  tree-bound manifests, canonical pure-operation ASTs, content-addressed source/output/evidence,
  ...
  `task.resources_released` coordinate per child before aggregate settlement. Failed children cannot
  be overclaimed, recovery converges across admission/Plan/Wave/result/CAS/settlement boundaries,
  Run stop targets live Context calls and proves zero remaining Context ownership, and direct,
  generic CLI, authenticated Web, and MCP use the same application action and inspection cascade.
  The focused Phase 84 matrix, affected lifecycle/transport matrices, and the 2071-test complete
  ...
  honestly blocked by invalid external authentication metadata rather than bypassed.
  Phase 84 does not ship reduce, semantic review, deterministic verify, recursive retry, exact
  per-output lineage, Atlas/Scratch/knowledge-graph effect branches, or a general programmable
  orchestration loop. Phase 85 retains the load-bearing order: persist per-output lineage, preserve
  the root semantic role catalog across successor Plans, generalize `map | reduce` effect calls,
  settle and release failed generations before selective retry, consolidate pure evaluation behind
...
5. **Complete recursive Context and adaptive workstream composition without a second engine:**
   ...
   Execute Phase 85 in its fixed dependency order—per-output lineage, durable root role catalog,
   generic `map | reduce`, failure cleanup before selective retry, unified `context_eval`, then live
