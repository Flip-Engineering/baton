...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  The focused Phase 84 matrix, affected lifecycle/transport matrices, and the 2084-test complete
  ...
  lineage rather than an aggregate union. The next slice adds the durable evidence-only failed-call
  settlement above and preserves it across restart and Run stop. It deliberately does not yet admit
  retry: an accepted child first needs a private provider-result capsule so a later generation or
  reduce consumes actual grounded content. Phase 85 retains the remaining load-bearing order:
  project accepted provider results into private capsules, preserve the root semantic role catalog
  across successor Plans, generalize `map | reduce` effect calls, add workerless non-admission and
...
5. **Complete recursive Context and adaptive workstream composition without a second engine:**
   ...
   Phase 85's exact per-output pure lineage, map-v2 projection, all-terminal resource release, and
   evidence-only failed-generation settlement are shipped. Continue its fixed dependency
   order—provider-result capsules, durable root role catalog, generic `map | reduce`, workerless
