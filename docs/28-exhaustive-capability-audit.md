...
- Scratch and causal knowledge primitives ship. Cairn Rungs 0–2 scorecard, verified RouteStats,
  ...
  and cancelled children now retain the same exact resource-release evidence; failed calls still
  need their own durable failure-settlement event. Failed children cannot be overclaimed, recovery
  ...
  lineage rather than an aggregate union. Phase 85 retains the remaining load-bearing order:
  preserve the root semantic role catalog across successor Plans, generalize `map | reduce` effect
  calls, durably settle and release failed generations before selective retry, project accepted
  provider results into private capsules, consolidate pure evaluation behind
...
5. **Complete recursive Context and adaptive workstream composition without a second engine:**
   ...
   Phase 85's exact per-output pure lineage and map-v2 projection are shipped. Continue its fixed
   dependency order—provider-result capsules, durable root role catalog, generic `map | reduce`,
   durable failure settlement before selective retry, unified `context_eval`, then live
