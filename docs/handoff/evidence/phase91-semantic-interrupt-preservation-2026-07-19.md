...
3. Interrupt confirmation alone is insufficient. Receipt creation requires exact session identity,
   ...
   Plan/Run bindings, resolved interaction authority, and valid provider terminal governance.
...
physical stop-before-successor race but not durable Application `run.stop` admission. The integrated
