...
> **Review round 2 caveat (doc 13 T3/T5/T6).** Two claims in this doc were deflated by the red/blue/explore pass. (1) **"Emergence" (§4) is CUT as a design claim** — emergence is an observation category, not an architecture: *detect* it (a hub-verified op-chain matching no template, kept only if the reuse counter fires), don't *engineer* it. The uniform envelope survives as **interop**, not as "the emergence engine." (2) **The ensemble economics (§3) are REVISED** — pass@N lift is a *repeated-sampling* phenomenon best captured **inside one vendor** (cheap, shared KV-cache); cross-vendor best-of-N is *not* the asset. The cross-vendor-unique value is **learned routing (1×) + cross-review (1×, proven)**, not best-of-N generation. What survives intact and load-bearing: **§1 context composition** (provenance-typed context, the compaction firewall, push-minimal/pull-by-handle) and **§2 semantics-invariant/syntax-projected tools + deferred loading**. See doc 13 and the honest project center of gravity, the **Referee** (doc 13 T5).
