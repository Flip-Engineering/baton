...
reported `@randomlabs/slate` 1.0.42 as a small launcher package selecting proprietary platform
packages. The Darwin arm64 package reported about 118.5 MB unpacked. The package has no substantive
npm README and declares a proprietary license. These facts show that the current runtime is shipped
as a closed native artifact; they do not reveal its scheduler, compactor, state store, sandbox, or
recovery implementation. No binary decompilation is used here.
