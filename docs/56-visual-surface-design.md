...
Stage 3 implements the set above, with the four changes the root orchestrator's review of stage 2
added: the root wake message is designed rather than printed as raw JSON; `baton statusline`
answers from one bounded read, finds the resident from any checkout of the repository, and leads
with what is owed to the root; progress messages name the latest event; and the mark follows one
rule wherever Baton speaks to a person.
...
on a TTY for stderr, `baton top` reads it for its header, the status line reads it, and the root
wake message reads it. Nothing else spells the mark.
...
`ready`, which is what lets the status line read a swarm frame's own status. Event-shaped classes
...
| D2 | The blocking reads wait in slices and offer a derived progress frame between slices: `progress` is the count of observations this call made, `total` is absent, and the message names the latest observation (`waiting on fleet events · no events yet · 12s of 25s`, `waiting on run events · last #12 running · 3s of 25s`). A slice that observed anything ends the call, which is the leg's own exit, so the fleet wait's and the run follow's frames read `no events yet` and the run wait's name the moved view. A call with no token, and a transport with no sink, behave exactly as before. |
| D3 | New CLI verb `baton statusline`. One bounded read: `swarm.list`, then one `attention` slice per swarm (never the participant roster, a run view or a list furnace). The line leads with what is owed to the root — the `root_wake_undelivered` rows the read carries — then the attention count and the served commit; the status word derives from the attention rows, else the swarm's own status. An absent resident, a refused read or a cwd outside any checkout prints nothing and exits 0: the harness shows no line rather than an invented one. The settings stanza lives in `baton statusline --help`, not in the operator's settings — Baton never writes those. |
...
| Root wake | `rootWakeBody` (wake-delivery.mjs) composes the message the root reads inside its own session through the ONE mark rule and carries no JSON: the owed class, the seat and its contribution, the ask when one is addressed to the root, and the one runnable next command, each line bounded and the whole body bounded. A delivery the root's session never receives is unchanged in the ledger. |
...
D3's design named a README stanza. The settings stanza is taught by `baton statusline --help`
instead: it is the surface an operator reaches by asking about the verb, and the README describes
the product rather than one harness's chrome configuration.
