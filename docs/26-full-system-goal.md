...
- **Phase 81 — common Context Program and RLM-style externalized-context strategy active:**
  ...
  search/chunk/coverage cells, content-addressed artifacts, source-integrity refusal, replay
  identity, and contextual help. It deliberately does not claim durable ledger cells, dynamic
  model-backed map/reduce, ContextManifest/Atlas/Scratch partition-to-Plan/Wave compilation,
  recovery/stop union, transport parity, or RLM utility evaluation yet. Those are the next gated
  slices; depth greater than one and persistent shared kernels remain closed.
- **Phase 81 live dogfood — useful friction, not a polished success claim:** the first ordinary
  ...
  recovery/reap proof, not a successful Kimi Candidate or successor-Plan proof.
