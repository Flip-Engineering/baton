...
- **Phase 79 — dynamic workflow composition specified; implementation active:**
  ...
  and replay/recovery. Parallel workers share immutable bases, addressed context, Atlas/artifacts,
  ...
  full-permission same-UID posture. Phase 79A first closes group `status()`/`complete()` and
  selective-member AX; later slices add durable workflow admission/scheduling, feedback/synthesis,
  workspace composition, causal projection, and transport parity. This is one multi-node Run
