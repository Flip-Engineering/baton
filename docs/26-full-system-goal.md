...
- **Harness matrix — red where measured:** the integrated deployment currently discovers the
  authenticated Grok route, but a literal Grok Build request is still provider-observed as
  `grok-4.5` and therefore rejected as an exact-model mismatch. Its accepted concurrent
  start/kill/reap proof remains red rather than borrowing the historical Composer proof. Claude,
...
- **Phase 78 — integrated deployment surface active, not a capability-plane completion claim:**
  ...
  The remaining closure gates are deployment-owned dependency/verification readiness, host
  capacity admission, crash-safe owner recovery, contextual doctor/help, and recursive live proof
  through the same surface. That proof must exercise multiple harnesses in parallel, select exact
  ...
  callers do not supply budgets, file-size ceilings, temporary roots, or capacity knobs.
