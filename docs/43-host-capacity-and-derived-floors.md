...
- A runtime scope the resident cannot remove yet is a pending removal, at the start and at the stop. `RuntimeIsolation.reconcile` reports every scope it could not remove on the `runtime_cleanup_failed` error's `pending` list, the coordinator records one durable `host.cleanup_pending {code, reconciler, record, observed}` row per scope, keeps the set on `coordinator.cleanupDeferred()`, and reconciles exactly those scopes again — never the whole sweep, so a scope a worker admitted since the pass is untouched — on the reap grace until each is absent, which lands one `host.cleanup_completed {reconciler, record, attempts}` row. The same deferral wraps the drain's historical runtime-scope reconcile (`_reconcileDrainRuntimeScopes`), so a stop converges with the leftover named on the ledger and on the read for the next start to remove. The retry count is unbounded and the loop's timer is unref'd, so a start proceeds and a stop converges while a killed worker's child is still releasing its scratch (`ENOTEMPTY` for one filesystem turn), and nothing awaits the loop.
