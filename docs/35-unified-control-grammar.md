...
- **F5 — Twenty phase strings** across runs, waves, workflow projections, and the CLI terminal
  ...
  `wait`/`follow` on a run resting in either selection state blocks past provider settlement
  today (v2 acceptance finding).
...
|---|---|---|
| read | `view`, `watch`, `list`, `help` | `view` = one bounded view at a depth, **optionally change-aware** (`--cursor N --wait D` — the registry's own preferred continuation, `application-semantics.mjs:135-140`) **or condition-awaiting** (`--until settled\|terminal`, absorbing `run.wait`'s deployment-bounded settle-block, R-OP-9/R-KM-2 — **and its two values bind to the two registry predicates, not to prose**: `settled` ⇔ `providerSettled(phase)`, `terminal` ⇔ `applicationTerminal(phase)` (§7.1, L4). Unbound, `terminal` reads as "any terminal union", which is exactly the R-CX-4 collapse and exactly the bug A-7 found already shipped at `application-cli.mjs:29`. R-CX-2 asked for a third value `change`; it is deliberately absent — change-awareness is the `--cursor N --wait D` continuation above, and the two mechanisms must not be merged, v2 acceptance); `watch` = the only **event-channel** read (`channel: progress\|events\|output\|changes`, `--to RECIPIENT` for output, inherits `followPolicy` gating, per-channel cursors, `afterCursor` + `timeoutMs` (`application.mjs:137`), the bounded categorized change page, and post-wait reauthorization — pinned as **schema**, not merely as a name, R-CX-2); `list` = bounded collections; `help` = self-description |
...
**`application-cli.mjs:29` is not merely a `closed`-deletion site** (v2 acceptance): the whole set
must be replaced by a `providerSettled()` call at M2. It is a mis-named provider-settled union
that omits `selection_required` and `candidate_selected`, so the CLI's wait/follow stop condition
(`:1029,1923,1945`) already returns wrong liveness for both selection-resting states. Deleting
`closed` from it alone would leave a third hand-maintained union standing — precisely the failure
mode L4 exists to end.
