...
try {
  baton = await openBaton({
    ...
    advanced: {
      ...
      verification: {
        command: 'node',
        arguments: ['--test', 'impl/test/phase84-*.test.mjs'],
      },
    },
  });
  ...
  const sourceCell = await context.search('context_map_child_failed', {
  ...
  const releaseProofs = releaseEvents.map((event) => {
    ...
    return { release: event, mapped, operational };
  });
  if (!callAdmission || !callSettlement
    ...
    || releaseProofs.some(({ release, mapped, operational }) => (
  ...
  record = {
    ...
    resourceReleases: releaseProofs.map(({ release, mapped, operational }) => ({
