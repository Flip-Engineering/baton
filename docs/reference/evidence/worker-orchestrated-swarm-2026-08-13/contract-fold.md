...
- **Verification HEAD:** `3dd41439225267843597ef7ad8820f9b39b74d61` ("Baton private effective-tree
  snapshot"), the tree this v1.2 fold was verified against (the v1.1 fold was verified at
  `7e68187741369c207835b7ce98565030429eddaf`; the snapshot history is rewritten, and the blue-team
  §3.1 measured the +4/+14 drift that landing re-based — every `file:line` citation below is
  re-verified at the v1.2 HEAD, not inherited). The two NUL-bearing files whose anchors are
  grep/sed-verified, never whole-file reads: `application.mjs` and `coordination-store.mjs` (3 NUL
  bytes each). `coordinator.mjs`, `claude-session.mjs`, `mcp-northbound.mjs`,
  `workflow-interpreter.mjs`, `recipes.mjs`, `application-cli.mjs`, and `application-deployment.mjs`
  are NUL-free and read with plain `grep`/`sed`.
- **Fold source:** v1.0 (`worker-orchestrated-swarm-contract.md`, same dir) + the adversarial
  red-team report (`contract-redteam.md`, same dir) + the blue-team suite review
  (`suite-blueteam.md`, same dir — the v1.2 fold). The v1.1 fold applied every red-team blocker
  fix, the three citation-drift corrections, and every amendment/note; the verdict'd-SOUND
  substance (D1 double-claim and fabricated-results seams, D3 seat discipline, the two-level
  posture) is carried byte-stable in substance. The v1.2 fold applies the blue-team's three
  blockers (the D1.2 fixture seam, the D1.3 permanence + re-attempt policy, the A2 wave-scoped
  grant path), the §1.2a denied-record shape (`optionId?`/`text?`), the §3.2 static-anchor ruling
  (drop tight absolute line windows, keep ORDER/EXISTENCE/byte-string), the §4.1 driveLane-wide
  counter scan, and the §4.3 file-not-directory mechanism correction + structural enforcement.
  The fold-map tables below route every finding to its resolution; the topology audit
  (`comm-topology-audit.md`, same dir) is context, not re-litigated here.
...
| drift 2 — `coordinator.mjs:11234-11243` (`submitBoardReport` starts `:11247`) | must-fix | corrected — every citation updated to **`coordinator.mjs:11234-11256`**: G11, D1 artifact conventions. *(v1.2 drift note: the board worker-half has since re-landed at `:11453-11475` — G11, D1 artifact conventions cite the v1.2 HEAD.)* |
| drift 3 — `application.mjs:11613-11617` (roster built `:11610-11614`, `route` at `:11612`) | must-fix | corrected — every citation updated to **`application.mjs:11610-11614`** (route **`:11612`**): G2, D3, A6. *(v1.2 drift note: the roster build has since re-landed at `:11617-11621`, route `:11619` — G2/D3/A6 cite the v1.2 HEAD.)* |
| A5 GREEN over-states the gate. OQ1 is CONFIRMED: `waves.*` direct ports dispatch before the recursive-session gate (`application.mjs:12502-12512` vs the gate's `context?.sessionAuthority` check at `:12527-12532`), so a lease-bound `waves.start` draws no #12 refusal. *(v1.2 drift note: re-verified at the v1.2 fold HEAD — dispatch `:12506-12516`, gate `:12531`, throw `:12535`.)* | amendment → folded | **§D2 full shape** states the pre-gate finding; **A5's GREEN** narrowed (the #12 codes are NOT claimed for `waves.*`; the full shape needs `waves.start`/`waves.run`/`waves.stop` added to the recursive gate, or explicitly refused for lease holders, at the dispatch seam). **OQ1 → answered** with the code finding carried into §D2. |
...
## Fold-map (v1.1 blue-team → v1.2)

| Finding (from `suite-blueteam.md`) | v1.1 verdict | Resolution — where in v1.2 |
|---|---|---|
| **BLOCKER §1.1 — A1/A2 fixtures install the permissive authorize**, so the D1.2 sibling-refusal legs can never go green (the deployment seam `authorize: async () => true` is bypassed by the hermetic fixture). | NEEDS-FOLD | **§D1.2 — the enforcement seam is now double-pinned**: the suite's fixtures install the restricting authorize at the seam's stand-in (so the sibling-refusal legs are REACHABLE and pass hermetically), and a STATIC pin requires the permissive literal to be GONE from the deployment seam (`application-deployment.mjs:2012`). A1/A2 red move to that static pin. |
| **BLOCKER §1.2a — the denied-record shape lacks the attempted option.** §D1.3 records `{…, outcome:'denied', refusal:<code>}`; a faithful impl that drops the attempted option fails A3. | NEEDS-FOLD | **§D1.3 — the denied record carries `optionId?`** (and `text?` for the free-text path), so the audit knows which option was attempted. The refusal-vocabulary row is raised to match. |
| **BLOCKER §1.2c / §2.1 — the re-attempt policy and the permanence half are unpinned.** Removing the pre-answer `s.answeredKeys.add` makes the drive loop re-attempt a denied ask on EVERY poll (a `denied` record per poll, masked by the suite's `.find()`); an impl can record `denied` while still marking the key handled. | NEEDS-FOLD | **§D1.3 — the re-attempt policy is stated**: a denied decision is recorded ONCE per requestId and never re-auto-answered — the interpreter skips `answerDecisions` for a requestId it has already denied (the ask stays pending; a later human answer settles it). The permanence pin is structural: the pre-answer `s.answeredKeys.add` is absent from the drive loop, and the trail carries exactly one `denied` record per requestId. |
| **BLOCKER §2.2 — the wave-scoped grant path is unasserted.** A refuse-everything restrictor (no grant surface) passes A2. | NEEDS-FOLD | **§D1.2 pt 3 — the grant path is asserted REACHABLE**: the suite's grant-aware restrictor mints an explicit wave-scoped grant and asserts a granted swarm-row read of the coordinator's partition succeeds; over-refusal fails. |
| §1.2b (§2.1 permanence) — see the §1.2c row (same fold). | NEEDS-FOLD | folded into **§D1.3** (above). |
| §1.3 — A5 seat-class: a hardcoded allowlist of the four fixture principals would pass. | SOUND + minor | **§D2/A5 — the suite adds a second top-orchestrator principal** (`s74-observer`) asserting it too can start a wave — the boundary narrows the worker SEAT, not identity. |
| §1.5 — A8 delivery unpinned: an impl that widens only the interpreter's admission set passes while the coordinator boundary (`coordinator.mjs:6864`) silently drops `brief`/`result`. | NEEDS-FOLD | **§D4/A8 — delivery is pinned end-to-end**: a `messageOnSpawn` steering entry with a DELIVERED `messageId` and the `signalOnMembersDone` recipients force the coordinator boundary to accept `brief`/`result`, not just the interpreter's closed kind set. |
| §3.2 — the static-anchor ruling: tight absolute line windows are churn; the load-bearing alarms are ORDER/EXISTENCE/byte-string. | NEEDS-FOLD churn | **P-A5-static / P-A10 — the suite drops the tight numeric windows** and keeps the ORDER/EXISTENCE/byte-string anchors; the contract keeps its code-verified anchors but they are re-verified, not inherited. |
| §4.1 — the P-D1.4 counter scan runs on the matched loop line only; a counter inside processMember/answerDecision escapes. | NEEDS-FOLD cheap | **§D1.4 — the counter scan covers the WHOLE driveLane body** (the drift-immune awk range `async function driveLane` → next top-level function), so a per-key retry cap cannot hide. |
| §4.3 — `git show <sha>:<dir>` does NOT fail (it returns the tree listing); the directory case only refuses via a `mustContain` mismatch, so a directory path WITHOUT mustContain harvests `ok:true`. | NEEDS-FOLD | **§D4 — the mechanism is corrected** (`git show` on a tree returns the listing) and the file-not-directory law is enforced STRUCTURALLY — an admission/refusal-time check that each harvest path is a regular file, refusing `harvest_miss` for directories regardless of `mustContain`. New RED row A8-dir pins the gap. |
| §4.2 — P-A4 two-level byte-identical refusal | SOUND | no change. |
| §1.4 / P-A7 / P-A9 / P-A3g | SOUND | no change. |
...
| # | Claim | Anchor |
|---|---|---|
| G1 | **The wave surface is landed as facade direct ports, dispatched before the recursive-session gate.** `waves.start`/`waves.progress`/`waves.send`/`waves.stop` dispatch at `application.mjs:12506-12509`; `waves.list` (the #132 observe verb) at `:12512`; `waves.run` (the #114 interpreter) at `:12516`. The eight #87 workflow-surface direct ports (`run.message.send/receipt`, `run.attention.watch`, `run.scratchpad.read/elevate`, `run.board.post/read`, `run.knowledge.seed`) dispatch at `:12471-12478`, explicitly BEFORE `normalizeCommandContext`/`validateApplicationCommandArgs`/the recursive gate (`:12464-12470`, FP-18 pins the pre-gate dispatch). Each validates through its own closed normalizer, then delegates to its landed kernel lane. | `application.mjs` |
| G2 | **`wave.started` mints pre-loop, exactly-once, idempotency-keyed.** Mint site `application.mjs:4642-4661`: payload `{waveId, deploymentId, roster, idempotencyKey}` under `wave.started:${waveId}`; the roster is the member-object shape `[{role, route, scope}]` built at `:11617-11621` (`route` is the normalized `exact`, `:11619`), and each member rides the SAME exact-route profile admission ordinary `run.start` uses (`:11628-11636`). | `application.mjs` |
...
| G4 | **DECISION_REQUEST is landed wire grammar.** Grammar `claude-session.mjs:27`; the single-request pin `:1132-1141` admits at most ONE live emulated decision request per session — a second (possibly contradictory) line is ignored as prose while one is pending; the worker can always re-ask once it settles. Admission `coordinator.mjs:12998`: malformed payloads never mint a pending record; an oversize question draws a typed, coaching refusal. Reconstruction `:14163`. The human answers via `run.answer` (`application.mjs:180`, capabilities `['approve','observe']`) or `baton_decision_answer` (`mcp-northbound.mjs:92`). The #94 demo proved the round-trip: the LEAD printed the single mandated DECISION_REQUEST; the driver resolved it via `run.answer` with `synthesize` (`control-surface-audit.md:119-126`). | `claude-session.mjs`, `coordinator.mjs`, `application.mjs` |
| G5 | **The workflow-as-data interpreter is landed.** `waves.run` direct port (`application.mjs:12516`). Closed spec `SPEC_FIELDS = ['schemaVersion','idempotencyKey','members','steering','harvest']` (`workflow-interpreter.mjs:48`); member `['role','exact','scope','objectiveRef','report']` (`:49`); steering `['approveOnAdvertisedPlan','nudgeOnCheckpoint','claimOnStall','messageOnSpawn','elevateWhenNotes','answerDecisions','signalOnMembersDone']` (`:51-54`). `answerDecisions` is a policy map `question → optionId | text | "defer"` (`:260-270`, driven `:693-699`, `defer` outcome `:789-793`, `matchDecision` first-match-wins `:454-466`). D6 receipt EXACTLY seven keys, sorted: `{basis, harvest, manifestDigest, outcomes, steering, verdict, waveId}` (`:594-602`); `outcomes` is `[{role, phase, terminal, resultSha, report?}]`; `verdict` is `WAVE-OK`/`WAVE-INCOMPLETE` (`:589-590`). `verification` is REMOVED from the spec (`:137`); `report` is a declared member field, never executed, carrying the result-pin path (`:209-214`). | `workflow-interpreter.mjs` |
...
| G7 | **The wave registry projection is landed.** `_waveRegistry` Map (`coordination-store.mjs:1231`); `wave.started` fold `:8099-8120` (a roster neither a well-formed object-array nor a well-formed string-array throws `wave_registry_invalid`); `wave.closed` fold `:8793-8803` (state flips to `'closed'`, `waves.list` reads only open rows). The read side: `waves.list` at `application.mjs:11715-11751` (typed `wave_not_found` for a missing member run). Row shape `{deploymentId, idempotencyKey, roster, waveId}` (wave-observability-2026-08-06 D2.2). | `coordination-store.mjs`, `application.mjs` |
...
(`run.scratchpad.read`, contracted #87, dispatched at `application.mjs:12474`). Rows are claimed
and reported through the board worker-half (`requestBoardClaim`/`submitBoardReport`,
`coordinator.mjs:11453-11475`) where the surface exists; today that half is kernel-only (G11), so
...
**D1.2 — The scratchpad read-authorization law (v1.2 — BLOCKER 2 fold + the blue-team §1.1/§2.2
fold).** Who may read a `worker:<role>` partition is contract law, stated so the D1/A2 artifact
handoff is decidable:
...
**Enforcement seam (v1.2 — the blue-team §1.1 fold).** `run.scratchpad.read` passes `{scope}`
straight to `_authorize('run.scratchpad.read', principal, runId, {scope})` (`application.mjs:13040`)
and then snapshots whatever scope was requested (`scratchpadSnapshot(request.runId, request.scope)`,
`:13041`); `_authorize` throws `application_unauthorized` when the deployment authorize does not
return `true` (`:3215`). The shipped default deployment authorize is permissive —
`authorize: async () => true` (`application-deployment.mjs:2012`) — so under the default there is
**no scope restriction at all**: a swarm row could read `worker:row-2` as easily as
`worker:coordinator`, and could read the coordinator's partition across runs. v1.2 therefore
...
an explicit wave-scoped grant, point 3); everything else refuses `application_unauthorized`. This
is the "unknown ≡ foreign at the policy seam" default (#87, `facade-projection-contract.md:636`)
implemented for scopes foreign to the caller's run — the same policy law the #87 contract already
pins for the lane. The acceptance suite pins this closure TWO ways, because the hermetic fixture
bypasses the deployment seam: (a) the A1/A2 fixtures install the restricting authorize at the
seam's stand-in, so the sibling-refusal and grant legs are REACHABLE hermetically (green at HEAD);
and (b) a STATIC pin requires the permissive literal `authorize: async () => true` to be ABSENT
from the deployment seam — the A1/A2 red rows move to that pin. The wave-scoped grant path
(point 3) is asserted REACHABLE: a grant-aware restrictor mints an explicit grant and a granted
swarm-row read succeeds, so a refuse-everything restrictor fails (§2.2 fold).

**D1.3 — The truthful steering trail (v1.2 — BLOCKER 1 fold + the blue-team §1.2a/§1.2c/§2.1
fold, contract requirement).** The `answerDecisions` policy answering is part of the escalation
audit the top orchestrator reads, so the steering trail must not be falsifiable. When the policy
answering throws — a denied option (the answering principal cannot exercise the mapped answer) or a
raced terminal member — the interpreter MUST:

1. record the truth in the steering trail: `{trigger: 'answerDecisions', role, requestId,
   outcome: 'denied', refusal: <code>, optionId?}` — and `text?` for the free-text path — where
   `<code>` is the thrown refusal's typed code (the facade's code from the `handle.answer` throw —
   e.g. `application_unauthorized` for a capability-denied answer at `application.mjs:3215`, or the
   terminal-member refusal for a raced member). The audit needs to know WHICH option (or text) was
   attempted, so `optionId?`/`text?` are part of the denied record's shape (§1.2a fold);
2. NOT mark the decision key handled — neither `s.answeredKeys` nor `s.handledDecisionKeys` (the
   permanence half, §2.1 fold: the suite pins this STRUCTURALLY — the pre-answer
   `s.answeredKeys.add(key)` is absent from the drive loop, so a denied key stays NOT-handled);
3. leave the ask pending for the human — the member's task stays parked at `input_required` (the
   DECISION_REQUEST admission is a blocking decision gate, `coordinator.mjs:12998` → `:13047-13056`
   → `input_required`), and the later human answer via `run.answer` (`application.mjs:180`) settles
   it;
4. **record a denied decision ONCE per requestId and never re-auto-answer it** (the re-attempt
   policy, §1.2c fold): with the pre-answer add gone, the drive loop would re-attempt a denied ask
   on EVERY poll (a `denied` record per poll, ~200 at the driver's hardCap). The interpreter skips
   `answerDecisions` for a requestId it has already denied — exactly one `denied` record, no later
   `answered` for the same requestId, the ask stays open for the human.
...
this to change (the pre-answer add moves after a successful answer or is removed, and the denied
record carries `optionId?`/`text?`). The D6 steering trail is then the honest escalation audit:
every entry is either `deferred` (no policy match), `refused` (a policy-named option the ask does
not actually offer, `:801-805`), `denied` (a throw — the ask stays open, recorded once, never
re-auto-answered), or a genuinely delivered `answered`.

**D1.4 — The escalation sequence bound (v1.2 — §3.3 note fold + the blue-team §4.1 fold).**
Escalation spam is bounded in concurrency and audited end-to-end, but the sequence of re-asks after
a human answers is not volume-capped:

- **One live ask per session** (`claude-session.mjs:1132-1141`) and **one pending decision per
  worker at admission** (R-BD-4, `coordinator.mjs:13047-13056`) — concurrent spam is structurally
  impossible.
...
`blocking !== false` → task `input_required`, the admission seam `coordinator.mjs:12899-12938`), a
...
- **The full shape (#12 composition, Ring-4 impl-queued) — the `waves.*` pre-gate finding
  (v1.2 — OQ1 answered, A5 amendment, re-verified at the v1.2 fold HEAD).** The code answers sharper
  than "unverified":
  `startWave` has no `_authorize` of its own (`application.mjs:11604`), and the `waves.*` direct
  ports are dispatched BEFORE the recursive-session gate (`:12506-12516`; the gate's
  `context?.sessionAuthority` check and `run_orchestrator_command_forbidden` throw at
  `:12531`/`:12535`, allowlists `:12528-12530`). A lease-bound coordinator reaching `waves.start`
  therefore does NOT draw `run_orchestrator_command_forbidden` — the gate never sees it; the only
  gate is the per-member `this.start(...)` run.start admission (`:11628-11636`), and `run.start` is
  itself a `recursiveEffectCommand` (`:12530`), so it is *allowed* for a lease holder. **The
  ...
  `run_orchestrator_command_forbidden` (gate, `application.mjs:12531`; kernel side
...
(`application.mjs:11617-11621`, route `:11619`), so the top orchestrator sees exactly which member is
...
shape (v1.2 — the harvest path names a FILE, per §5.1 of the red-team; the §4.3 mechanism is
corrected below):
...
- **The harvest path is a FILE, never a directory (v1.2 — the blue-team §4.3 mechanism
  correction).** `harvestOne` reads each path via `gitShow(repoRoot, outcome.resultSha, path)`
  (`workflow-interpreter.mjs:615`). The v1.1 mechanism text was wrong: `git show <sha>:<dir>` does
  NOT fail — it returns the tree listing and exits 0. So a directory path refuses today ONLY when
  its `mustContain` mismatches the recovered listing; a directory path WITHOUT `mustContain`
  recovers the listing → `ok:true` → the wave can go `WAVE-OK`. The file-not-directory law is
  therefore enforced STRUCTURALLY: an admission/refusal-time check that each harvest path is a
  regular file, refusing `harvest_miss` for directories regardless of `mustContain`. A directory
  path without `mustContain` must refuse — the new A8-dir red acceptance row pins the gap. The
  example above names `docs/results/coordinator.md` — an implementer copying the spec shape gets a
  working harvest.
...
steering trail records `{trigger: 'answerDecisions', role, requestId, optionId?, outcome: 'denied',
refusal: <code>}` (with `text?` for the free-text path — the audit needs the attempted option/text)
where `<code>` is the thrown refusal's own typed code (e.g. `application_unauthorized` for a denied
option, or the terminal-member refusal for a raced member) — the code is reused, the record shape
is the new truth, and a denied ask is recorded ONCE per requestId, never re-auto-answered (the
D1.3 re-attempt policy). No sorted-key literal is introduced.
...
| A5 | **Authority boundary, full shape + new refusal (D2).** Red: no `coordinator_authority_forbidden` exists; a would-be coordinator authority action is indistinguishable from any denial. Green: a coordinator-seat principal reaching for a wave/steering authority verb draws `coordinator_authority_forbidden` with `{attempted, gracefulPath}`; the top orchestrator's own authority actions never fire the new code — pinned as seat-CLASS, not identity: at least TWO top-orchestrator principals can start a wave (blue-team §1.3). The #12 codes are NOT claimed for the `waves.*` verbs — those dispatch before the recursive gate (the ORDER is pinned: the `waves.*` direct ports precede the gate's `context?.sessionAuthority` check and `run_orchestrator_command_forbidden` throw), so a lease-bound `waves.start` is not refused by the gate; the full shape requires `waves.start`/`waves.run`/`waves.stop` added to the recursive gate (or explicitly refused for lease holders) at the dispatch seam (§D2, OQ1 answered). | **RED** (no coordinator seat; the `waves.*` pre-gate dispatch must be closed for the full shape) |
| A6 | **Seat discipline (D3).** Red: no route-map law — any member may be any seat. Green: the recipe route map `exact: {harness, model, effort}` names the heavyweight coordinator and cheap swarm rows; each member rides the same exact-route profile admission ordinary `run.start` uses; the wave roster (`application.mjs:11617-11621`) and `waves.list` expose the coordinator's route to the top orchestrator. | **RED** (route map exists in recipes/interpreter, but no coordinator seat pins it) |
...
| A8 | **#114 composition (D4).** Red: no spec shape declares the pattern. Green: the workflow-as-data spec above (`schemaVersion`, `idempotencyKey`, members with `objectiveRef`, steering `answerDecisions`/`messageOnSpawn`/`signalOnMembersDone`, harvest with a FILE path) runs through `waves.run` (`application.mjs:12516`); the D6 receipt is the closed seven-key shape; `verification` is absent; a spec declaring the coordinator + swarm settles every row to `WAVE-OK` or honestly `WAVE-INCOMPLETE`. **The message kinds `brief`/`result` must be accepted END-TO-END at the coordinator boundary** (`coordinator.mjs:6864`), not just by the interpreter's admission set — the delivery is pinned (a `messageOnSpawn` steering entry with a DELIVERED `messageId`; the `signalOnMembersDone` recipients), blue-team §1.5. | **RED** (the interpreter runs specs today, but no spec declares the coordinator pattern, and the example kinds are refused) |
...
- **OQ1 — waves.* lease-holder authorization (ANSWERED — code finding carried into the body,
  ...
  (`application.mjs:11604`), and the `waves.*` direct ports are dispatched BEFORE the
  recursive-session gate (`:12506-12516`; the gate's `context?.sessionAuthority` check and
  `run_orchestrator_command_forbidden` throw at `:12531`/`:12535`). A lease-bound coordinator reaching
  `waves.start` therefore does not draw `run_orchestrator_command_forbidden`; the only gate is the
  per-member `this.start(...)` run.start admission (`:11628-11636`), and `run.start` is itself a
  `recursiveEffectCommand` (`:12530`). The full shape requires `waves.start`/`waves.run`/
...
- **#68 (BD3-A read port)** — the `run.scratchpad.read` lane the swarm reads the coordinator's
  ...
  `application.mjs:12474`). Cross-referenced in D1/A2.
