...
|---|---|---|
| `application.mjs:7979-8022` (run.wait) | `7979-8022` present; loop + `coordinator.wait` + `status()` re-read confirmed; `application_wait_invalid` check/throw at `7989-7991` | exact |
...
- **H-1 (blocking) — "the moment any authority check fails" (D1.2) and "never the full clock"
  ...
  - CLI: `run view --until`/`run status --wait` delegate to `run.wait` with **no surface layer
    ...
    wait ceiling is `timeoutMs > 24 * 60 * 60 * 1000` (`application.mjs:7989-7991`), so a CLI
...
- **H-6 (non-blocking) — `application_wait_invalid` is missing from the refusal vocabulary.** The
  application-layer wait-request refusal (`application.mjs:7989-7991`, timeout past the 24 h
...
1. **D3.2/D1.2 over-claim the per-cycle transport-principal recheck; the "never the full clock"
   ...
   application ceiling (`application.mjs:7989-7991`) — the exact #148 instance class
