...
const TASK = [
  'Implement issue #46 per docs/37-wave-driver.md v2 — READ IT IN FULL FIRST (it is the',
  'controlling contract, including laws L1-L7 with the L6 termination law). Red-first TDD:',
  '(1) WRITE impl/test/wave-driver-policy-red.test.mjs FIRST: rows D1-D10 exactly as §3',
  'specifies (requestId dedup; cursor-stripped liveness + sibling-cursor immunity; true stall;',
  'hard cap with stall-before-cap precedence; salt semantics; the L6 termination law;',
  'envelope shape; nudge-failure tolerance; claim fan-out; unavailable semantics), every test',
  'failing for the right reason before implementation. Harness per §3: the checkpoint',
  'conjunction (pausable card override AND steering.registered) and a neutralized worker',
  'watchdog.',
