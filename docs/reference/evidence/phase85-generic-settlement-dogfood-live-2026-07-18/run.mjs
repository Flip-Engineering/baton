...
try {
  lifecycleResult = await lifecycle.run(async ({ signal }) => {
    ...
    repo,
    advanced: {
      deploymentRoot, routes: [codex, glm, kimiCode],
      verification: {
      ...
      if (recoveryOnly) {
        ...
        record = {
          ...
          recovered: true, phase: stopped.outline.phase,
  ...
  const mapRoute = !forceCodex && kimiReady ? kimiCode : codex;
  const team = [
  ...
  progress = {
    ...
    mapEvidence: completedMap.item.state === 'failed' ? await mapped.evidence() : null,
      ...
      if (completedMap.item.state === 'failed') {
        ...
        progress.retry = { callId: retry.id, outline: completedMap.item };
      }
