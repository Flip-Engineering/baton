# #158 CONTRACT-FOLD BRIEF — fold the red-team into the contract

You are folding an adversarial red-team report into the #158 contract. Read fully, in order:
(1) `contract-redteam.md` (NOT FOLD-READY — numbered blockers at the end, each with its
concrete fix); (2) the contract (v1, your edit source — see the brief's read-order); (3) any
cross-referenced landed laws the red-team cites.
...
`grep -an`/`sed -n` on `application.mjs` + `coordination-store.mjs`); sorted-key
