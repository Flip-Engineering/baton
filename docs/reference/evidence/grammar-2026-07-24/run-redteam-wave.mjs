...
const evidencePath = resolve(evidenceDir, 'evidence-redteam.json');
...
const MEMBERS = Object.freeze([
  Object.freeze({
    role: 'grammar-redteam-codex',
    exact: Object.freeze({ harness: 'codex', model: 'gpt-5.6-sol', effort: 'high' }),
    scope: Object.freeze([`${relativeRoot}/redteam-codex.md`]),
    report: `${relativeRoot}/redteam-codex.md`,
    objective: [
      `Write ${relativeRoot}/redteam-codex.md — your independent adversarial red-team report`,
      'on the unified control grammar. Prefix your finding ids R-CX-N.',
...
const log = (line) => console.log(`[grt ${new Date().toISOString()}] ${line}`);
let failure = null;
let wave = null;
