...
const MEMBERS = Object.freeze([
  Object.freeze({
    ...
    report: null,
    objective: [
      'Implement grammar phase M1 per docs/35-unified-control-grammar.md §9 M1 (READ the doc',
      'FIRST — §4, §5 L1/L2, §6, §6.1, §8.1, §8.2 — it is v2 FINAL and your binding contract),',
      'building on the landed M0 conformance harness (impl/scripts/surface-conformance.mjs,',
      'surface-divergence-ledger.json — study both before writing anything). Scope, red tests',
      'first (impl/test/grammar-m1-red.test.mjs):',
      '(1) CANONICAL ALIASES EVERYWHERE (L1 for the §6 table): every canonical operation name',
      'resolves on every surface it applies to — the Web bus admits the canonical command',
      'name beside the legacy one (web-northbound.mjs entry derivation), the CLI accepts the',
      'canonical verb spelling beside the legacy row (application-cli.mjs parse tables from',
      'the registry), and the registry records the alias relation (application-semantics.mjs:',
      'each operation gains aliases: [...legacy names] and deprecated marks on legacy',
      'spellings; keep the registry digest deterministic). Legacy names keep working',
      'byte-identically — NO removal, NO behavior change on legacy paths; this is additive.',
      '(2) L2 DO-BLOCKS: every advertised action in a RunView (nextActions[], attention[])',
      'carries do: {action, inputs} executable verbatim through run.act/run.do — the exact',
      'bound inputs (planDigest for approve_plan, requestId+response shape for answers), so',
      'the application_action_scope_mismatch wrong-guess class dies. Wire the same block',
      'through the Web/MCP action-authority digest path (mcp-web-bridge.mjs actionAuthority —',
      'the do block must survive the authority digest checks; extend the digest payload',
      'compatibly, never weaken it).',
      '(3) LEDGER SHRINK: move every ledger entry retiresIn:M1 to resolution — a canonical',
      'name that now resolves is no longer a divergence; update the ledger and keep SC1',
      '(novel empty) and the whole conformance suite green.',
      '(4) Red tests: M1-1 canonical Web command admits and returns the identical outcome as',
      'its legacy spelling (same authority, same view modulo cursors); M1-2 canonical CLI verb',
      'parses to the same envelope as legacy; M1-3 every advertised action carries an',
      'executable do block and executing it verbatim succeeds (approve_plan round-trip from',
      'outline WITHOUT reading any deeper depth); M1-4 legacy spellings are marked deprecated',
      'in registry/help/tool annotations but execute unchanged; M1-5 the registry digest is',
      'stable across two constructions (determinism).',
      'Then the focused files green, then the FULL SUITE green from the worktree root (node',
      'scripts/run-suite.mjs inside impl/). House style: match neighboring code; comments only',
      'for constraints code cannot show. No git commits, no scratch/log writes (including',
      '/tmp). Do not call gh. Do not invoke nested Baton.',
      OVERSIZE,
    ].join(' '),
  }),
]);
