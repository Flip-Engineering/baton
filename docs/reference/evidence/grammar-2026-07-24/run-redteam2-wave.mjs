...
const evidencePath = resolve(evidenceDir, 'evidence-redteam2.json');
...
const MEMBERS = Object.freeze([
  Object.freeze({
    ...
    objective: [
      ...
      'on the unified control grammar. Prefix your finding ids R-KM-N.',
...
const baton = await openBaton({
  repo,
  advanced: {
    deploymentRoot: resolve(repo, '.baton', 'grammar-2026-07-24-b'),
    routes: [{ harness: 'kimi-code', model: 'kimi-code/k3', effort: 'high' }],
    verification: VERIFY,
  },
});

const log = (line) => console.log(`[grt2 ${new Date().toISOString()}] ${line}`);
let failure = null;
let wave = null;
