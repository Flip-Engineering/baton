...
Date: 2026-08-06 · Contract: **reply-chains v1.1** (folded) · Suite: 26 rows (6 PINs · 20 red)
...
`suite-fold-2-brief.md` (the fold-2 dispatch), `suite-fold-2.md` (the finding → resolution map).
...
Recorded after the fold-2 edits landed (E2 rewritten for B1/T2, C1 tightened for T1, C2 pinned for
T3, H7/G1 widened for T4, F3 function-body-scoped for T5, B2 target-verbatim for T6, H4 in-range
for N1). Two consecutive runs of the finished suite both produced **pass 6 · fail 20** — the split
is deterministic. The 6 passes are exactly the six PIN rows (A1, C2, G1, H2, H6, H7); the 20
failures are the red rows, each confirmed to fail at its NAMED stage (the per-row stage is in the
header and in each row's assertion message).
...
| B2 | Walk | | **target-inheritance-missing** | reply records mint `target: {workerId: null}` (coordinator.mjs:12580) → `messageRunId(r1)` → null → the facade refuses `application_unauthorized`; the walk dies at the first hop (the T6 target-verbatim deep-equal fails on the same mint) |
| C1 | Member | | **membership-check-missing** | a foreign worker's reply is ADMITTED (fills the slot); no `message_target_not_member` exists — the T1 sibling control (memberC, same run) sits behind the foreign refusal at (a) |
| C2 | Member | PIN | parent-exists before membership | green today — a reply to an unknown `message:` id draws `message_parent_not_found` (coordinator.mjs:12530) for BOTH a run-member and a foreign worker, never `message_target_not_member` |
...
| E2 | Replay | | **replay-topology-not-rebuilt** | a SECOND coordinator over a FRESH `CoordinationStore` on the same logDir rebuilds nothing — `_replay()` never seeds `_messages` (coordinator.mjs:13274), `messageReceipt(root.messageId)` is null; the B1 alias discriminators (`alias: true`, key `message.sent:<workerId>:<tail>`, no `inReplyTo`, absent depth fields) pass at HEAD |
...
| F3 | Observability | | **allowlist-missing** | the `stateFailureCode` FUNCTION BODY (mcp-northbound.mjs:198-261) knows no `message_budget_invalid` — T5 scopes the check to the body via `/function stateFailureCode\(cause\) \{/` |
| G1 | Escalation | PIN | interaction-lane | green today — blocking `question.asked` transitions the task to `input_required` (coordinator.mjs:12614-12631); a reply never transitions a phase, even a reply frame carrying a `blocking: true` marker (T4) |
...
| H4 | MCP/web | | **mcp-message-budget-missing** | a `budget` argument dies at the generic key-closure as `unknown_argument_field` (mcp-northbound.mjs:898); the `_dispatch` branch (mcp-northbound.mjs:1771-1778) strips budget, so the N1 in-range acceptance probe (budget: 3 → `isError: false`) fails too |
...
| H7 | MCP/web | PIN | wire asymmetry | green today — the scanner rejects any reply frame with an extra `budget`, `blocking`, or `priority` field (closed sorted-key literal `'body,inReplyTo'`; T4 widens the probes) |

## Invented surfaces

No invented module is imported. Every invented member is probed through a REAL surface entry point
and is absent from the surface at HEAD (the seam the red row holds) — or, for a PIN, is present
today and kills a wrong implementation:
...
| `message_parent_not_found` behavioral (the parent-exists leg of the B-2 order) | worker `message.rejected` on an unknown `message:` id | green today — pre-existing code (coordinator.mjs:12530); C2 pins the ordering both ways |
| fresh-store `_replay()` topology rebuild (root→r1 with depth/budget/remaining, `parent.reply` re-link) | `replayCoordinator(fx)` — a second `Coordinator` over a FRESH `CoordinationStore` on the same logDir | `_messages` stays empty (coordinator.mjs:13274); the alias is never minted (E2) |
| reply-frame `blocking` / `priority` rejection | `scanForMessageSend` | rejected by the closed sorted-key literal `'body,inReplyTo'` (H7); a blocking-marker reply stays prose in the lane (G1) |
| in-range budget accepted through the MCP surface | `baton_run_message_send` with `budget: 3` | dies at the key-closure as `unknown_argument_field` — the `_dispatch` branch strips budget (H4/N1) |
...
`replayCoordinator(fx)` releases the live store's writer lease, builds a FRESH `CoordinationStore`
over the same logDir, and constructs a second `Coordinator` — the real replay entry point is the
constructor's `_replay()` (E2).
...
| **C2** parent-exists before membership | an impl that checks run-membership before the parent-exists check — a foreign worker replying to an unknown `message:` id would see `message_target_not_member` instead of `message_parent_not_found` |
| **G1** blocking → interaction lane | a machine-readable blocking marker riding the reply frame (violates RC-11 wire asymmetry) or a reply transitioning a task phase — including a `blocking: true` on the reply payload (T4) |
...
| **H7** wire asymmetry | the scanner accepting `budget`/`blocking`/`priority`/any extra field in the reply frame (a worker setting a budget or an escalation marker) |
...
- **target-inheritance-missing** → B-1: the reply record carries `target: parent.target` VERBATIM
  (T6 — the record-level deep-equal pins the copy, not a resolved `{runId}` denormalization); every
  hop resolves through `messageRunId` to the root's run under resolve-then-authorize.
- **membership-check-missing** → B-2: a reply whose `from` is not a member of `parent.target`'s run
  ...
  budget hop never spent by a non-member. The ADMISSION clause (T1) is a worker of the parent's run
  who is NOT the target — the C1 positive control is the sibling `memberC`.
- **parent-exists before membership (C2 pin)** → B-2 admission order: the parent-exists check
  (`message_parent_not_found`, pre-existing at coordinator.mjs:12530) fires before the run-membership
  check for EVERY worker — a foreign reply to an unknown parent never draws `message_target_not_member`.
...
- **reply-row-absent / replay-topology-not-rebuilt** → B-4/D5: reply hops are store-audited
  ...
  (`alias: true`, key `message.sent:<workerId>:<tail>`, NO `inReplyTo`, no depth fields) are
  replay-skipped and never mint a phantom root; `_replay()` (the Coordinator constructor's rebuild,
  E2's entry point) reconstructs the chain topology — root→r1 walking through `messageReceipt` with
  `parent.reply` re-linked — from the durable rows on a FRESH store.
...
- **allowlist-missing / web-mapper-branch-missing** → D3/D7: `message_budget_invalid` is the ONE new
  code INSIDE the `stateFailureCode` function body (mcp-northbound.mjs:198-261; T5 — a literal
  elsewhere in the file is not an allowlist) and the web `dispatchFailure` gains a 400 branch; the
  worker-stream codes (`message_depth_exceeded`, `message_target_not_member`,
  `message_parent_not_found`) stay absent from the body and the web mapper.
- **mcp-message-budget-missing** → D7: `baton_run_message_send` schema gains optional `budget
  {integer, minimum: 1, maximum: 8}`; the `_dispatch` branch (mcp-northbound.mjs:1771-1778) forwards
  budget in the command object (N1) so an out-of-range value surfaces `message_budget_invalid` and an
  in-range value is accepted — never `unknown_argument_field` / `command_outcome_unknown`.

## Suite-law hygiene (verified)

- **Hermetic**: ScriptableAdapter (no harness, no network), `mkdtempSync` repos/logs, global
  `test.after` cleanup; `replayCoordinator` reuses the lane's own `tmpDir` (a second store over the
  same logDir, no new fixture); the MCP fixture uses `McpFleetServer.handle` in-process (no live
  server); the verification command is the brief's `true` stub.
- **NUL discipline**: the two NUL files are never read whole — `application.mjs` only through the
  ...
  (E1/E2, including the fresh-store replay). All other sources are NUL-free and read whole for the
  source pins (mcp-northbound.mjs F3, web-northbound.mjs H5, wave-driver.mjs H6). The suite file
  itself is NUL-free (0 NUL bytes).
