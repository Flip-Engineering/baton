# ROW BRIEF — row-lc-ledger: the ledger contract
...
- #194 (dsh-adoption ①): model-visible-means-logged as a dispatch-seam invariant + the durable no-step turn (a rejected/empty attempt is RECORDED) — any member's exact served context must be reconstructable from the coordination store (via the cited spill artifact, never bodies-inline — limits.mjs keeps bodies out)
- #205: the decision lane round-trips but NEVER ledgers (zero decision.* events ever — the lane-proof wave's answered DECISION_REQUEST left no record)
