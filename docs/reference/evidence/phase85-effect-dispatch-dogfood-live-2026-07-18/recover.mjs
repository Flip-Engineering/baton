...
let stopped = null; let closed = null; let candidates = []; let runStatus = null;
try {
  const workflow = baton.open(runId);
  const before = await workflow.status();
  runStatus = {
    phase: before.phase, attempts: before.attempts, nodes: before.nodes,
    verification: before.verification, attention: before.attention,
    progress: before.progress,
  };
  const section = await workflow.candidates();
  candidates = section.section.items.map(({ value }) => {
    const body = git([
  ...
  await workflow.stop('Recover the interrupted Phase 85 effect-dispatch Run and reap exact ownership.');
  const after = await workflow.status();
  stopped = {
    beforePhase: before.phase, afterPhase: after.phase,
    stop: after.stop, ownership: after.ownership,
  };
} finally {
  closed = (await baton.close()).ownership;
}
const evidence = {
  schemaVersion: 1, deploymentRoot, runId,
  outcome: 'stalled_then_recovered', runStatus, candidates, stopped, closed,
};
writeFileSync(join(evidenceDir, 'evidence-recovery-latest.json'),
...
if (![1, 3].includes(stopped?.stop?.receipt?.schemaVersion)
  ...
  || stopped.ownership?.workers !== 0 || !closed?.closed || closed.workers !== 0) {
  throw new Error('recovered Phase 85 effect-dispatch deployment did not prove zero ownership');
}
