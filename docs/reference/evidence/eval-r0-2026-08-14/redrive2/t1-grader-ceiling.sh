#!/usr/bin/env bash
# EVAL-R0 redrive2 t1 evidence — grader-ceiling check: each rung's grading suite must be
# GREEN at its pinned IMPL commit (pre-registration §2's "grading suite" column). A suite
# that cannot pass on the landed implementation is not a valid grader for the arms.
# Offline/deterministic. Method mirrors t0 (real detached worktrees — NOT `git archive`,
# which is pathspec-filtered to empty trees in baton worktrees; see report §4.2).
...
run_ceiling () { # $1=rung  $2=impl  $3=suite
  ...
  local tmp="/tmp/eval-r0-rd2-t1-${rung}"
  rm -rf "$tmp"
