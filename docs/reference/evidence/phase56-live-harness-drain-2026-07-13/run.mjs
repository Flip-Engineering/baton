...
const TARGET_REPO = join(OWNER_ROOT, 'phase56-clean-target');
const LOG_DIR = mkdtempSync(join(tmpdir(), 'baton-phase56-live-log-'));
const RUN_ID = 'phase56-live-harness-drain';
const TASK_TYPE = 'phase56-drain-adversarial-review';
const TASK_CATALOG = [
  { taskId: 'phase56-codex-review', harness: 'codex', model: 'gpt-5.6-sol', family: 'openai', target: 'reviews/dogfood/phase56-codex-review.md', focus: 'drain fencing, exact async ownership, deadline truth, and driver close ordering' },
  { taskId: 'phase56-claude-review', harness: 'claude', model: 'claude-opus-4-6', family: 'claude', target: 'reviews/dogfood/phase56-claude-review.md', focus: 'durable replay, actor binding, receipt validation, and crash recovery' },
  { taskId: 'phase56-glm-review', harness: 'glm', model: 'glm-4.7', family: 'glm', target: 'reviews/dogfood/phase56-glm-review.md', focus: 'closed capacity, startup readiness, historical cleanup, and path privacy' },
  { taskId: 'phase56-grok45-review', harness: 'grok', model: 'grok-4.5', family: 'grok', target: 'reviews/dogfood/phase56-grok45-review.md', focus: 'kill/process-close correlation, process-group convergence, and late process events' },
  { taskId: 'phase56-grokbuild-review', harness: 'grok', model: 'grok-build', family: 'grok', target: 'reviews/dogfood/phase56-grokbuild-review.md', focus: 'reflexive Baton-on-Baton friction, evidence ownership, drain retries, and recursive cleanup' },
];
...
function brief(task) {
  return createBrief({
    goal: `Independently review committed Phase 56 at ${IMPLEMENTATION_SHA.slice(0, 7)}, focusing on ${task.focus}. Inspect spec/phase56/public-drain-and-close.md and its implementation/tests. Write ${task.target} with exactly the headings "## Verdict", "## P0-P1 findings", and "## Required corrections".`,
    constraints: [
    ...
    verification: {
      command: `test -s ${task.target} && grep -Fq '## Verdict' ${task.target} && grep -Fq '## P0-P1 findings' ${task.target} && grep -Fq '## Required corrections' ${task.target} && node impl/scripts/run-evidence.mjs impl/scripts/run-suite.mjs impl/test/phase56-drain-and-close.test.mjs`,
      expectExit: 0, timeoutMs: 180_000,
    },
    budget: { tokens: Number(process.env.BATON_TASK_TOKEN_BUDGET ?? 60_000), usd: 3, wallMin: 14 },
  });
}

function bounded(events) {
  const keep = new Set(['worktree.ready', 'lifecycle.spawned', 'lifecycle.turn_started', 'lifecycle.turn_completed', 'lifecycle.crashed', 'resource.tokens', 'model.mismatch', 'verify.reverified', 'kill.requested', 'kill.confirmed', 'lifecycle.process_started', 'lifecycle.process_closed']);
...
try {
  sourceGit(['worktree', 'add', '--detach', TARGET_REPO, IMPLEMENTATION_SHA]); targetCreated = true;
  ...
  driver = createDriver({
    repoRoot: TARGET_REPO, logDir: LOG_DIR, repoId: 'baton-phase56-live',
    adapters: {
  ...
  pump = (async () => {
    ...
    while (pumping) {
      for (const worker of coordinator.list()) {
        ...
        responses.push({ workerId: worker.id, requestId, ack: await coordinator.respond(requestId, worker.pendingApprovalId ? { decision: 'allow' } : { text: 'Finish only the scoped report.' }, 'orchestrator:phase56-live') });
      }
  ...
  if (driver) {
    try { closureReceipt = await driver.drainAndClose('orchestrator:phase56-live'); }
...
try {
  ...
  if (driver && targetGit) {
    ...
    for (const row of rows) {
      ...
      row.processStarted = row.events.findLast((event) => event.kind === 'lifecycle.process_started') ?? null;
  ...
  const startedRows = rows.filter((row) => row.processStarted);
  const correlatedClose = (row) => Boolean(row.processClosed && row.processClosed.payload?.pid === row.processStarted.payload?.pid && row.processClosed.payload?.processGroupId === row.processStarted.payload?.processGroupId && row.processClosed.payload?.generation === row.processStarted.payload?.generation);
  const providerProof = {
    startedProcessCount: startedRows.length,
    allSelectedProcessesStarted: startedRows.length === TASKS.length,
    simultaneousActiveGrokPidSampleObserved: simultaneousGrokSamples.length > 0,
    everyStartedProcessClosedExactly: startedRows.every(correlatedClose),
    everyStartedLeaderGone: startedRows.every((row) => !alive(row.pid)),
    everyStartedGroupGone: startedRows.every((row) => !groupAlive(row.processGroupId)),
  };
  const cleanup = {
  ...
  const lifecyclePass = fatal === null && routeAdmission.allAdmitted && routeAdmission.exactRequestedResolved && routeAdmission.noObservedMismatch && providerProof.allSelectedProcessesStarted && (!REQUIRE_GROK_PAIR || providerProof.simultaneousActiveGrokPidSampleObserved) && providerProof.everyStartedProcessClosedExactly && providerProof.everyStartedLeaderGone && providerProof.everyStartedGroupGone && Object.values(cleanup).every(Boolean);
  const matrixPass = lifecyclePass && reviewProof.allSelectedVerified && projectionProof.allWorkersBound && projectionProof.allVerifiersBound && projectionProof.sourcePathAbsentFromBoundedEvents;
  summary = {
    at: new Date().toISOString(), implementationSha: IMPLEMENTATION_SHA, runId: RUN_ID, selectedTaskIds: TASKS.map((task) => task.taskId),
    interpretation: { lifecyclePass: `all ${TASKS.length} selected exact routes admitted${REQUIRE_GROK_PAIR ? ', both Grok process groups sampled live simultaneously' : ''}, every started generation closed exactly, driver drain closed coordinator/writer, and all owned residue disappeared`, matrixPass: `lifecyclePass plus ${TASKS.length} fresh-verified report(s) and identical worker/verifier toolchain projection binding` },
    manualWorkerKills: false, legacyDriverClose: false, closureReceipt,
    credentialMeasurements, simultaneousGrokSamples, attempts,
    rows: rows.map((row) => ({ taskId: row.taskId, harness: row.harness, model: row.model, workerId: row.workerId, pid: row.pid ?? null, processGroupId: row.processGroupId ?? null, result: row.result ? { status: row.result.status, ready: row.result.ready } : null, route: row.handle ? { harnessRequested: row.handle.harnessRequested, harnessResolved: row.handle.harnessResolved, modelRequested: row.handle.modelRequested, modelResolved: row.handle.modelResolved, modelObserved: row.handle.modelObserved, modelMismatch: row.handle.modelMismatch, effortRequested: row.handle.effortRequested, effortResolved: row.handle.effortResolved, effortObserved: row.handle.effortObserved } : null, budgetUsed: row.handle?.budgetUsed ?? null, verifyAccept: row.verify?.payload?.accept ?? false, budgetAdmission: row.verify?.payload?.budgetAdmission ?? null, reportCaptured: Boolean(row.report), processStartedSeq: row.processStarted?.seq ?? null, processClosedSeq: row.processClosed?.seq ?? null, terminalReason: String(row.events?.findLast((event) => ['lifecycle.crashed', 'model.mismatch'].includes(event.kind))?.payload?.error ?? row.events?.findLast((event) => event.kind === 'lifecycle.crashed')?.payload?.reason ?? '').slice(0, 512) })),
...
if (targetCreated) {
  try {
    sourceGit(['worktree', 'remove', '--force', TARGET_REPO]); sourceGit(['worktree', 'prune']);
...
process.stdout.write(`${JSON.stringify({ lifecyclePass: summary.lifecyclePass, matrixPass: summary.matrixPass, routeAdmission: summary.routeAdmission, providerProof: summary.providerProof, cleanup: summary.cleanup, reviewProof: summary.reviewProof, fatal: summary.fatal, targetWorktreeRemoved: summary.targetWorktreeRemoved }, null, 2)}\n`);
