...
const LANES = Object.freeze({
  ...
  'frame-economics': Object.freeze({
    ...
    anchors: [
      ...
      '2048); resend within the 2048-byte cap") — the B-section pins dual-assert payload AND message on all',
      '14 lanes with cap+actual. The spill lane: mintSpill/materializeSpill in coordination-store.mjs',
      '(1 MiB ceiling + spill_body_exceeded coaching refusal, byte-identical roundtrip, durable',
      'spill.minted event), the spill query kind on the read port (CONTEXT_READ {kind:"spill"} through',
      '_renderContextRead, UNTRUSTED), spilled message.delivered/messageReceipt envelopes',
      '{body: head, bytes, digest, spill} with head+citation NEVER cap-voiding full delivery (C4) and',
      'NEVER unresolvable head-only (C5); the wave-member admission multibyte path admits-with-spill with',
      'byte-measured accounting (C10, wave-member-spill-missing stage — the validText char wall goes).',
      'Scanner posture: all SIX grammars shape-only + the BOARD_REPORT doc-comment law sentence',
      '(claude-session.mjs — the D8 row). Doctor: doctorReadiness().limits projection + card()',
      'limitsRegistryDigest + handshake cli_connection_incompatible on mismatch. Store-consumer',
      'dispositions per v1.2 Decision 2 (board.report.body LIVE 4096 → board_report_exceeded coaching,',
      'run.legacy_send.body LIVE 16384 → run_legacy_send_exceeded). Wave-driver onAdvisory callback.',
      'Anchors: impl/src/coordinator.mjs sendMessage/_renderContextRead, coordination-store.mjs',
      'submitBoardReport :14442 region, application.mjs notifyWorkstream/validText, application-cli.mjs',
      ':1970 handshake, wave-driver.mjs policy, application-semantics/mcp-northbound (Decision 1 consumers).',
    ].join(' '),
  }),
