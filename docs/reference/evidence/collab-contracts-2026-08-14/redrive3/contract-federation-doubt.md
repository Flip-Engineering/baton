# CONTRACT — federation-doubt (package ⑤ collaboration) v1.1

[attempt: 5262cdfa-7068-4a59-8ad5-f80446d710a7 row-federation-doubt]
...
- **Verification HEAD:** `5ae2c7e5c93d99404d3a292e777dd30f7d2ead27` (this worktree's base). Every
  ...
  session; never read whole) and plain `grep -rn` elsewhere. Both seed red suites were **re-run at
  this HEAD** (commands and splits recorded in §4).
- **Lineage:** this is the redrive3 re-drive of the redrive2 v1.0 contract (commit `ea1cc1ed`,
  verified at HEAD `09200e9`). v1.1 carries every v1.0 ruling forward UNCHANGED, re-verifies the
  full ground-truth base at the new HEAD, and records the anchor drift (J-3) — three impl commits
  landed between the two HEADs (`85519556` steering index, `cda6355b` bootstrap base-commit,
  `8ec52a6c` hardCap removal) and moved eight anchors; none moved a ruling.
...
the #70 project-tier-across-roots rung is contract-only"; row re-read this session), and it still
holds. The #70 suite is the pinned contract for that gap (22 RED / 9 PIN; re-run split in §4).
...
(`coordinator.mjs:399-402`, `application.mjs:757-761` — **drifted +3** from v1.0's `:754-756`, J-3).
There is **no** `knowledge.doubt_*` event kind (`grep -rn "knowledge.doubt"` over `impl/src`
returns nothing), no `resolveDoubt`, no `knowledge.doubts` row, no `openDoubts` field, and no
`view.open_doubts.*` / `doubt.resolution.bytes` rows in `FRAME_LIMITS` (`grep -an "doubt"` over
`impl/src/limits.mjs` returns nothing). The settle ritual selects `note`+`plan` only
(`coordinator.mjs:12031`), scans the shared partition for `note` only (`:12040-12043`),
materializes on notes only (`:12050`, **drifted +1**), posts board candidacy for notes only
(`:12089`), and returns `candidatesAwaitingAdmission: elevatedNotes.length` (`:12098`); the wave
receipt folds the same four knowledge fields with no doubt field (`wave-driver.mjs:863-869`, and
the `wave.closed` record's fold at `:883-888` — both **drifted** from v1.0's `:861-864`/`:879-884`,
J-3). The #66 suite is the pinned contract for this gap (30 RED / 5 PIN; re-run split in §4).
...
'note'` (`:14441`, bridge mint through `:14461`) and carries `scratchFactId: factPayload?.id ??
null` for everything else; unselected rows disposition `not_elevated` with `reasonCode: steering ?
'orchestrator_skipped' : 'no_driver'` (`:14486-14487`, **drifted** from v1.0's `:14475-14489`).
`_deriveKnowledgePromotion` (`:16049`) re-derives candidates from scratch facts — a doubt has no
bridge fact, so it cannot auto-candidate (structural, unchanged from the #66 GT5).
`sweepSettlementLeases` is at `:12618`; `settleWorkflowScratchpad` dispositions `type_ineligible`
for every non-note shared row (`:14546`).

**GT4 — The knowledge read lanes both contracts ride.** `run.knowledge.seed` dispatches at
`application.mjs:12711` (**drifted +53** from v1.0's `:12658`); the MCP lane exposes
`knowledge.recall` / `knowledge.horizon` (`mcp-northbound.mjs:861-862`, tool validation at
`:1187-1191`, dispatch at `:2199-2204`). `coordinator.projectHorizon` is at `coordinator.mjs:12273`
and returns the plain HEAD shape (`edges, fenceTuple, nodes, repoId` — the #70 suite's G1 pin). The
store's read/recall verbs and their refusal codes (`temporal_incoherence`,
`knowledge_read_conflict`, `causal_recall_*`, `coordination_writer_busy`) are pinned by the #70
suite's K-P1 row — re-verified green in this session's re-run.
...
`application-deployment.mjs` (`repositoryAuthority`, `:176-183` this session). These are the
...
campaign-state tier is replay-derived from `wave.closed` records (`:8846-8847`, `:13364-13381`)
and the orchestrator briefing's closed top-level field set is frozen
(`BRIEFING_TOP_LEVEL_FIELDS`, `:514-518`, **drifted −1** from v1.0's `:515-518`). **There is no
impact surface anywhere in `impl/src`** (`grep -rln "impact" impl/src/` matches only the atlas CPG
files — `atlas-cpg-delta.mjs`, `atlas-representation-producer.mjs`,
`atlas-representation-review.mjs`: the phase-19 code-analysis delta surface, unrelated to portfolio
impact).
...
never a throttle"** (`pm-comparison-2026-08-13/pm-qa.md:73` row D2; rubric `:105`: "only as
projection (never throttle — else Q8 ornament)") and the DAG row ruled it "**read-side advisory
only, structurally derived, never a focus-window driver** … downstream-blocked-count is structural
(event-derived), pm's operator-assigned `impact` scalar is not" (`pm-comparison-2026-08-13/
pm-dag.md:91` row C2, detail §`C2` at `:124`). pm's own landed code is the counter-example this
contract forbids: `next_phases` sorts by the operator-assigned bare `impact` number
(`pm-dag.md:30-31`, `:126-128`). All five evidence anchors re-read this session.
...
## 2. Decisions (carried from v1.0 unchanged; re-affirmed at this HEAD)
...
- **Never a throttle:** no admission, lease, claim, dispatch, or elevate path reads the projection.
  ...
  `wave-driver.mjs:872-878` — "advisory, never gating … a typed refusal is captured into the
  bounded settlement.errors … and the wave stays closed", re-read this session after the hardCap
  removal moved the block).
...
### The seed rungs — re-verified this session at HEAD `5ae2c7e`
...
- **P-70.** `node --test impl/test/cross-deployment-knowledge-red.test.mjs` →
  **tests 31 · pass 9 · fail 22 · cancelled 0 · skipped 0 · todo 0** (verbatim this session;
  exit 1). The 22 RED rows fail at their named stages per the suite header; the 9 PIN rows stay
  green. Green only for a correct #70 impl.
- **P-66.** `node --test impl/test/doubt-review-red.test.mjs` →
  **tests 35 · pass 5 · fail 30 · cancelled 0 · skipped 0 · todo 0** (verbatim this session;
  exit 1). The 30 RED rows fail at their named stages; the 5 PIN rows (A2, G1-G4) stay green.
  Green only for a correct #66 impl. *(The row brief's "5/35 RED" reads as this split — 5 green of
  35, i.e. 30 RED; judgment call J-2.)*
...
- **FD3 — local authority survives federation (RED, discriminating).** A non-primary deployment
  ...
  is note-only at HEAD (`coordinator.mjs:12031`, re-verified this session) — RED with the whole #66
  rung; this row's discriminating half (a wrong impl wiring the primary check onto the doubt verbs
  fails it) is unobservable until both contracts land — recorded, not hidden.**
...
- **FD6 — the #192 red lines (RED, landing-zone-independent per D3).**
  ...
  - **FD6b (never a throttle):** source-scan pin — no admission/lease/claim/dispatch/elevate path
    ...
    window, the `wave-driver.mjs:872-878` pattern). **Stage: absent at HEAD (GT6).**
...
- **FD7 — vocabulary hygiene (RED).** The seam's refusal family constants exist frozen in ACTUAL
  ...
  stays empty (re-verified empty this session). **Stage: the constants are absent at HEAD
  (GT1/GT2 — `grep` over `impl/src` finds them only in the #70 test file).**
...
**DR-1 — the #192 landing zone (authority-class ambiguity, escalated with options; OPEN,
carried from v1.0).** The pm-comparison rulings (GT7) leave the hosting surface open, and the two
natural homes sit in different contract domains: the campaign/wave tier is package ③'s surface
(the lifecycle rows), while this row holds #192. Options:
...
- **(b) A block on the `wave.closed` campaign-state record + a receipt fold** — closest to
  ...
  contract froze — the closed 8-key `wave.closed` shape at `coordination-store.mjs:13375` is
  re-verified frozen this session) and crosses into package ③'s boundary map.
...
the green row lands. Until settled, (a) is the working assumption and is NOT pinned as
green-anywhere. **No settling decision has landed between redrive2 and redrive3** — DR-1 rides
again, verbatim.

## 6. Judgment calls (recorded)

- **J-1 — the foundry-brief frame mismatch.** `redrive3/foundry-brief.md` is the
  LIFECYCLE-CONTRACT frame: its row assignments name the `row-lc-*` rows (package ③) and do not
  name `row-federation-doubt`, though its header sentence names the COLLABORATION package (⑤).
  The wavefile, the row brief, and the Baton dispatch all name my deliverable unambiguously, and
  the brief's laws (attempt-echo, red-first, no clocks, NUL discipline, publish-or-refuse) are
  package-generic — so I proceeded on the row brief + wavefile and treated the laws as binding.
  Not escalated: the deliverable is not ambiguous. Flagged for the coordinator's cross-check: the
  redrive3 seed copied a lifecycle foundry-brief into a collab wave dir (same defect as redrive2,
  so it propagated with the seed copy).
- **J-2 — reading "5/35 RED".** The row brief's "doubt-review-red 5/35 RED" matches the suite's
  recorded split as *5 pass of 35 tests* (30 RED). My re-run at this HEAD reproduces exactly that.
  Recorded rather than silently paraphrased.
- **J-3 — anchor drift between redrive2 v1.0 and this re-drive, and the deliverable-path
  discrepancy.** (a) Three impl commits landed between the two verification HEADs
  (`85519556` · `cda6355b` · `8ec52a6c`: the single-pass steering index, the async bootstrap
  base-commit, the hardCap removal). They moved eight anchors — `application.mjs:754-756→757-761`
  and `:12658→12711`, `coordinator.mjs:12049→12050`, `wave-driver.mjs:861-864→863-869` and
  `:879-884→883-888` and `:871-875→872-878`, `coordination-store.mjs:14475-14489→14486-14487`
  and `:515-518→514-518`, `application-deployment.mjs:177-183→176-183` — and moved no ruling;
  every v1.0 ground truth re-verified true at `5ae2c7e`. All drift is disclosed inline in §1.
  (b) The row brief's deliverable line says `redrive2/contract-federation-doubt.md` (stale seed
  text — it is byte-identical in the redrive2 and redrive3 seed dirs), while the wavefile's
  report path, the wavefile's harvest row, and the Baton dispatch scope all say
  `redrive3/contract-federation-doubt.md`. The wavefile is the admission authority and the Baton
  scope is the hard boundary, so the contract is written to **redrive3**. Not escalated: the
  authoritative sources agree; only the stale brief line disagrees.
- **J-4 — D1-ii (doubt minting stays local on non-primary roots) is a seam ruling, not a re-opened
...
- **J-5 — v1 honesty on resolution prose.** No resolution text ever crosses a root in v1 (D1-i).
  A future "answer a carried doubt from the primary" surface is a named follow-up (OQ1), not a
  claim this contract makes.
- **J-6 — the shared publish (foundry law 3).** Publishing to `shared` is not executable from this
  session: the write seam hardcodes the worker scope (GT8, `coordination-store.mjs:14169`/`:14366`,
  re-verified this session) and the #158 append verb is unlanded. The refusal is recorded here with
  fresh citations; this file is the durable artifact. (Same posture the pm-comparison coordinator
  recorded, `pm-qa.md` §1.)
...
| Pin | Rung | Stage (RED at HEAD `5ae2c7e`) | Green when |
|---|---|---|---|
