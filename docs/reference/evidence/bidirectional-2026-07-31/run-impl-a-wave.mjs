...
const baton = await openBaton({
  repo,
  advanced: {
    deploymentRoot: resolve(repo, '.baton', 'bidirectional-impl-a2-2026-07-31'),
...
try {
  const driver = createWaveDriver(baton, {
    ...
    evidencePath: resolve(repo, 'docs/reference/evidence/bidirectional-2026-07-31/impl-a2-evidence.json'),
    onProgress: (line) => log(`progress ${line}`),
  });
  const receipt = await driver.run({
    members: [{
      role: 'bd-a2-implementer-sonnet',
      objective: OBJECTIVE,
