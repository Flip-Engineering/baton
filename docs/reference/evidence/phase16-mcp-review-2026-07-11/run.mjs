...
const LOG_DIR = mkdtempSync(join(tmpdir(), 'baton-mcp-review-log-'));
const TASK_ID = 'codex-mcp-northbound-review';
const MODEL = 'gpt-5.6-sol';
const EFFORT = 'low';
const REPORT = 'docs/reference/evidence/phase16-mcp-review-2026-07-11/review.md';
...
const { coordinator, log } = createDriver({
  ...
  approvalTimeoutMs: 60000, stopDeadlineMs: 15000, watchdog: { stallMs: 300000 },
});
const brief = createBrief({
  goal: 'Independently adversarially review Phase 16 MN1-MN10. Trace protocol lifecycle/framing, schema compliance, injected identity and quotas, durable effect idempotency, post-effect ambiguity, exact harness/model/effort propagation, error disclosure, bounded waits, audit behavior, restart folding, stdout purity, and process usability. Write exact severity/source/failure/regression findings or explicitly none.',
  constraints: [
    `Write only ${REPORT}.`,
    'Review the Phase 16 spec, MCP source/tests/entry point, coordination fold, and stable protocol semantics represented in the contract; do not read prior evidence logs.',
    'Do not expand this review into Streamable HTTP, Tasks, WebSocket, homelab, or unrelated Atlas work.',
    'No source edits, commit, network, homelab, or fleet actions.',
  ],
  pathScope: [REPORT],
  definitionOfDone: 'Every MN1-MN10 seam is reviewed with actionable findings or an explicit clean verdict',
  verification: { command: `test -s ${REPORT} && npm --prefix impl test -- --test test/phase16-mcp-northbound.test.mjs`, expectExit: 0, timeoutMs: 240000 },
...
const pump = (async () => {
  ...
  while (pumping) {
    for (const worker of coordinator.list()) {
      ...
      seen.add(requestId);
      await coordinator.respond(requestId, worker.pendingApprovalId ? { decision: 'allow' } : { text: 'Finish the report-only MCP review.' }, 'human');
    }
    await sleep(100);
  }
})();
...
try {
  ...
  workerId = handle.id;
  const spawned = await until(() => log.read(workerId).find((event) => event.kind === 'lifecycle.spawned' && event.actor === 'worker'), 'worker spawn');
  ...
  if (workerId) {
    try {
      await until(() => (!pid || !alive(pid))
        ...
        && git(['branch', '--list', `baton/${TASK_ID}`]) === '', 'complete reap', 30000);
    } catch (error) {
