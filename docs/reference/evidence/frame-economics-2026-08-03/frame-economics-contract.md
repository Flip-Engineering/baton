# Issue #89 — Frame-economics contract (v1.2)
...
Fold: post-blue-team v1.2 — the verdicts of
`docs/reference/evidence/frame-economics-2026-08-03/suite-blueteam.md` (**NOT-READY, 4
blockers**; the suite itself verified honest and sharp) are folded in place on 2026-08-04:
**4/4 blockers folded, 0 rejected**. (1) The v1.1 fold's belief that the store's
board-report body check is shape-only was WRONG about this tree: `submitBoardReport`
enforces a LIVE 4,096-byte bound (`coordination-store.mjs:416` declared, `:14442` enforced
via `boardBounded` `:430-432` — `Buffer.byteLength(value) <= maxBytes`, refusal
`invalid_board_report`; second live door `application-semantics.mjs:1426`). Decision 2 and
Acceptance A now catalog `board.report.body` 4,096 as a live ADMISSION row (hard, coaching,
`board_report_exceeded`) — the `board.title`/`board.detail` disposition — at the LIVE value,
so Decision 8's no-value-change law holds; deleting the bound would have smuggled a behavior
REMOVAL in as "consolidation", the inverse of blocker 8's own rationale. (2) The
legacy-alias send door (`run.send` / `run.act send` / `run.workstream.notify` / `waves.send`
message args, enforced inline at 16,384: `application.mjs:1797`, `:2930`,
`coordination-store.mjs:4292`, schemas at `application-semantics.mjs:299/:523/:1596` and
`mcp-northbound.mjs:357/:412/:485`) is adjudicated: cataloged as the named admission lane
`run.legacy_send.body` at its LIVE 16,384 value, hard with the Decision-3 coaching shape —
an unscoped 16,384 door beside the cataloged 2,048+spill lane was a documented bypass of the
frame economy. Collapsing the alias into `message.send.body` (2,048 + spill) is an
operator-visible follow-up (Non-goals), not consolidation. (3) The schema/northbound
second-door class is named explicitly: Decision 1's consumer list and Decision 8's
no-re-declare law now reach `application-semantics.mjs` / `mcp-northbound.mjs` /
`web-northbound.mjs` — 16 of the suite scan's 46 current hits (17 counting the
`application.mjs:1855` wave-member char door, which the byte law names separately) — which
import the registry row or retire their duplicate checks; the suite's unconditional reading
of "no module re-declares" is ratified. (4) The wave-member byte law gains its oracle in the
SUITE (a new row drives an oversize multibyte wave member through the real `waves.start`
admission and the `waves.attach` door, asserting admit-with-spill, never refusal, and
byte-measured accounting); no contract text change — Decision 2's byte law and Acceptance C
already name the behavior. The suite-side fold map is `suite-fold.md` beside this file.
Line citations moved by the third same-day tree shift are re-anchored in place (send
TypeError `6609-6610` → `6633-6634`; orientation note `6898` → `6922`; reply seam
`11835-11878` → `12167-12217`, spread seam `12179-12184`; closed envelope `11864-11866` →
`12207-12209`; decision seam `11974-11989` → `12317-12329`; renderer `10416-10452` →
`10419-10472`; replay switch `13129` → `13472` region; scope-refresh shrink `8470-8475` →
`8496`; serveKnowledge `10141-10147` → `10188`; inspectCapturedFile `4610` → `4634`;
wave-driver precheck `304-312` → `321-329`; store reuse literals `3471` → `3485`; board
title/detail enforcement `14064-14066` → `14270-14272`; board-report check `14423-14425` →
`14442`; pack body enforcement `13042` → `13060`).

Status: implementation contract, red-team-folded (v1.1) and blue-team-folded (v1.2). This specifies behavior; it does not amend
...
`impl/src/wave-driver.mjs:321-329`). Refs: #86 (the exchange that spawned this), #10 (AX spine
...
1. **The send-lane sin.** `sendMessage` validates
   ...
   (`impl/src/coordinator.mjs:6633-6634`). The cap appears in prose, the actual size does not,
   there is no payload carrying either number, and there is no spillover path. The lane's doc
   comment promises durable-stream receipts (`coordinator.mjs:6623-6627`).

2. **The reply lane has no body bound.** The `message.send` admission case checks only that
   `frameBody` is a non-empty string (`coordinator.mjs:12177-12185`). Its refusal helper
   `refuse(reason, extra)` appends `message.rejected` with payload `{reason, inReplyTo,
   ...extra}` (`coordinator.mjs:12179-12184`) — the spread seam a coaching payload needs
   already exists. Existing reasons: `message_frame_invalid`, `message_target_caller_named`,
   `message_parent_not_found`, `message_depth_exceeded` (`coordinator.mjs:12186-12200`).
   Delivered replies mint `message:<digest>` ids and append `message.delivered`
   (`coordinator.mjs:12204-12221`); the closed reply envelope `{messageId, inReplyTo, from,
   body}` is frozen at `coordinator.mjs:12207-12209` (Decision 4 amends it explicitly).
...
3. **The BD3-A oversize pattern (the model to generalize).** `_renderContextRead` is the
   ...
   overflow (`coordinator.mjs:10419-10472`; the truncation + digest citation at
...
5. **The decision-question leak (a live wire-level cap).** `createDecisionRequest` enforces
   ...
   refusal at `coordinator.mjs:12317-12329` (`control.malformed_interaction_rejected` +
   ...
   `postBoardItem` at `:14270-14272`). **The live board-report lane is ALSO live-bounded in
   the store (v1.2 correction, blue-team blocker 1):** `MAX_STORE_BOARD_REPORT_BYTES = 4_096`
   (`coordination-store.mjs:416`, declared beside the title/detail constants) is enforced in
   `submitBoardReport` at `:14442` — `boardBounded(fields.body,
   MAX_STORE_BOARD_REPORT_BYTES)` throws `CoordinationRefusal('board report body must be
   bounded non-empty', 'invalid_board_report')`, and `boardBounded` (`:430-432`) is
   `Buffer.byteLength(value) <= maxBytes`, a BYTE bound, not a shape check. The lane is live
   end to end: the #78 grammar ships (`scanForBoardReport`, `claude-session.mjs:195-216`), the
   coordinator admits reports (`admitWorkerBoardCommand('report', …)`,
   `coordinator.mjs:12161`), and the application-semantics `board.report` command binds
   `liveMethod: 'admitWorkerBoardCommand → submitBoardReport'` with a SECOND live 4,096 door
   (`body: {…, maxLength: 4096 }`, `application-semantics.mjs:1426`). The v1.1 fold's anchor
   (`:14423-14425`) landed inside the function's doc comment/idempotency prelude, ~17 lines
   short of the enforcement line — the check it called "shape-only" is a numberless 4,096 cap
   refusal, precisely the sin class this epic exists to eliminate. The de-facto input bound
   on the lane is therefore 4,096, not 20,480 — see Decision 2.
...
6. **The run-objective class.** Run intent validates `validText(value.objective)` whose
   ...
   `wave_driver_objective_oversize` (`impl/src/wave-driver.mjs:27, 321-329`), receipted at
   `docs/37-wave-driver.md:90-94`.

7. **Other admission-side caps.** Orientation push note ≤2,048, bare TypeError 'orientation
   push note is invalid' (`coordinator.mjs:6922`); its internal scope-refresh caller already
   degrades gracefully by shrinking the observed path until the note fits
   (`coordinator.mjs:8496`). Steering policy focus ≤2,048 (`coordinator.mjs:1000`). Reuse
   ...
   (`coordination-store.mjs:3485`; `boundedText` at `:357`) — silently flooring the deployment
...
8. **View-side ceilings already degrade gracefully with a flag — the precedent class.** Board
   ...
   maxBytes 2,048 at `coordinator.mjs:10188`). Remaining view ceilings:
   ...
   `inspectCapturedFile` 4MiB (`coordinator.mjs:4634, 4644`). NOTE: `boundedAttentionText`
...
9. **Substrate guards (not policy).** `DEFAULT_MAX_WIRE_FRAME_BYTES` 1MiB
   ...
   ≤8,192 (`impl/src/coordination-store.mjs:487`, enforced at `:13060`).
...
10. **The artifact machinery — and why none of it fits spillover verbatim.**
    ...
    (`:487`, enforced `:13060`) — all wrong for spill, which must out-cap the inline lane and
...
`board.title` / `board.detail` / `board.report.body` bounds today and is a first-class
registry consumer, ground truth 5/7/9), refusal text builders, the CLI/doctor projections,
and the scanner module's doc posture. **The arg-schema and northbound layers are registry
consumers too (v1.2, blue-team blocker 3):** the `application-semantics.mjs` command schemas
and the `mcp-northbound.mjs` / `web-northbound.mjs` adapters carry char `maxLength` schemas
and byte checks that are SECOND DOORS of cataloged lanes (`run.objective`, `decision.text`,
`board.detail`, `board.report.body`, `orientation.note` — e.g.
`application-semantics.mjs:163/:487/:1356/:1369/:1426/:1551/:1575`,
`mcp-northbound.mjs:314/:322/:444/:464/:607/:910/:927/:935`, `web-northbound.mjs:458`).
Exempting them would ratify the wave-member char-check sin class at the schema layer; each
such door imports the registry row (never a hand-typed re-declaration) or retires its
duplicate check to the lane's admission seam. Decision 8's no-re-declare law is explicit
about this.
...
`coordination-store.mjs:3485`): the hidden floor becomes the declared ceiling-of-ceilings
...
- `lane` — dotted, stable: `message.send.body`, `message.reply.body`, `run.objective`,
  ...
  `run.legacy_send.body`, `scratchpad.entry.body`,
...
- `value` + `unit` — **the byte law: every admission bound is measured in UTF-8 BYTES via
  ...
  (`coordinator.mjs:6633`; `application.mjs:289-290`). **Scope of the law (fold):** every
...
**`board.report.body` is a live ADMISSION row at the live 4,096 store value (v1.2 correction,
blue-team blocker 1; supersedes the v1.1 "substrate-bounded" fold, blocker 8).** The v1.1 fold
believed the store's body check was shape-only (`coordination-store.mjs:14423-14425`) and
declared the lane substrate-bounded with no admission row; that belief is verifiably wrong in
this tree (ground truth 5): `submitBoardReport` enforces `MAX_STORE_BOARD_REPORT_BYTES =
4_096` (`:416` declared beside the title/detail constants, `:14442` enforced via
`boardBounded` `:430-432` — `Buffer.byteLength(value) <= maxBytes`, refusal
`invalid_board_report`), with a second live door at the arg-schema layer
(`application-semantics.mjs:1426`). Implementing the v1.1 text would have forced either
DELETING the live bound — a behavior removal smuggled in as "consolidation", the exact
inverse of blocker 8's stated reason for not cataloging the dead factory's 4,096, and a
violation of Decision 8's no-value-change law — or keeping it and failing the single-source
ratchet forever. v1.2 therefore catalogs `board.report.body` 4,096 as an admission row, hard
with the Decision-3 coaching shape, `refusalCode: 'board_report_exceeded'` — the
`board.title`/`board.detail` disposition exactly, at the LIVE value, no behavior change. The
dead `createBoardReport` factory (`messages.mjs:315, 378`) stays uncataloged (blocker 8's
core ruling stands); the `scanner.window.board_report` 20,480 substrate row stays declared,
so doctor shows both the admission bound and the wire-layer resource guard. Promoting the
lane to `spill-digest-citation` later is an operator-visible follow-up (Non-goals), as it was
in v1.1. `board.title` / `board.detail` are unchanged: live-bounded in the store
(`coordination-store.mjs:414-415`, enforced `:14270-14272`), not in the dead factory —
cataloged as admission rows at those live values (160 / 4,096).

**The legacy-alias send door is a named admission lane at its live 16,384 value (v1.2,
blue-team blocker 2).** The legacy `run.send` / `run.act send` / `run.workstream.notify` /
`waves.send` message argument is enforced inline at 16,384 BYTES today
(`application.mjs:1797` and `:2930` `validText(…, 16_384)`; `coordination-store.mjs:4292`
`boundedText(p.message, 16_384)`; arg schemas `application-semantics.mjs:299/:523/:1596` and
`mcp-northbound.mjs:357/:412/:485` `maxLength: 16384`) — beside the cataloged
`message.send.body` 2,048+spill typed lane, an unscoped 16,384 alias is a documented bypass
of the frame economy: every driver that can reach the alias routes around the registry. v1.2
adjudicates the door the way every other live bound is adjudicated: catalog `run.legacy_send.body`
16,384 as an admission row, hard with the Decision-3 coaching shape, `refusalCode:
'run_legacy_send_exceeded'` — the LIVE value, no behavior change beyond the coaching payload
— and the doors import the registry row like every other cataloged literal (the alias's own
bound predates the typed lane, which is why it is named as its own lane rather than silently
inheriting `message.send.body`). Collapsing the alias into `message.send.body` (2,048 +
spill) or promoting it to `spill-digest-citation` is an operator-visible follow-up
(Non-goals); leaving it unscoped was the one disposition rejected, because doctor could not
show the real bound and the ratchet could not name it.
...
- `:3485` — `boundedText(p.need, 2_048)` / `boundedText(p.rationale, 8_192)` → **registry
...
- `:487` — `MAX_CONTEXT_PACK_BODY_BYTES = 8_192` (enforced `:13060`) → already-cataloged
...
- `:414-415` — `MAX_STORE_BOARD_TITLE_BYTES` 160 / `MAX_STORE_BOARD_DETAIL_BYTES` 4_096
  (enforced `:14270-14272`) → **registry rows** `board.title` / `board.detail`; the store
  imports them.
- `:416` — `MAX_STORE_BOARD_REPORT_BYTES = 4_096` (enforced `:14442` via `boardBounded`
  `:430-432`, refusal `invalid_board_report`) → **registry row** `board.report.body` 4,096,
  admission class, hard with the Decision-3 coaching shape (v1.2, blue-team blocker 1); the
  store imports it.
- `:4292` — `boundedText(p.message, 16_384)` on the legacy run-control send record →
  **registry row** `run.legacy_send.body` 16,384, admission class, hard with the Decision-3
  coaching shape (v1.2, blue-team blocker 2); the store imports it. The sibling application
  doors (`application.mjs:1797/:2930`) and arg schemas (`application-semantics.mjs:299/:523/
  :1596`, `mcp-northbound.mjs:357/:412/:485`) import it too (Decision 1's consumer list).
...
- a structured payload `{reason|code, cap, actual, unit: 'bytes', gracefulPath}` — for the
  ...
  `message.rejected` on the worker's durable stream (`coordinator.mjs:12179-12184`); for the
...
- a human message naming BOTH numbers: e.g.
  ...
  (`coordinator.mjs:6634`), never `'run intent is invalid'` with no number
  (`application.mjs:1406`).

The wave-driver precheck (`wave-driver.mjs:321-329`) is the behavioral floor — it already
...
3. Receipts carry the digest: `message.sent` / `message.delivered` and the coordinator's
   ...
   `coordinator.mjs:12207-12209`) to `{messageId, inReplyTo, from, body, spilled?, bytes?,
...
- **The decision-question split (the consequence that makes this honest):**
  ...
  travels; the admission seam (`coordinator.mjs:12317-12329`) applies the registry bound and
...
- **Replay neutrality (fold).** Registry bounds apply at LIVE admission only: the
  replay/reconstruction switch (`coordinator.mjs:13472` region) rebuilds pending-interaction
...
(`coordinator.mjs:12177-12185`) with spill per Decision 4, so an oversize reply degrades to
...
`coordination-store.mjs` lane literals (`:3485`, `:484`, `:487`, `:414-416`, `:4292`) move to or
...
locals — named, not discovered. **Layer-unconditional (v1.2, blue-team blocker 3):** "no
module re-declares" reaches the arg-schema and northbound layers exactly as it reaches
coordinator/store/application code — the `application-semantics.mjs` char `maxLength`
schemas and the `mcp-northbound.mjs` / `web-northbound.mjs` byte checks on CATALOGED lanes
(the second-door class of Decision 1's consumer list) import the registry row or retire
their duplicate checks; no schema re-declares a cataloged lane's bound, as a char
`maxLength` or otherwise. The suite's unconditional reading of this law is the ratified one:
those hits are not exemptable, precisely so the wave-member char-check sin class cannot
re-emerge at the schema layer.
...
number (the `'<=2048 bytes'` prose at `coordinator.mjs:6634` and the `<=${MAX} bytes`
...
- **Spill beyond the two first lanes.** Decision-question text, reuse need/rationale, board
  ...
  them to `spill-digest-citation` is a follow-up per lane. (`board.report.body` keeps its
  LIVE 4,096 store bound as a hard admission row with coaching — v1.2, blue-team blocker 1;
  promoting it to spill is the operator-visible follow-up, not this epic.)
- **Collapsing the legacy-alias send door into `message.send.body`.** `run.legacy_send.body`
  is cataloged at its LIVE 16,384 value (v1.2, blue-team blocker 2); cutting the alias to
  2,048 + spill, or deleting the alias outright, is an operator-visible behavior change for
  its own decision, not consolidation.
...
`coordination-store.mjs:3485`, Decision 1(d)); `orientation.note` 2,048 hard;
`steering.focus` 2,048 hard; `board.title` 160 / `board.detail` 4,096 hard (LIVE store
values, `coordination-store.mjs:414-415`, enforced `:14270-14272` — NOT the dead
`messages.mjs:313-314` factory constants); `board.report.body` 4,096 hard (LIVE store value,
`coordination-store.mjs:416`, enforced `:14442` via `boardBounded` `:430-432` — the
`board.title`/`board.detail` disposition, v1.2 blue-team blocker 1; today's refusal is the
numberless `invalid_board_report` and gains the Decision-3 coaching shape,
`refusalCode: 'board_report_exceeded'`); `run.legacy_send.body` 16,384 hard (the legacy
`run.send` / `run.act send` / `run.workstream.notify` / `waves.send` message door — LIVE
inline values `application.mjs:1797/:2930`, `coordination-store.mjs:4292`, v1.2 blue-team
blocker 2; `refusalCode: 'run_legacy_send_exceeded'`); `decision.option.label` 160 /
...
`refusalCode`). `board.report.body` is an admission row (above); the
`scanner.window.board_report` substrate row stays declared beside it, so doctor shows both
the admission bound and the wire-layer resource guard (Decision 2, v1.2). View class:
...
8/2,048 (`coordinator.mjs:10188`), context-read items 8/64 (`coordinator.mjs:10427`),
`inspectCapturedFile` 4MiB (`coordinator.mjs:4634`).
...
`message.rejected` (`coordinator.mjs:12179-12184`). Includes the previously-silent cases:
...
`coordinator.mjs:6633-6634`). **De-tautologized (fold, blocker 10):** at least one HARDCODED
...
driver advisory passes it through — Open question 5, resolved). **The wave-member row drives
the REAL application admission (v1.2, blue-team blocker 4):** an oversize, multibyte wave
member through `waves.start` (and the `waves.attach` member door) — never a facade — so a
char or byte WALL surviving behind the driver's advisory
(`application_wave_attach_invalid` / `application_wave_start_invalid`) cannot green the row;
the accounting is byte-measured (`Buffer.byteLength`, the Decision-2 byte law fixing
`application.mjs:1854-1855`). Idempotent re-drive replays
...
(`coordinator.mjs:12317-12329`), closing the ground-truth-5 leak.
...
and `coordinator.mjs:6633` are the first two literals this row retires; the store's lane
literals (`coordination-store.mjs:3485, 484, 487, 414-416, 4292`) retire with them, as do the
schema/northbound second doors (Decision 1's consumer list, v1.2).
...
4. **Ceiling-of-ceilings for deployment-injected ceilings — RESOLVED in the fold (blocker
   ...
   8,192 (`coordination-store.mjs:3485`) silently floored every deployment override. The
...
5. **Wave-driver precheck retention — RESOLVED in the fold (blocker 9): downgraded to a
   spill-aware advisory.** The precheck (`wave-driver.mjs:321-329`) today REFUSES oversize at
