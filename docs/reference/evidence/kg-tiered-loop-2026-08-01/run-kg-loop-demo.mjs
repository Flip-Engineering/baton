...
import { openBaton, createWaveDriver, DEFAULT_RUN_LINEAGE_POLICY } from '../../../../impl/src/index.mjs';
...
try {
  ...
  if (!waveRunId) {
    const runRow = snapshot.runs[0] ?? null;
    waveRunId = runRow?.runId ?? runRow?.id ?? null;
...
writeFileSync(resolve(EVIDENCE, 'kg-loop-receipt.json'), `${JSON.stringify(receipts, null, 2)}\n`);
