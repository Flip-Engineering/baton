...
try {
  ...
  await workflow.stop('Recover the interrupted result-capsule Run and reap exact ownership.');
...
const evidence = {
  schemaVersion: 1, runId, outcome: 'interrupted_then_recovered', runStatus,
  candidates, stopped, closed,
};
...
if (![1, 3].includes(stopped?.stop?.receipt?.schemaVersion)
  ...
  throw new Error('recovered Phase 85 result-capsule deployment did not prove zero ownership');
}
