...
## Fold record (incremental — blue-team wave-a → suite v1.2; full map in `fold-suite-155.md`)

This section is an append-only delta. The suite file was folded in place per
`blue-team-2026-08-13-a/blueteam-155.md` (NEEDS-FOLD, QA-uphold); the v1.1 contract is unchanged.
Sections above (row inventory, stage table, pre-fold measured splits) record the pre-fold suite and
remain historically accurate.

### What changed in the suite

1. **PT-4(e) prefix mismatch fixed (blocking, finding 1).** The minted-code comparison is now
   prefix-corrected (`actualCodes` bare vs `headCliCodes.some(hc => hc === \`cli_${c}\`)`), so at HEAD
   `minted = []` and the row is green-capable. PT-4 stays RED at HEAD for the right reasons (a)–(d).
2. **ALIAS_FIRST_TOKENS drift resolved (blocking, finding 2).** `extractAliasFirstTokens(sem,
   lifecycle)` now excludes lifecycle verbs → derives the contract D1 set `{view, list, member}`
   (verified: `do`/`resume`/`retry` are lifecycle verbs). Contract governs the conflict.
3. **Member-guard placement (finding 5) hardened + recorded.** `extractRunBranchFacadeLabels(cli,
   lifecycle, aliases)` excludes alias first-tokens from the facade labels, so the rule-2 `member`
   guard cannot inflate the facade set; the `:1578`-after-`lifecycleActions` placement is documented.
4. **Composition-form requirement (finding 6) recorded** on `extractLifecycleVerbs` —
   `RUN_RECOGNIZED_FIRST_TOKENS` must be spread-composed, never a 39-string enumerated literal.
5. **PT-2a/PT-2b SHALLOW accepted** (kept as headline-example namers; PT-2c is the enforcement).
   PT-2c's vacuous zero-match arm accepted (PT-3 carries the zero-match law).

### Post-fold split (split-twice, from the repo root)

```
node --test impl/test/cli-silent-start-red.test.mjs
```

- **Run A — 12 tests, 7 pass / 5 fail** (7 PIN rows green; PT-2a/PT-2b/PT-2c/PT-4/PT-5 red)
- **Run B — 12 tests, 7 pass / 5 fail** (stable — identical split)

PT-4 failure message post-fold (verified): 4 failures, all RIGHT-reason stages —
`facade-nouns-symbol-absent` · `alias-first-tokens-symbol-absent` · `derivation-symbol-absent` ·
`guard-replaces-naked-fallthrough`. The `new-cli-code-minted` wrong-reason failure is gone. Sweep
composition unchanged: exactly-one **2711**, zero **0**, two-or-more **4**, skipped-exact **3**.

### Satisfiability proof (fold outcome)

Simulated a correct implementation (FACADE_NOUNS declared, ALIAS_FIRST_TOKENS = {view,list,member},
spread-composed RUN_RECOGNIZED_FIRST_TOKENS, naked fall-through replaced, no new `cli_*` code):
**PT-4 passes all five sub-assertions (a)–(e)** — the permanently-red row is now green-capable.
Wrong-impl bite preserved: a 6-token `ALIAS_FIRST_TOKENS` (the old raw derivation) fails PT-4(b);
the contract-faithful 3-token set passes.
