...
| `swarm.notifications` | `ordinary` | `baton swarm notifications` | `baton swarm notifications SWARM_ID --receipt RECEIPT_ID` |
| `swarm.notify` | `ordinary` | `baton swarm notify` | `baton swarm notify SWARM_ID sibling "Reuse my published contract"` |
...
The recruit's context leg (#441, #480): `baton swarm recruit … --issue N [--doc PATH …]` pulls the GitHub issue through THIS host's own `gh` credential and admits ONE ContextPackage (branch `issue:N`, plus one branch per repository doc the issue cites and per `--doc`) which the seat's run carries (scope `worker:<seat>`); the seat's brief renders the package digest, each branch and the head of its text, and the recruit's receipt names every document it could not pull. Every cited and named doc resolves against the DEPLOYMENT's checkout root (`repository.root`), never the CLI's cwd. A worker never calls `gh`. A named document that checkout does not carry is a NAMED GAP (`docs: [{path, state: 'unreadable', reason}]` on the receipt and in the seat's brief) and the recruit is admitted. The reader refuses typed before any effect: `issue_reader_unavailable`, `issue_not_found`, `context_doc_unreadable` (a path outside the checkout, or a leg that composed no readable member at all), `context_source_oversize`.
