...
// renders beyond the run.start objective cap — waves.run returns a receipt and the wave starts,
// then EVERY member phantom-fails AT START (terminalCause:'start' with the machinery's typed
// refusal: the embedded client's nonempty byte check, application-client.mjs:11-13, throws
// application_client_invalid "Run objective is required"; the registry-declared run.objective
// refusal code is spill_body_exceeded). The seam failure is a fleet-wide per-member phantom —
// admission must fail LOUD instead.
//
// Contract (row brief, closed): an objectiveRef brief whose rendered objective exceeds the
// run.objective cap (4096 bytes, limits.mjs) refuses at compile/admission with a typed
// workflow_spec_invalid-class refusal naming BOTH byte counts (the measured rendered bytes and
// the cap). The 64KiB D5 read envelope (OBJECTIVE_REF_MAX_BYTES) stays the read/containment
// bound — the interpreter does NOT split; the run cap is the admission bound (judgment call,
// wave-e notes). No cap change to run.objective itself.
//
// Suite law: hermetic (mkdtemp fixture, MockAdapter, real createDriver stack, bus-level
// application.command('waves.run', ...) — the shipped surface) · red-first: AA-1/AA-2b/AA-3b
// are RED at the pre-change head (return a settle receipt carrying the phantom failure instead
// of refusing) · AA-2a/AA-3a are green guards pinning the machinery boundary (createWave
// admits an exactly-at-cap rendered objective) and the unchanged D5 envelope · the salt line
// the interpreter prepends (`[attempt: <uuid> <role>] `, workflow-interpreter.mjs renderObjective)
// is a fixed 36-char UUID, so the expected rendered byte count is deterministic:
// briefBytes + 49 + role.length.
...
// AA-1 (RED stage[admission-refusal-missing]): an 8 KiB brief (inside the 64 KiB D5 envelope,
// above the 4096 run cap) refuses AT ADMISSION with workflow_spec_invalid naming both byte
// counts — never a settle receipt whose member phantom-fails at start (the pre-change head
// admits, then every member dies terminalCause:'start').
test('AA-1 (stage[admission-refusal-missing]): an 8 KiB objectiveRef brief refuses at admission naming both byte counts — workflow_spec_invalid, never a per-member phantom start failure', async (t) => {
  const briefBytes = 8 * 1024;
  const fx = await fixture(t, 'aa1', { briefBytes });
  ...
  assert.ok(result.error, 'stage[admission-refusal-missing]: the over-cap brief must REJECT at admission — the pre-change head returns a settle receipt instead (RED)');
