...
test('RG-02 RED: application tools/list is the served ordinary table and includes every pre-spread-uncovered sibling (stage: application-tools-count-49)', async () => {
  ...
  assert.equal(names.length, 58, 'application tools/list count 58 (stage: application-tools-count-49)');
  ...
  assert.equal(names.length, 36 + uncovered.length + 3 + 2 + 2 + 1,
    'application tools/list count ties to composition: 36 target base + 14 lifecycle + 3 wakes + 2 message + 2 harvest pair + 1 attention wake = 58 (stage: application-count-composition)');
...
test('RG-09 RED: combined tools/list is the served combined table with the 14 siblings leading the ordinary prefix (stage: combined-102-includes-siblings)', async () => {
  ...
  await initialized(server);
  const names = (await request(server, 2, 'tools/list', {})).result.tools.map((tool) => tool.name);
  assert.equal(names.length, 111, 'combined tools/list count 111 (stage: combined-102-includes-siblings)');
  ...
  assert.equal(typeof mcpNorthbound.uncoveredCommands, 'function',
    'uncoveredCommands export exists (stage: uncovered-set-export)');
  const uncovered = mcpNorthbound.uncoveredCommands();
  assert.equal(names.length, 87 + 2 + uncovered.length + 3 + 2 + 2 + 1,
    'combined count ties to composition: 87 target base + 2 fleet + 14 siblings + 3 wakes + 2 message + 2 pair + 1 attention wake = 111 (stage: combined-count-composition)');
