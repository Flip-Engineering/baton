...
//   RG-02  RED  application tools/list = 57 (36 + 14, composition fold) including every
...
//   RG-09  RED  combined tools/list = 150 (87 + 2 + 14 + 7 + 40), the 14 siblings
...
//   RG-11R RED  surface-inventory-artifact encodes mcp.application 57 / mcp.combined 150,
//               tied to composition (36 + 14 / 87 + 2 + 14 + 7 + 40, fold)                  (stage: artifact-counts-49-102)
...
test('RG-02 RED: application tools/list is the served ordinary table and includes every pre-spread-uncovered sibling (stage: application-tools-count-49)', async () => {
  ...
  assert.equal(names.length, 57, 'application tools/list count 57 (stage: application-tools-count-49)'); // composition (final landing): 36 base legacy (incl. the restored knowledge.seed row) + 14 lifecycle siblings + 3 wakes (issue #294 registry-mandated) + 2 message lane (issue #206) + 2 harvest pair (issue #99/#179) = 57
  ...
  // and the count ties to composition (36 HEAD ordinary tools + the 14 lifecycle siblings), so a
...
test('RG-09 RED: combined tools/list is the served combined table with the 14 siblings leading the ordinary prefix (stage: combined-102-includes-siblings)', async () => {
  ...
  assert.equal(names.length, 87 + 2 + uncovered.length + 3 + 2 + 2 + 40,
    'combined count ties to composition: 87 + 2 fleet + 14 siblings + 3 wakes + 2 message + 2 pair + 40 dot twins (stage: combined-count-composition)');
...
test('RG-10b RED: renderMcpToolInventory has the canonical-miss fallback byte-string in executable source (stage: renderer-fallback-absent)', () => {
  ...
  // Fold (blue-team #2 on RG-10b — SHALLOW): the byte-string must appear in COMMENT-STRIPPED source
  // so a comment-placed decoy cannot satisfy the anchor. The behavioral force stays RG-10c (SOUND).
  assert.ok(stripComments(source).includes(fallback),
...
test('RG-11-R RED: the surface-inventory artifact encodes the measured mcp.application / mcp.combined counts, tied to composition (stage: artifact-counts-49-102)', () => {
  ...
  assert.equal(artifact.counts.mcpCombinedTools, 150, 'artifact mcp.combined count 150 (stage: artifact-counts-49-102)');
  // Fold (blue-team #4 — SHALLOW): the committed counts also tie to composition (36 + 14 /
  // 87 + 2 + 14 + 7 + 40), so an arbitrary self-consistent pair of counts (artifact == live,
  ...
  assert.equal(artifact.counts.mcpCombinedTools, 87 + 2 + uncovered.length + 3 + 2 + 2 + 40,
    'artifact mcp.combined ties to composition: 87 + 2 + 14 + 3 wakes + 2 message + 2 pair + 40 dot twins (stage: artifact-combined-composition)');
});
