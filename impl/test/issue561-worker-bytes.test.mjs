...
// on an admitted lease. This file pins the measurement loop that feeds it — the mechanism for the
// named real run where a crash-restarted swarm drove the 16 GB host to swap exhaustion while the
// resident reported memoryTight:true holding 39 worker leases that weighed nothing:
//
...
//   lifecycle     the loop arms with the first admitted worker lease and never doubles
//
// Every pass is driven through deferred releases: the pass reads the table only when the test
// releases it, so no assertion races the landing loop's own microtasks.
...
/** A swarm with one recruited, running seat, its arming measurement already landed: the recruit
 * wired the seat's lease into the loop, one pass read the table, and the seat's bytes are on its
 * lease. `observed` holds that one landing; the next kick takes the next deferred. */
async function recruited(t) {
  ...
  assert.ok(f.runtime._workerMeasureTimer !== null, 'the admission armed the loop');
  const arming = f.runtime._measureWorkerBytes();
  assert.equal(releases.length, 1, 'the pass reads the table');
  releases[0]();
  await arming;
  await passSettled(f.runtime);
  assert.deepEqual(f.observed, [{ holder: HOLDER, nonce: 'n1', bytes: 5 * M }],
    'the seat’s measured bytes landed on its lease');
  return { ...f, releases };
}
...
test('#561: a table that cannot be read measures nothing and never throws', () => {
  const groups = hostProcessGroupBytes({ ps: () => { throw new Error('ps died'); } });
  assert.deepEqual([...groups], [], 'a missed sample is an empty answer, never a fabricated zero');
});
...
test('#561: a worker whose group reads nothing keeps its lease unmeasured', async (t) => {
  ...
  await f.call('create', { purpose: 'Measured worker weights (#561)' });
  await f.call('recruit', { participantId: 'builder', objective: 'Hold a measured lease' });
  const pass = f.runtime._measureWorkerBytes();
  assert.equal(releases.length, 1, 'the pass reads the table');
  releases[0]();
  await pass;
...
test('#561: a holder that left the live set has its token pruned by the same pass', async (t) => {
  ...
  f.workers.length = 0;
  const pass = f.runtime._measureWorkerBytes();
  assert.equal(f.releases.length, 2, 'the next pass reads the table again');
  f.releases[1]();
  await pass;
  ...
  assert.equal(f.observed.length, 1, 'a pruned holder lands nothing');
});
...
test('#561: concurrent measurements collapse onto one probe pass', async (t) => {
  const f = await recruited(t);
  const first = f.runtime._measureWorkerBytes();
  const joined = f.runtime._measureWorkerBytes();
  assert.equal(f.releases.length, 2, 'a pass in flight is the pass');
  f.releases[1]();
  await first;
  await joined;
  assert.equal(f.observed.length, 2, 'the joined callers share the one landing');
  const again = f.runtime._measureWorkerBytes();
  assert.equal(f.releases.length, 3, 'the guard reset when the pass ended, so the next caller measures again');
  f.releases[2]();
  await again;
  assert.equal(f.observed.length, 3, 'the fresh pass lands its own reading');
});

test('#561: the loop arms once with the first admitted lease and never doubles', async (t) => {
  const f = await recruited(t);
