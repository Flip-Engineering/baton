...
function initRepo(repo) {
  ...
  execFileSync('git', ['config', 'user.name', 'Issue 360 drain'], { cwd: repo });
  execFileSync('git', ['config', 'user.email', 'issue360@example.invalid'], { cwd: repo });
...
test('360a: the drain releases a settled stopped seat through the custody boundary and the shutdown exits zero', async (t) => {
  ...
  const retained = driver.coordination.eventsView().find((event) => event.kind === 'driver.recorded'
    && event.payload?.kind === 'worktree.custody_content_retained'
    && event.payload?.workspaceId === ownerId);
...
test('360c: the drain deadline names the reaper and since per waited resource', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.detail?.stage, 'convergence');
