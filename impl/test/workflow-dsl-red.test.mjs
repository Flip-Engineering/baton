...
//   Fold rows (RED at HEAD, blue-team §4):  answerDecisions behavioral (repeatable accumulation,
//                                 F1) · symlink-escape (harvest realpath containment, F2/B3) ·
//                                 compiler code-family scan (PIN-B twin, F6) · bare-harvest +
//                                 false-steering variants (F7)
//
...
test('P6 capability [surfaces-parity] — the compile seam lands on all four surfaces (CLI verb, bus transport, MCP tool, facade method)', () => {
  ...
  // Facade: baton.waves.compile is a new method on the waves accessor — scoped to the accessor
  // (NOT any `compile:` anywhere) and accepting both the property (`compile: …`) and method-shorthand
  // (`compile(text) {}`) spellings (blue-team finding 4).
  const client = readFileSync(CLIENT_PATH, 'utf8');
  assert.ok(/waves[\s\S]{0,400}compile/u.test(client), 'stage[surfaces-parity-facade] the embedded facade must expose a baton.waves.compile method scoped to the waves accessor (property or method-shorthand spelling)');
});
...
test('R1 capability [unknown-directive] — 2–3 distinct unknown directives each refuse workflow_spec_invalid with the same closed-list expected', async () => {
  const ns = await compiler();
  const dir = mkdtempSync(join(tmpdir(), 'baton-dsl-r1-'));
  try {
    // Three distinct names kill a single-token hardcode (`if (token === 'memberr') throw …`) — the
    // refusal must hold for ANY unknown token, each named in `field` with the SAME closed-list
    // `expected` (blue-team finding 5).
...
test('S5 capability [constants] — the compiler closed constants equal the interpreter byte-for-byte (inline) OR both import one shared closed-constants module', async () => {
  const ns = await compiler();
  const interpSrc = readFileSync(INTERPRETER_PATH, 'utf8');
  const compilerSrc = readFileSync(COMPILER_PATH, 'utf8');
  // Inline-declaration extractions (the interpreter's own form at HEAD). Whitespace-tolerant so the
  // shared-module form — which has NO inline `= …` literal — yields undefined and takes the else leg.
  const extractPattern = (src) => /IDEMPOTENCY_PATTERN\s*=\s*\/([^/]+)\//u.exec(src)?.[1];
  const extractCeiling = (src) => /MAX_MEMBERS\s*=\s*(\d+)/u.exec(src)?.[1];
  const extractEnum = (src, name) => new RegExp(`${name}\\s*=\\s*new Set\\(\\[([\\s\\S]*?)\\]\\)`, 'u').exec(src)?.[1];
  const interpPattern = extractPattern(interpSrc);
  const interpCeiling = extractCeiling(interpSrc);
  const interpMsg = extractEnum(interpSrc, 'MESSAGE_KINDS');
  const interpScratch = extractEnum(interpSrc, 'SCRATCHPAD_KINDS');
  assert.ok(interpPattern && interpCeiling && interpMsg && interpScratch,
    'stage[constants-interpreter-inline] the interpreter must declare the closed constants inline (its own ground truth)');
  const compilerPattern = extractPattern(compilerSrc);
  const compilerCeiling = extractCeiling(compilerSrc);
  const compilerMsg = extractEnum(compilerSrc, 'MESSAGE_KINDS');
  const compilerScratch = extractEnum(compilerSrc, 'SCRATCHPAD_KINDS');
  if (compilerPattern && compilerCeiling && compilerMsg && compilerScratch) {
    // Form 1 (inline): byte-compare every closed constant to the interpreter.
    ...
    assert.equal(compilerMsg.trim(), interpMsg.trim(), 'stage[constants-message-kinds] MESSAGE_KINDS must be byte-identical');
    assert.equal(compilerScratch.trim(), interpScratch.trim(), 'stage[constants-scratchpad-kinds] SCRATCHPAD_KINDS must be byte-identical');
  } else {
    // Form 2 (shared module — contract S5 "OR both modules import one shared closed-constants module"):
    // the compiler references the names via an import (no inline literal), and the interpreter must
    // import the SAME module so the values stay single-source (blue-team finding 3). The byte-equal
    // leg holds by construction — one shared module cannot desync.
    const namesPresent = ['IDEMPOTENCY_PATTERN', 'MAX_MEMBERS', 'MESSAGE_KINDS', 'SCRATCHPAD_KINDS']
      .every((name) => compilerSrc.includes(name));
    assert.ok(namesPresent, 'stage[constants-shared-module] the compiler must reference every closed constant (inline or via a shared import)');
    const sharedImport = /import\s*\{[^}]*IDEMPOTENCY_PATTERN[^}]*\}\s*from\s*['"]([^'"]+)['"]/u.exec(compilerSrc);
    assert.ok(sharedImport, 'stage[constants-shared-module] the shared-module form must import IDEMPOTENCY_PATTERN from a shared closed-constants module');
    assert.ok(interpSrc.includes(sharedImport[1]),
      `stage[constants-shared-module] the interpreter must import the SAME shared module (${sharedImport[1]}) so the constants stay single-source`);
  }
});

// ── Blue-team §4 fold rows (RED at HEAD — added by suite-fold-170) ─────────────

test('answerDecisions capability [answerDecisions] — the directive lowers to steering.answerDecisions.policy and REPEATS to accumulate (behavioral, not registry-only)', async () => {
  const ns = await compiler();
  const dir = mkdtempSync(join(tmpdir(), 'baton-dsl-ans-'));
  try {
    const text = [
    ...
    assert.deepEqual(ir.steering.answerDecisions.policy, { q1: 'opt1', q2: 'opt2' },
      'stage[answerDecisions-accumulate] repeatable answerDecisions directives accumulate into steering.answerDecisions.policy in directive order');
    assert.equal(ir.steering.answerDecisions.policy.q1, 'opt1',
      'stage[answerDecisions-q1] the first pattern lowers to its value');
    assert.equal(ir.steering.answerDecisions.policy.q2, 'opt2',
      'stage[answerDecisions-q2] the second repeat lowers to its value (never overwrites the first)');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('symlink-escape capability [symlink-escape] — a harvest path that is a symlink resolving outside repoRoot refuses workflow_harvest_invalid with repoRoot, and compiles without it (B3 gating)', async () => {
  const ns = await compiler();
  const repoRoot = mkdtempSync(join(tmpdir(), 'baton-dsl-esc-'));
  const outside = mkdtempSync(join(tmpdir(), 'baton-dsl-out-'));
  try {
    symlinkSync(outside, join(repoRoot, 'escape'));
    const text = 'wave ok-key\nharvest escape\n';
    // With repoRoot the realpath containment resolves the symlink outside the repo → refusal (B3).
    let caught;
    try {
      ns.compileWavefile(text, { repoRoot });
    } catch (error) {
      caught = error;
    }
    assert.ok(caught, 'stage[symlink-escape-refusal] expected a refusal for a harvest symlink escaping the repoRoot');
    assert.equal(caught?.code, 'workflow_harvest_invalid', 'stage[symlink-escape-refusal] wrong code');
    assert.equal(caught?.line, 2, 'stage[symlink-escape-refusal] the refusal line is the harvest directive');
    assert.equal(caught?.field, 'harvest.paths[0]', 'stage[symlink-escape-refusal] the field leg is the harvest entry index');
    // The contract §3 pins no distinct `expected` literal for the realpath escape (it names only the
    // lexical path-class escape); assert the triple SHAPE — a non-empty expected — without inventing a
    // contract string (the interpreter's own phrasing is "resolves outside the repository root").
    assert.ok(typeof caught?.expected === 'string' && caught.expected.length > 0,
      'stage[symlink-escape-refusal] the expected leg must be a non-empty string');
    assert.deepEqual(caught?.detail, { line: 2, field: 'harvest.paths[0]', expected: caught?.expected },
      'stage[symlink-escape-refusal] the detail wire leg must equal {line, field, expected}');
    // Without repoRoot the compiler runs a lexical-only pass — the same text compiles to { path }.
    const ir = ns.compileWavefile(text, {});
    assert.deepEqual(ir.harvest.paths, [{ path: 'escape' }],
      'stage[symlink-escape-ungated] omitting repoRoot skips the realpath containment, so the same text compiles clean');
  } finally {
...
test('compiler-code-family capability [compiler-code-family] — every workflow_* literal the compiler throws is within the closed 5-code family (the compiler-facing twin of PIN-B)', async () => {
  await compiler(); // the module must exist (named stage at HEAD)
  const src = readFileSync(COMPILER_PATH, 'utf8');
  const family = [
    'workflow_spec_invalid',
    'workflow_member_invalid',
  ...
  const literals = [...src.matchAll(/'workflow_[a-z0-9_]+'/gu)].map((m) => m[0].slice(1, -1));
  assert.ok(literals.length > 0, 'stage[compiler-code-family-present] the compiler must throw the closed workflow_* family (it cannot be silent)');
  for (const literal of literals) {
    assert.ok(family.includes(literal), `stage[compiler-code-family-${literal}] the compiler must not mint a 6th admission code: ${literal} is outside the closed 5-code family`);
  }
  // The compiler must mint each of the four admission codes the R-rows pin (objective_ref_invalid is
  // interpreter render-time — the compiler never emits it, but it stays in the closed family).
  for (const code of family.slice(0, 4)) {
    assert.ok(src.includes(`'${code}'`), `stage[compiler-code-family-${code}] the compiler must mint ${code}`);
  }
});

test('steering-harvest-variants capability [steering-harvest-variants] — the bare harvest <path> form (no mustContain) and the explicit false steering forms lower correctly', async () => {
  const ns = await compiler();
  const dir = mkdtempSync(join(tmpdir(), 'baton-dsl-var-'));
  try {
    const text = [
      'wave variants',
      'scope reports/**',
      ...
      'claimOnStall false',
      'harvest reports/a.md',
      'harvest reports/b.md mustContain "B"',
    ].join('\n');
    const ir = ns.compileWavefile(text, { repoRoot: dir });
    assert.equal(ir.steering.approveOnAdvertisedPlan, false,
      'stage[steering-harvest-variants-approve-false] approveOnAdvertisedPlan false lowers to false (not the bare true default)');
    assert.equal(ir.steering.claimOnStall, false,
      'stage[steering-harvest-variants-claim-false] claimOnStall false lowers to false');
    assert.deepEqual(ir.harvest.paths, [
      { path: 'reports/a.md' },
      { path: 'reports/b.md', mustContain: 'B' },
    ], 'stage[steering-harvest-variants-harvest] the bare harvest form emits { path } (no mustContain) and the mustContain form emits { path, mustContain }');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
