...
//
// ROW INVENTORY (every contract pin P1–P10 / R1–R10 / S1–S5 plus the suite-fold-170 rows
// P11–P13 / S6 is a row at its named stage):
...
//                                 P11 answerDecisions behavioral (repeatable accumulation) ·
//                                 P12 symlink-escape (B3 gating) · P13 false-steering + bare-harvest ·
//                                 R1–R9 refusal triples · R10 head-seam (waves run compiles) ·
//                                 S1 no-eval/no-fs · S2 no-driver/schemaVersion · S3 three-way
//                                 invariant · S4 closure · S5 constants (inline OR shared-module) ·
//                                 S6 compiler code-family scan · OQ6 registry seam
...
test('P6 capability [surfaces-parity] — the compile seam lands on all four surfaces (CLI verb, bus transport, MCP tool, facade method)', () => {
  ...
  // Facade: baton.waves.compile is a new method on the waves accessor. Scoped to the accessor so an
  // unrelated `compile:` elsewhere in the client file cannot satisfy it, and accepting BOTH spellings
  // (property `compile:` and method-shorthand `compile(...)`) so a correct impl is never rejected.
  const client = readFileSync(CLIENT_PATH, 'utf8');
  assert.ok(/waves[\s\S]{0,400}compile\s*(?:\(|:)/u.test(client), 'stage[surfaces-parity-facade] the embedded facade must expose a baton.waves.compile method on the waves accessor');
});
...
test('P11 capability [answerDecisions-behavioral] — answerDecisions "<pattern>" "<value>" lowers to steering.answerDecisions.policy[pattern]=value and repeats accumulate', async () => {
  const ns = await compiler();
  ...
  try {
    ...
    const ir = ns.compileWavefile(text, { repoRoot: dir });
    // The totality claim (P4/S3) is registry-level only; this row is the behavioral leg — the only
    // directive that had no compile-time coverage before this fold (blueteam-170.md §3 finding 1).
    const policy = ir.steering.answerDecisions.policy;
    assert.equal(policy.q1, 'opt1', 'stage[answerDecisions-first] answerDecisions "q1" "opt1" must set policy.q1 to "opt1"');
    assert.equal(policy.q2, 'defer', 'stage[answerDecisions-repeat] the second answerDecisions must accumulate policy.q2 — repeatable, never overwrite');
    assert.deepEqual(Object.keys(policy).sort(), ['q1', 'q2'], 'stage[answerDecisions-accumulate] both policy entries must be present (the repeatable-accumulation leg)');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('P12 capability [symlink-escape] — a harvest path that is a symlink resolving outside repoRoot refuses workflow_harvest_invalid with repoRoot and compiles without it (the B3 gating)', async () => {
  const ns = await compiler();
  const dir = mkdtempSync(join(tmpdir(), 'baton-dsl-p12-'));
  const outside = mkdtempSync(join(tmpdir(), 'baton-dsl-p12-outside-'));
  try {
    // A symlink INSIDE repoRoot whose target is a directory OUTSIDE repoRoot — the exact B3 escape.
    symlinkSync(outside, join(dir, 'escape-link'), 'dir');
    const text = 'wave symlink-escape\nharvest escape-link\n';
    let caught;
    try {
    ...
    assert.ok(caught, 'stage[symlink-escape-gated] a symlink-escape harvest must refuse when repoRoot is provided');
    assert.equal(caught?.code, 'workflow_harvest_invalid', 'stage[symlink-escape-gated] the symlink-escape refusal code is the closed workflow_harvest_invalid');
    assert.equal(caught?.line, 2, 'stage[symlink-escape-gated] the refusal line is the harvest directive');
    assert.equal(caught?.field, 'harvest.paths[0]', 'stage[symlink-escape-gated] the refusal field names the escaped harvest entry');
    // The exact expected SPELING for the realpath escape is left to the impl (the contract §3 pins
    // only the path-class expected; judgment call — the code/line/field/detail legs are the teeth).
    assert.ok(typeof caught?.expected === 'string' && caught.expected.length > 0, 'stage[symlink-escape-gated] the refusal carries a non-empty expected leg');
    assert.deepEqual(caught?.detail, { line: 2, field: 'harvest.paths[0]', expected: caught?.expected },
      'stage[symlink-escape-gated] the detail wire leg must equal {line, field, expected}');
    // The gating itself is pinned: WITHOUT repoRoot the containment check is skipped (a lexical-only
    // pass) and the same text compiles — a bare harvest path emits { path } with no mustContain.
    const uncontained = ns.compileWavefile(text);
    assert.deepEqual(uncontained.harvest, { paths: [{ path: 'escape-link' }] },
      'stage[symlink-escape-ungated] without repoRoot the symlinked path compiles as a bare { path } entry');
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('P13 capability [steering-false-bare-harvest] — the explicit false steering forms and the bare harvest (no mustContain) form lower correctly', async () => {
  const ns = await compiler();
  ...
  try {
    ...
    const ir = ns.compileWavefile(text, { repoRoot: dir });
    // The bare (omitted) steering form defaults to true (Appendix A exercises it); the EXPLICIT
    // false form was unprobed before this fold (blueteam-170.md §3 finding 7).
    assert.equal(ir.steering.approveOnAdvertisedPlan, false, 'stage[steering-false-approve] approveOnAdvertisedPlan false must lower to false, never the bare true default');
    assert.equal(ir.steering.claimOnStall, false, 'stage[steering-false-claim] claimOnStall false must lower to false');
    // Bare harvest <path> (no mustContain) emits { path } with no mustContain key.
    assert.deepEqual(ir.harvest, { paths: [{ path: 'reports/a.md' }] }, 'stage[bare-harvest] a bare harvest <path> must emit { path } with no mustContain key');
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test('R1 capability [unknown-directive] — 2–3 distinct unknown directives each refuse workflow_spec_invalid naming the token with the closed-list expected', async () => {
  const ns = await compiler();
  const dir = mkdtempSync(join(tmpdir(), 'baton-dsl-r1-'));
  try {
    // 2–3 distinct unknown names kill a one-token special case; `signalOnMembersDonee` also catches a
    // prefix-matching impl (blueteam-170.md §3 finding 5).
    for (const unknown of ['memberr', 'harnes', 'signalOnMembersDonee']) {
      assertRefusal(
        () => ns.compileWavefile(`wave ok-key\n${unknown} foo\n`, { repoRoot: dir }),
        { code: 'workflow_spec_invalid', line: 2, field: unknown, expected: '<closed directive list>' },
        `stage[unknown-directive-${unknown}]`,
      );
    }
  } finally {
    rmSync(dir, { recursive: true, force: true });
...
test('S5 capability [constants] — the compiler closed constants equal the interpreter constants byte-for-byte (inline form) OR are imported from one shared closed-constants module (shared form)', async () => {
  const ns = await compiler();
  const interpSrc = readFileSync(INTERPRETER_PATH, 'utf8');
  const compilerSrc = readFileSync(COMPILER_PATH, 'utf8');
  // The interpreter is the byte-unchanged authority and always declares its constants inline.
  const extractPattern = (src) => /IDEMPOTENCY_PATTERN = \/([^/]+)\//u.exec(src)?.[1];
  const interpPattern = extractPattern(interpSrc);
  assert.ok(interpPattern, 'stage[constants-pattern] the interpreter must declare IDEMPOTENCY_PATTERN inline (byte-unchanged authority)');
  // The COMPILER may declare the closed constants inline (byte-identical to the interpreter) OR
  // import them from one shared closed-constants module (the contract S5 "OR both modules import one
  // shared closed-constants module" form — a single source, so byte-drift is structurally impossible).
  // The blue-team probe (blueteam-170.md §3 finding 3) proved the inline-only regexes REJECT a
  // correct shared-module compiler; accept either form.
  const compilerInline = ['IDEMPOTENCY_PATTERN', 'MAX_MEMBERS', 'MESSAGE_KINDS', 'SCRATCHPAD_KINDS']
    .every((name) => new RegExp(`${name} = `, 'u').test(compilerSrc));
  if (compilerInline) {
    ...
    // Shared-module form: the compiler references (imports) the closed constants instead of
    // redeclaring them — there is no second source to desync against. Every closed constant name
    // must be imported/referenced.
    for (const name of ['IDEMPOTENCY_PATTERN', 'MAX_MEMBERS', 'MESSAGE_KINDS', 'SCRATCHPAD_KINDS']) {
      assert.ok(compilerSrc.includes(name), `stage[constants-shared-${name}] the shared-module compiler must reference ${name}`);
    }
  }
});

test('S6 capability [compiler-code-family] — every workflow_* code literal the compiler throws is within the closed 5-code family (the compiler-facing twin of PIN-B)', async () => {
  await compiler(); // module must exist (named stage at HEAD)
  const src = readFileSync(COMPILER_PATH, 'utf8');
  const family = [
    'workflow_spec_invalid',
  ...
  // PIN-B scans the INTERPRETER for a 6th admission code; this row scans the COMPILER — where a
  // compiler-minted 6th code would actually live (blueteam-170.md §3 finding 6). Every workflow_*
  // string literal the compiler throws must stay within the closed family (G5).
  const minted = [...src.matchAll(/['"](workflow_[a-z_]+)['"]/gu)].map((m) => m[1]);
  assert.ok(minted.length > 0, 'stage[compiler-code-family] the compiler must throw at least one workflow_* code');
  for (const code of new Set(minted)) {
    assert.ok(family.includes(code), `stage[compiler-code-family] the compiler must not mint ${code} — every thrown workflow_* code stays within the closed 5-code family`);
  }
});
