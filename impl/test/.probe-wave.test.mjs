...
test('W5: a bare-directory scope fails admission with the corrective glob form; glob scopes pass', async (t) => {
  ...
  const outcomes = await wave.settle({ timeoutMs: 20_000 });
  try {
    const memberRun = wave.runs && wave.runs.get ? wave.runs.get("alpha") : null;
    console.error("DEBUG outcome:", JSON.stringify(outcomes[0]));
    const insp = memberRun && typeof memberRun.inspect === "function"
      ? await memberRun.inspect({ depth: "section", section: "result" }) : null;
    console.error("DEBUG inspect:", JSON.stringify(insp).slice(0, 1500));
  } catch (error) { console.error("DEBUG failed:", error?.code ?? "", error?.message ?? String(error)); }
