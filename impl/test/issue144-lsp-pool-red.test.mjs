...
function gitRepo(label, files = {}) {
  ...
  Object.assign(process.env, { GIT_AUTHOR_EMAIL: 'lsp144@example.invalid', GIT_COMMITTER_EMAIL: 'lsp144@example.invalid' });
  Object.assign(process.env, { GIT_AUTHOR_NAME: 'LSP144', GIT_COMMITTER_NAME: 'LSP144' });
...
test('GP-G (pin): the supervised process-lifecycle machinery the pool inherits — bounded kill-wait, closed reap reasons, slot-clear latch (GT7, D1.3/D4.2, R3)', () => {
  ...
  // validating seam — G-30 added `attempts_exhausted` so an exhausted reap is never laundered into
