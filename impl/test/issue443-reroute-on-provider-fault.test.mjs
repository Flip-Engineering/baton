...
// The 2026-09-24 shape the operator ruled on: a codex route whose measured quality would rank
...
test('443-a2: the proposal wakes the new closed wake class, and a declared manual posture carries no roll-forward', async (t) => {
  ...
  // successor is spawned by the runtime.
  const view = await w.call('view', { swarmId: SWARM });
...
test('443-a3: with no declared policy the default is auto — a fault death performs the resume itself (#574)', async (t) => {
  ...
  // `rerouteOnProviderFault: auto`, so the death the observation folds is answered by the
  // runtime's own recruit without an orchestrator in the loop.
  const w = await faultedSwarm(t, {
  ...
  assert.equal(w.starts.some((row) => row.runId === successor.runId), true,
    'the auto continuation started the successor without a question');
...
test('443-a4: the operator routing rule excludes a harness — a fault never reroutes onto it (#574)', async (t) => {
  // The 2026-09-24 ruling: no seat may be routed onto codex, and the day's proposals ranked codex
  // FIRST. The declared exclusion removes the harness from the derivation before ranking, names
  // it in the excluded rows, and the performed resume lands on the best candidate the rule still
  // allows. The rule is declared in any spelling; the predicate compares case-insensitively.
  const w = await faultedSwarm(t, {
    rows: [faultedRouteRow(), codexRow(), openSubscriptionRow(), openApiRow()],
  ...
  assert.deepEqual(proposal.candidates.map((row) => label(row)),
    [label(OPEN_SUBSCRIPTION), label(OPEN_API)], 'the excluded harness is never a candidate');
  ...
  assert.equal(codexExcluded.reason, 'excluded_by_operator', 'the row names the operator rule');
  assert.equal(codexExcluded.billing, 'subscription', 'with the billing basis its profile carries');

  assert.equal(view.policy.rerouteOnProviderFault, 'auto', 'no policy declared: the default is auto');
  const rerouted = rowsOf(w.store, 'swarm.rerouted');
  ...
  // A recruit that names the excluded harness refuses typed, before any queue.
  await assert.rejects(
...
test('443-a5: the config-less serve declares the rule from the environment, validated like its sibling declarations', async () => {
  ...
  assert.throws(() => parseRoutingExcludeHarnesses(','), /names no harness between commas/u);
});
