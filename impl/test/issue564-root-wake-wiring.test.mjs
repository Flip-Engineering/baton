...
test('564-w4: a contribution-less turn_reported row crosses the resident attachment exactly once', async (t) => {
  ...
  assert.match(message.message.content,
    /✦\(◕‿◕\)✦ ▲ needs you — turn report to read in swarm-564/u);
  assert.match(message.message.content, /^  from: lead$/mu);
...
test('572-w4: a real parentless turn report reaches the resident root transport once', async (t) => {
  ...
  // Issue #572: the producer half (reportTurnEnd) belongs to another lane. This row is the house
  // forward hook — a guarded skip until it exists, measuring the delivery end to end the moment it
  // lands — so no checked-in expectation of a failing test is needed for it.
  if (typeof f.runtime.reportTurnEnd !== 'function') {
    t.skip('reportTurnEnd is the rigidity #572 half; this row exercises it the moment it lands');
    return;
  }
