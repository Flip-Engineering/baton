...
const root = (name) => mintFixtureDirectory(join(tmpdir(), `baton-proposal-${name}-`));
...
`); chmodSync(bin, 0o700);
  ...
  const raceRoot = root('lease-race'); writeFileSync(join(raceRoot, 'supervisor.lock'), `${JSON.stringify({ pid: 2_147_483_647, pidStart: 'stale', token: 'stale' })}\n`);
