...
test('461e: host.successor_started carries the spawn spelling and the log its narration rides', async (t) => {
  ...
  // …and the narration it writes is not discarded: the row names the log, and the bytes land there.
  assert.equal(started.payload.log, 'stderr', `the row names the log its narration rides: ${JSON.stringify(started.payload)}`);
...
test('461a: the old incarnation releases the successor handle once the handoff is done', async (t) => {
  ...
  // No earlier row's fixture child may still hold this process's loop (their after hooks end it).
  await until(() => processWraps() === 0, { timeoutMs: 15_000, label: 'a quiet process before this row' });
  const { deployment } = await resident(t, f, { onSpawn: spawnRealSuccessor(t, f) });
  await deployment.host();
  ...
  assert.equal(processWraps(), 1, 'while the handoff runs the old really holds the successor handle');
  ...
  await until(() => processWraps() === 0, { timeoutMs: 15_000, label: 'the successor process handle to be released' })
    .catch((error) => { throw new Error(`${error.message} (active resources: ${process.getActiveResourcesInfo().join(',')})`); });
  assert.equal(processWraps(), 0,
