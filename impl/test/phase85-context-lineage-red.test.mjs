...
function fixture(t) {
  ...
  const source = [
    { label: 'beta', rank: 2, ignored: 'first source item' },
    { label: 'alpha', rank: 1, ignored: 'second source item' },
    { label: 'gamma', rank: 3, ignored: 'third source item' },
  ];
  ...
  const bench = new StatelessContextBench({
    artifactRoot, sources: { [sourceRef]: source }, environmentDigest: environmentV1,
    policy: DEFAULT_CONTEXT_PROGRAM_POLICY,
  });
  return { artifactRoot, bench, manifest, source };
}
