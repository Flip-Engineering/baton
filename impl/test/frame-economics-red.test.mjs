...
test('C8 (OQ5): the wave driver downgrades its precheck to a spill-aware ADVISORY and passes the objective through', async () => {
  ...
  try {
    driver = createWaveDriver(fakeBaton, {
      preflight: false, settlement: 'none', pollIntervalMs: 5,
...
const F_EXEMPTIONS = Object.freeze([
  ...
  ['cli-adapters.mjs', /FRAME_LIMITS\['wire\.frame'\]\.value/u, 'cataloged: wire.frame registry import (#500)'],
  ['codex-appserver.mjs', /FRAME_LIMITS\['wire\.frame'\]\.value/u, 'cataloged: wire.frame registry import (#500)'],
  ['context-map.mjs', /MAX_TEXT_BYTES = 16 \* 1024/u, 'uncataloged: context-map text bound'],
  ['context-program-policy.mjs', /maxManifestBranches > 4_096|maxProgramNodes > 4_096|maxCellsPerSession > 16_384|maxManifestBranches: 4_096|maxProgramNodes: 4_096|maxCellsPerSession: 16_384/u, 'uncataloged: context-program policy counts and NORMALIZER_CEILING'],
  ['context-program.mjs', /MAX_TEXT_BYTES = 16 \* 1024/u, 'uncataloged: context-program text bound'],
  ['context-program.mjs', /\.\.\.NORMALIZER_CEILING/u, 'cataloged: context-program policy reads NORMALIZER_CEILING (#500)'],
  ...
  ['runtime-admission.mjs', /policy\.maxTargetBytes > 1024 \* 1024/u, 'uncataloged: scratch-oracle policy ceiling (the line moved here from coordinator.mjs in issue #259 slice 11)'],
  ...
  ['runtime-admission.mjs', /stringField\(opts\.(taskId|idempotencyKey), '(taskId|idempotencyKey)', 4_096\)/u, 'uncataloged: identity fields (the lines moved here from coordinator.mjs in issue #259 slice 11)'],
  ['runtime-admission.mjs', /Buffer\.byteLength\(requestId\) > 4_096/u, 'uncataloged: requestId id-class (the line moved here from coordinator.mjs in issue #259 slice 11)'],
  ...
  ['mcp-northbound.mjs', /scope: \{ type: 'array', minItems: 1, uniqueItems: true, items: \{ type: 'string', minLength: 1, maxLength: (4_096|4096) \} \}/u, 'uncataloged: scope item byte bound (the scope array carries no count ceiling — #598 F08)'],
  ...
  ['swarm-runtime.mjs', /const SELECTION_TEXT_BYTES = 4_096;/u, 'uncataloged: recruit selection text and scope path id-class validator (#474); every rule interpolates this one declaration'],
