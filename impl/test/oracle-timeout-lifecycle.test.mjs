...
test('_json deadline timer fires and settles the promise before process exit', async () => {
...
test('scan wall deadline timer fires and settles the promise before process exit', async () => {
  const childPath = writeChild([
    `import { PublicSupplyChainOracle } from '${oracleUrl}';`,
    `import { mkdtempSync } from 'node:fs';`,
    `import { tmpdir } from 'node:os';`,
...
test('pre-aborted caller signal yields cancelled before process exit', async () => {
  const childPath = writeChild([
    `import { PublicSupplyChainOracle } from '${oracleUrl}';`,
    `import { mkdtempSync } from 'node:fs';`,
    `import { tmpdir } from 'node:os';`,
...
test('successful vet clears its deadline timer so the process exits promptly', async () => {
...
test('successful scan clears its wall timer so the process exits promptly', async () => {
