...
import {
  contextEffectCallIdentity, contextMapCallToEffectCall, normalizeContextEffectCall,
...
function authority(overrides = {}) {
  return {
    ...
    requester: {
      ...
      authorizationDigest: sha('a'), commandDigest: sha('b'),
    },
...
test('CC85-2: reduce is one unit selecting every completed-call output in canonical order', () => {
  ...
  const source = {
    ...
    evidenceRef: artifact(
      'context_evidence', 'application/vnd.baton.context-cell-evidence+json', '8',
    ),
  ...
  const reduce = contextEffectCallIdentity(mapRequest({
    ...
    units: [unit(0, {
      ...
      coordinateDigest: sha('f'), lineageDigest: sha('1'),
    })],
  }));
...
test('CC85-4: the Phase 84 map adapter projects into the generic envelope without identity loss', () => {
  const map = contextMapCallIdentity({
