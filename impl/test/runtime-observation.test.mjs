...
const REROUTE_TOTALS = Object.freeze({ logAppend: 36, mapEvent: 16, coordRecord: 6, coordination: 156 });
...
test('RO2: every observation_port delegate keeps the member name, parameter list, and hands over the recorder', () => {
  ...
  assert.equal(delegated.length, 147, `expected the 147 delegates in the map, found ${delegated.length}`);
...
test('RO5: the map sees the move', () => {
  ...
  assert.equal(target.members.length, 159,
    'the module target carries the 150 bodies plus 9 relocated helper functions (slice 12 moved noop and closedVerificationVerdict to the runtime-recovery base layer; they stay on this module\'s surface as re-exports)');
  const coordinatorFile = map.files.find((file) => file.file === COORD_FILE);
