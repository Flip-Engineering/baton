...
test('RO2: every observation_port delegate keeps the member name, parameter list, and hands over the recorder', () => {
  ...
  assert.equal(delegated.length, 138, `expected the 138 moved delegates in the map, found ${delegated.length}`);
...
test('RO5: the map sees the move', () => {
  ...
  assert.equal(target.members.length, 147,
    'the module target carries the 138 bodies plus 9 relocated helper functions (slice 12 moved noop and closedVerificationVerdict to the runtime-recovery base layer; they stay on this module\'s surface as re-exports)');
  const coordinatorFile = map.files.find((file) => file.file === COORD_FILE);
