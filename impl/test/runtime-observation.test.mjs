...
//      151 members (143 bodies + 8 relocated helper functions; slice 12 moved noop and
...
test('RO5: the map sees the move', () => {
  ...
  assert.equal(target.members.length, 157,
    'the module target carries the 149 bodies plus 8 relocated helper functions (slice 12 moved noop and closedVerificationVerdict to the runtime-recovery base layer; they stay on this module\'s surface as re-exports)');
  const coordinatorFile = map.files.find((file) => file.file === COORD_FILE);
