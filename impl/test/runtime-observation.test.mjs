...
// observation bucket moved into (impl/src/runtime-observation.mjs — 143 members: the read
...
//      (36 log appends, 16 evidence maps, 6 driver records, 147 coordination calls across the
...
//      152 members (143 bodies + 9 relocated helper functions; slice 12 moved noop and
...
const REROUTE_TOTALS = Object.freeze({ logAppend: 36, mapEvent: 16, coordRecord: 6, coordination: 155 });
...
test('RO2: every observation_port delegate keeps the member name, parameter list, and hands over the recorder', () => {
  ...
  assert.equal(delegated.length, 141, `expected the 141 moved delegates in the map, found ${delegated.length}`);
...
test('RO5: the map sees the move', () => {
  ...
  assert.equal(target.members.length, 153,
    'the module target carries the 141 bodies plus 12 relocated helper functions (slice 12 moved noop and closedVerificationVerdict to the runtime-recovery base layer; they stay on this module\'s surface as re-exports)');
  const coordinatorFile = map.files.find((file) => file.file === COORD_FILE);
