...
async function fixture({ id = 'task-a', accept = true, terminal = accept ? 'completed' : 'failed', scope = 'task', addOutcome = true, policy = {} } = {}) {
  ...
  const recall = await cairn.invoke('causal.recall', recallArgs, context({ idempotencyKey: `recall:${id}` }));
  ...
  return { directory, operational, store, cairn, created, claimed, finding, recall, recallArgs, mapped, terminalEvent };
}
...
test('RA1-RA8: verified pass and failure produce exact non-causal historical assessments and honest audit metrics', async () => {
  for (const [id, accept, expected] of [['pass', true, 'verified_pass_after_recall'], ['fail', false, 'verified_fail_after_recall']]) {
    ...
    for (const forbidden of ['SECRET', '/Users/alice', 'summary', 'snippet', 'brief', 'helped', 'harmed']) assert.equal(JSON.stringify({ event, result }).includes(forbidden), false, forbidden);
...
test('RA2/RA3: Baton selects only task-scoped receipt-before-verification-before-terminal associations', async () => {
  for (const [name, options] of [['actor', { scope: 'actor' }], ['run', { scope: 'run' }], ['asserted', { addOutcome: false }], ['cancelled', { accept: false, terminal: 'cancelled' }]]) {
  ...
  const after = await fixture({ id: 'after-terminal', addOutcome: false });
...
test('RA5/RA6/RA9: every independent ceiling, cancellation, preflight, and append failure leaves no assessment', async () => {
  for (const [field, value] of [['maxScanEvents', 1], ['maxReceipts', 0], ['maxNodeRefs', 0], ['maxEvidenceRefs', 1], ['maxBatchBytes', 1], ['maxResultBytes', 1]]) {
    const overrides = { [field]: value }; if (value === 0) overrides[field] = 1;
...
test('RA6: ACI output refusal occurs before the assessment append', async () => {
  ...
  assert.equal(f.store.snapshot().lastSeq, before); assert.deepEqual(f.store.snapshot().knowledge.assessments, []); driver.close();
});
