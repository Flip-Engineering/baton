...
//
// REPL-1's own `repl.manifest_admitted` admission machinery may not be landed in this tree —
// this suite exercises it only through the REPL1-INTEGRATION seam defined in
// coordination-store.mjs (`admitReplManifest`/`_resolveReplManifestBranch`), which folds a
// minimal `{runId, replRole, principal}` authority record into `_replManifestAdmissions` and
// resolves `cell:`-typed branches at admission (Part F). REPL-2's `repl.binding_set`/
// `repl.binding_dropped` are exercised against the real contract shapes (Part A-E, G).
...
/** Admits a minimal ReplManifest (REPL1-INTEGRATION seam) citing one `cell:` branch so REPL-2
 * bindings have a real manifestDigest authority record to cite. `replRunId` is the REPL-owned
 * runId dimension (Part A rule 1) — independent of the fixture's Context Program runId, since
 * this stand-in seam never cross-checks it against the Context session/run. */
function admitManifest(f, { replRole, principalId, cellId, branchName = 'x', replRunId = runId, key }) {
  return f.store.admitReplManifest({
    runId: replRunId, replRole, branches: [cellBranch(branchName, cellId)],
  }, replAuth(principalId, key ?? `repl.manifest:${replRunId}:${replRole}:${principalId}:${cellId}`));
}
...
test('C1: a cell: branch naming a completed cell resolves at admission and bakes a ctx:sha256: coordinate — never the art:sha256: handle verbatim', (t) => {
  ...
  const admitted = f.store.admitReplManifest({
    runId, replRole: 'shared', branches: [cellBranch('resolved', settled.cellId)],
  }, replAuth('orchestrator', 'c1:admit'));
...
test('C2: a cell: branch naming an admitted-only or failed/attention/stopped cell is repl_manifest_cell_not_settled and the admission event is never appended', (t) => {
  ...
  const before = f.store.events().length;
  assert.throws(() => f.store.admitReplManifest({
    runId, replRole: 'shared', branches: [cellBranch('x', pending.cellId)],
  }, replAuth('orchestrator', 'c2:pending')), (error) => error?.code === 'repl_manifest_cell_not_settled');
  ...
  assert.throws(() => f.store.admitReplManifest({
    runId, replRole: 'shared', branches: [cellBranch('x', failed.cellId)],
  }, replAuth('orchestrator', 'c2:failed')), (error) => error?.code === 'repl_manifest_cell_not_settled');
...
test('C3: a cell: branch whose settled artifact fails reverification at the moment of admission is context_artifact_unavailable and the admission event is never appended', (t) => {
  ...
  const before = f.store.events().length;
  assert.throws(() => f.store.admitReplManifest({
    runId, replRole: 'shared', branches: [cellBranch('x', settled.cellId)],
  }, replAuth('orchestrator', 'c3:admit')), (error) => error?.code === 'context_artifact_unavailable');
...
test('C4: replay of an admitted ReplManifest reconstructs the identical resolved branch with zero store lookups', (t) => {
  ...
  const admitted = f.store.admitReplManifest({
    runId, replRole: 'shared', branches: [cellBranch('resolved', settled.cellId)],
  }, replAuth('orchestrator', 'c4:admit'));
  ...
  assert.deepEqual(reopened.replManifestAdmission(admitted.manifestDigest), {
    runId, replRole: 'shared', principal: { actor: 'direct:orchestrator', principalId: 'orchestrator' },
    admittedEvent: replayedEvent.seq, admittedAt: replayedEvent.ts,
  });
});
...
test('C6: a Program source op against the resolved branch reads through the ordinary _readSource path and recovers byte-identical content; a later-lost artifact settles a downstream reading cell to attention (retryable), never failed', (t) => {
  ...
  const admitted = f.store.admitReplManifest({
    runId, replRole: 'shared', branches: [cellBranch('resolved', settled.cellId)],
  }, replAuth('orchestrator', 'c6:admit'));
...
// New event kinds introduced by this suite (for the kind-inventory fold):
//   - repl.manifest_admitted   (REPL1-INTEGRATION minimal stand-in; repl1-decisions.md's own
//     kind, folded here only because REPL-1's admission machinery may not yet be landed)
