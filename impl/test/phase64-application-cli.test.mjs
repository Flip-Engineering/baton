  ...
  const responses = [
    {
      ...
      content: { kind: 'baton.run_timeline.page', channel: 'events', cursor: 'page-a', hasMore: true, items: [{ position: 1 }] },
    },
    {
      runId: 'run-a', depth: 'content', cursor: 10, terminal: false,
      content: { kind: 'baton.run_timeline.page', channel: 'events', cursor: 'page-b', hasMore: false, items: [{ position: 2 }] },
    },
    {
      runId: 'run-a', depth: 'content', cursor: 14, terminal: true,
      content: { kind: 'baton.run_timeline.page', channel: 'events', cursor: 'page-c', hasMore: false, items: [{ position: 3 }] },
    },
  ];
  const client = { command: async (name, args, key) => {
    calls.push({ name, args, key });
