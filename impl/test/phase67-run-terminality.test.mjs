...
const settledPhases = ['work_completed', 'completed', 'failed', 'cancelled', 'denied', 'stopped', 'closed'];
const runTerminalPhases = ['completed', 'failed', 'cancelled', 'denied', 'stopped', 'closed'];
const providerUnavailablePhases = ['work_completed', 'completed', 'failed', 'denied', 'closed'];
...
test('ordinary inspect and run.follow remain change-aware at work_completed', () => {
  ...
  const current = { goal: { runId: 'run-1' }, profile: { followPolicy: { maxWaitMs: 250 } } };
