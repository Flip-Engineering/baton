...
test('585-1: a review_owed wake names the swarm, the seat, the contribution and the check command', async () => {
  const body = await rootWakeBody((store) => {
    store.recordDriver('swarm.contribution_recorded', {
      swarmId: 'swarm-40e6', contributionId: 'contribution-ada-1',
      body: { subject: 'The ledger fold reads its checkpoint once' },
    }, { actor: 'ada', key: '585-contribution' });
    return owed(store, {
...
test('585-2: a needs_root wake carries the ask and the view command', async () => {
  ...
  assert.equal(body, [
    `${SMILE} ▲ needs you — answer owed in swarm-564`,
    '  from: lead · contribution-564',
...
test('585-4: every value is shown whole; nothing is shortened', async () => {
