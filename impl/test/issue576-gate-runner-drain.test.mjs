...
test('576-d: a gate run still queued for the host verify lease aborts and withdraws its queue row', needsGit, async (t) => {
  ...
  // The landing promise carries a handler from birth: whatever it later answers, the test never
  // leaves it unhandled — the stop below settles it through assert.rejects.
  const landing = w.integration({ idempotencyKey: 'i576:integrate-queued' });
  landing.catch(() => {});
  // The gate's lease request can only be queued once the landing reaches its gate step, so the
  // wait rides the landing's own progress — the attempt opens first, then the queue row appears.
  const opened = await observe(() => (w.startedRows().length === 1 ? true : null), 30_000);
  assert.ok(opened, 'the landing opened its attempt');
  const queued = await observe(() => w.hostCapacity.observeNow().queue
    .find((entry) => entry.kind === 'verify' && entry.holder === 'integrate:s1:contribution:1') ?? null, 30_000);
  ...
  assert.equal(w.hostCapacity.observeNow().queue
    .some((entry) => entry.kind === 'verify' && entry.holder === 'integrate:s1:contribution:1'), false,
