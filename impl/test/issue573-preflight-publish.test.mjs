...
async function world(t, { mode }) {
  ...
  if (mode === 'unreachable') {
    ...
    // Unref'd, and idle connections force-closed on teardown: a git client that walked away must
    // never hold the test runner open past its last row.
    httpServer.unref();
    publishRemote = `http://127.0.0.1:${httpServer.address().port}/baton-issue573.git`;
    t.after(() => {
