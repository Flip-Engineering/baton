...
const principal = (overrides = {}) => ({
  userId: 'operator-a', sessionId: 'stdio-a', capabilities: ['control', 'observe', 'approve', 'emergency_stop', 'adopt_result'],
...
function setup(overrides = {}) {
  ...
  const server = new McpFleetServer({
    coordinator, coordination, application: overrides.application, surface: overrides.surface,
...
test('UA5/MN1: an application-backed MCP server defaults to the closed nine-tool Run surface', async () => {
  const application = {
  ...
  assert.deepEqual(response.result.tools.map((tool) => tool.name), [
    'fleet_run_start', 'fleet_run_status', 'fleet_run_approve', 'fleet_run_wait', 'fleet_run_answer', 'fleet_run_steer', 'fleet_run_stop',
    'fleet_run_evidence', 'fleet_run_adopt',
  ]);
  assert.equal(response.result.tools.find((tool) => tool.name === 'fleet_run_wait').inputSchema.properties.timeoutMs.maximum, 25_000);
  ...
  assert.equal(combined.result.tools.length, 28);
...
test('UA5/MN: Run tools map exactly to the application bus and keep status/wait fresh', async () => {
  ...
  assert.deepEqual(applicationCalls.map((call) => call.principal), Array(9).fill({
  ...
  assert.deepEqual(coordination.events().filter((event) => event.kind === 'mcp.call_admitted')
    .map((event) => [event.payload.tool, event.payload.runId]), [
      ['fleet_run_start', 'run-mcp-a'], ['fleet_run_approve', 'run-mcp-a'], ['fleet_run_answer', 'run-mcp-a'], ['fleet_run_steer', 'run-mcp-a'], ['fleet_run_stop', 'run-mcp-a'], ['fleet_run_adopt', 'run-mcp-a'],
    ]);
