...
test('WAVE-OBSERVER: an answered status at the elapsed deadline admits no new drive', async (t) => {
  t.mock.timers.enable({ apis: ['Date'], now: 0 });
  const calls = [];
  const wave = await createWave(facadeFor(calls, {
    alpha: { status: async () => ({ view: { get phase() { t.mock.timers.tick(50); return 'working'; } } }) },
  }), { members: [member('alpha')] });
  const outcomes = await wave.settle({ timeoutMs: 50 });
  assert.equal(outcomes[0].phase, 'working');
  assert.equal(calls.includes('pump:alpha'), false, 'a successful but now-expired read grants no new drive');
  assert.equal(wave.pumpQuiescent, true);
});

test('WAVE-OBSERVER: caller cancellation also detaches pending pump confirmation', async () => {
  const calls = [];
  ...
  const wave = await createWave(facadeFor(calls, {
    alpha: {
      status: async () => ({ view: { phase: ++reads === 1 ? 'working' : 'completed' } }),
      complete: (signal) => { signal.addEventListener('abort', () => retired.resolve(), { once: true }); return pump.promise; },
    },
  }), { members: [member('alpha')] });
  const controller = new AbortController();
  const pending = wave.settle({ timeoutMs: 5_000, signal: controller.signal });
  ...
  controller.abort();
  await assert.rejects(pending, (error) => error.code === 'wave_observer_cancelled');
  assert.equal(wave.pumpQuiescent, false, 'detaching does not pretend the signal-ignoring drive ended');
  assert.equal(calls.some((call) => call.startsWith('stop:')), false);
  pump.resolve();
  await flush();
  assert.equal(wave.pumpQuiescent, true);
