...
test('BR1/BR3/BR4/BR6/BR8: deterministic bounded pull recall appends one compact receipt before content', async () => {
  ...
  const preflight = store.previewKnowledgeRecallBounded({ text: 'retry', limit: 2, observedSeq, types: ['Finding', 'Decision'], reader: { taskId: 'source' } }, recallPolicy(), { actor: 'operator:alice', key: 'preflight' });
  assert.equal(Object.hasOwn(preflight, 'projection'), false); assert.equal(JSON.stringify(preflight).includes('retry is idempotent'), false); assert.equal(store.snapshot().lastSeq, observedSeq, 'size preflight exposes no content and appends nothing');
