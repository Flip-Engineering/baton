...
test('SP1/SP8/SP9: direct, authenticated web, and MCP routes share exact promotion and reverify authority', async () => {
  ...
  await assert.rejects(driver.coordinator.invokeCapability('cairn', 'causal.promote', args, ctx({ actor: 'orchestrator', transport: 'web', idempotencyKey: 'orchestrator-web-smuggle' })), (error) => error.code === 'capability_transport_forbidden');
  ...
  await assert.rejects(driver.coordinator.invokeCapability('cairn', 'causal.promote', args, ctx({ actor: 'web:forged:session', transport: 'mcp', idempotencyKey: 'mismatched-transport' })), (error) => error.code === 'capability_transport_forbidden');
