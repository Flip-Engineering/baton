...
test('385-a: a resume-from successor binds to the predecessor\'s same workspace when no live worker holds it', async (t) => {
  ...
  // DIAG
  console.log('DIAG after second incarnation startup: alphaWorktree exists =', existsSync(alphaWorktree));
  {
    const kinds = second.driver.coordination.eventsView().map((e) => e.kind === 'driver.recorded' ? e.payload?.kind : e.kind);
    console.log('DIAG event kinds:', JSON.stringify(kinds));
  }
