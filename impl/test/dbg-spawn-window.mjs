...
function root(label) {
  const dir = mkdtempSync(join(tmpdir(), `baton-dbg-${label}-`));
...
const rawEmit = adapter._emit.bind(adapter);
adapter._emit = (session, kind, payload) => { rawEvents.push(kind); return rawEmit(session, kind, payload); };

const driver = createDriver({
  repoRoot: repo, repoId, logDir, adapters: { mock: adapter }, stopDeadlineMs: 2000,
  goalPlanAuthority: {
...
const application = new BatonApplication({
  ...
  profiles: {
    default: Object.freeze({
      schemaVersion: 1, repoId, definitionOfDone: ['deployment verification passes'], constraints: [], risk: 'low',
      goalBudget: { tokens: 200000, usd: 20, wallMin: 120, providerTurns: 64 },
      nodeBudget: { tokens: 50000, usd: 5, wallMin: 30, providerTurns: 16 }, pathScope: ['**'],
...
const wave = await createWave(baton, {
  repoRoot: repo,
  members: [{ role: 'alpha', objective: 'write the alpha report (marker:alpha)', harness: 'mock', model: 'mock-model', effort: 'low', scope: ['reports/**'], report: 'reports/alpha.md' }],
});
let t0 = Date.now();
console.log('createWave ms', Date.now() - t0); t0 = Date.now();
const outcomes = await wave.settle({ timeoutMs: 20000 });
console.log('settle ms', Date.now() - t0); t0 = Date.now();
console.log('OUTCOMES', JSON.stringify(outcomes, null, 1));
console.log('close start');
console.log('RAW ADAPTER EVENTS', rawEvents.join(', '));
const workerLog = [];
for (const h of driver.coordinator.list()) workerLog.push(...driver.log.read(h.id));
workerLog.sort((a, b) => a.seq - b.seq);
console.log('WORKER LOG');
for (const e of workerLog) {
  console.log(`  ${e.seq} ${e.kind} actor=${e.actor} code=${e.payload?.code ?? ''} phase=${e.payload?.phase ?? ''} status=${e.payload?.status ?? ''} result=${e.payload?.result ?? ''} reason=${e.payload?.reason ?? ''}`);
}
const co = driver.coordinator;
console.log('WORKER STATE', JSON.stringify(co.list().map((w) => ({ id: w.id, status: w.status, taskId: w.taskId, processRef: w.processRef, sessionRef: w.sessionRef, ownedWorktreeAuthority: w.ownedWorktreeAuthority, worktree: w.worktree, cleanupPending: w.cleanupPending, localAuthority: w.localAuthority })), null, 1));
console.log('TASK STATE', JSON.stringify(driver.coordination.task(co.list()[0].taskId)));
const before = Date.now();
const closeResult = await wave.close({ reason: 'dbg' });
console.log('close ms', Date.now() - before, 'remainingCount', closeResult?.remainingCount, 'stops', closeResult?.stops?.length);
console.log('WORKER STATE AFTER CLOSE', JSON.stringify(co.list().map((w) => ({ id: w.id, status: w.status, processRef: w.processRef, cleanupPending: w.cleanupPending, localAuthority: w.localAuthority }))));
