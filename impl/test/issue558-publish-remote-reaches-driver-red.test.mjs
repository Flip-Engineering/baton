...
//
// Measured 2026-09-23 on master 65c913f0: every real swarm.integrate in this deployment refused
// integrate_publish_undeclared even with BATON_PUBLISH_REMOTE exported for the serving process.
// The declaration never reached the landing authority. createDriver read the option and validated
// it (impl/src/index.mjs:1249) and carried it into the coordinator options (:1647), but the object
// it RETURNS (:1882-1885) had no `integrationPublishRemote` member, while the swarm runtime's
// landing authority reads exactly that member (impl/src/application.mjs:2276-2282) and passes the
// resulting null into the landing (impl/src/worktree.mjs:2351-2356, which refuses a real landing
// before anything moves). The unwired option is silent the way the CDW1 route authority was:
// nothing throws, the landing refuses with a code that names the deployment rather than the wiring.
//
