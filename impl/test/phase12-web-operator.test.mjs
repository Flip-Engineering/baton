  ...
  const application = {
    ...
    card() {
      return {
        ...
        profiles: [{ name: 'standard', digest: 'a'.repeat(64), routes: [{ harness: 'grok', model: 'grok-4-code', effort: 'high' }], pathScope: ['impl/**'] }],
      };
    },
...
test('BU3/BU4/BU5/BU6: static client makes Run flow primary and keeps fenced reap in the advanced seat without unsafe sinks', async () => {
  ...
  for (const term of ['run_start', 'run_status', 'run_approve', 'run_wait', 'run_answer', 'run_steer', 'run_stop', 'steer-target', 'steer-mode', 'steer-reason', 'stop-form', 'stop-reason', '/v1/application-card', 'harness', 'model', 'effort', 'expectedFence', 'kill', 'drain', 'idempotencyKey', 'crypto.randomUUID', 'x-baton-csrf', '/v1/stream-tickets', 'EventSource', '/v1/auth/logout']) {
