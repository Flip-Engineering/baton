...
const root = (name) => mintFixtureDirectory(join(tmpdir(), `baton-phase64-${name}-`));
...
test('UA5: the shared command bus exposes the same run flow and a deployment-derived route card', async () => {
  ...
  // The card's command list derives from the command table + the M4b canonical names — the ONE
  // derivation (surface-truth.applicationCardCommands), not a retyped literal.
  assert.deepEqual(card.commands, applicationCardCommands());
