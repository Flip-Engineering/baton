...
test('UA5: the shared command bus exposes the same run flow and a deployment-derived route card', async () => {
  ...
  assert.deepEqual(card.commands, ['run.start', 'run.status', 'run.approve', 'run.wait', 'run.answer', 'run.steer', 'run.stop', 'run.evidence', 'run.adopt', 'application.shutdown']);
...
test('UA4-UA8: accepted result is pinned, evidenced, and explicitly adopted without changing the checkout', async () => {
  ...
  assert.equal(adopted.phase, 'completed');
  ...
  assert.equal(adoptedEvidence.phase, 'completed');
