...
// Issue #574, the CDW6 shape one authority over: the operator's declared routing rule (harnesses
// no seat may be routed onto) is validated at the deployment open (advanced.routing
// `.excludeHarnesses`), handed through the driver options, and READ BACK OFF the object
// createDriver returns (`this.driver.routingExcludedHarnesses`) by the swarm runtime the
// application assembles. A returned object that omits the member makes that read undefined and
// the rule silently inert, so both values are asserted: the declared list, and the empty array
// an undeclared deployment gets.
