...
test('SM8: post-main coordination failure poisons and replay does not invent structured success', async () => {
  ...
  coordinator.closeAuthority(); coordination.releaseWriterLease(); const replay = createDriver({ repoRoot: root, logDir, coordination, adapters: { mock: new MockAdapter() }, structuredMerge: resolver, watchdog: { stallMs: 60_000 } }); // valid positive stallMs; watchdog never fires in this window
