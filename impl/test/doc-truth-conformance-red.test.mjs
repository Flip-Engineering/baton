...
// parseBatonCli / instantiateProfileInventory probes + doc-file reads. R3/R8/R9/R10 exercise the
// MCP surface behaviorally through minimal McpFleetServer fixtures (the CS-2 construction
// pattern) — no network, no host state.
//
...
// A SECOND fresh server whose coordination models an orchestrator-briefing family head, so the
// initialize response composes the REAL briefing sentence (the one with the promise this row pins).
// The shared answer fixture cannot serve R8: its coordination has no contextPackHead, so its
// initialize degrades to the honest-empty line (no names), and a second initialize on an already-
// ready server is Invalid Request. The briefing is composed per-initialize from the family head —
// a family-head fixture is exactly what drives the real sentence out.
let _briefingServer = null;
function briefingServer() {
  if (_briefingServer === null) {
    const commands = Object.keys(APPLICATION_COMMAND_DEFINITIONS);
    _briefingServer = new McpFleetServer({
      coordinator: {
        list: () => [],
        card: () => ({}),
        capability: () => null,
      ...
      coordination: {
        ...
        mcpCall: () => null,
        recordMcpAudit: () => {},
        contextPackHead: (family) => ({ packId: 'pack-1', observedSeq: 42, family }),
        ledgerHeadSeq: () => 99,
      },
      application: {
        repoId: 'repo-a',
...
test('R8 (initialize-context-briefing-unmet): the initialize briefing names no non-MCP command', async () => {
  // Behavioral, per cross-cutting fold #2: the ACTUAL string the server sends in the initialize
  // response is checked — a comment plant or dead source string changes nothing, and a rename
  // (contexts.briefing) still promises a non-MCP command in the sent sentence. The briefingServer
  // fixture models an orchestrator-briefing family head so the composed sentence (not the
  // honest-empty line) is what the client receives.
  const out = await briefingServer().handle({
    jsonrpc: '2.0', id: 1, method: 'initialize',
    params: {
      protocolVersion: '2025-11-25', capabilities: {}, clientInfo: { name: 'probe', version: '0.0.1' },
    },
  });
  const instructions = out?.result?.instructions;
  assert.ok(typeof instructions === 'string' && instructions.length > 0,
    'R8: the initialize response must carry a briefing string');
  const allowlist = new Set(mcpCombinedToolNames());
  const named = new Set();
  for (const match of instructions.matchAll(/`([A-Za-z_][A-Za-z0-9_.-]*)`/gu)) named.add(match[1]);
  for (const match of instructions.matchAll(/\b[a-z][a-z0-9_]*\.[a-z][a-z0-9_]*\b/gu)) named.add(match[0]);
  const phantom = [...named].filter((name) => !allowlist.has(name));
  assert.deepEqual(phantom, [],
    `R8 (initialize-context-briefing-unmet): the initialize briefing names non-MCP tools — ${phantom.join(', ')}`);
});
