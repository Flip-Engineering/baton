...
test('CONTROL TRUTH: a timed-out command is observed, never re-sent — the late correlated reply still settles it', async () => {
  ...
  await sleep(200); // several observation windows
  ...
  await sleep(160); // several further windows
...
test('CONTROL TRUTH: an observer that synchronously settles the wait never leaks a recurring timer', async () => {
  const child = new NativeChild();
  const stalls = [];
  let rpc = null;
  ...
  const frame = await rpc.send({ type: 'get_session_stats' });
  assert.equal(frame.success, true);
  assert.equal(stalls.length, 1);
  await sleep(150); // several further windows
  assert.equal(stalls.length, 1, 'a wait settled during its own observation is never re-armed');
  assert.equal(rpc._pending.size, 0);
  await rpc.kill();
});

test('CONTROL TRUTH: a throwing stall observer never breaks the wait — the late reply still settles it', async () => {
  const child = new NativeChild();
  let calls = 0;
  const rpc = new OmpRpcProcess({
    command: 'omp', args: ['--mode', 'rpc'], spawnFn: () => child, waitAttemptMs: 30,
  ...
  await sleep(110); // several windows: an escaping observer defect would crash the timer here
