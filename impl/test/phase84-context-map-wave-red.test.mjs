...
test('CM85-F1: failed children settle durably after exact release without aggregate output', async (t) => {
  const repo = repository();
  ...
  assert.equal(failure.value.children.every((child) => (
    child.state === 'failed' && child.route.harness === routeA.harness
      && child.route.model === routeA.model && child.route.effort === routeA.effort
      && child.resultSha === null && child.artifacts.length === 0
      && child.termination.code === 'provider_turn_failed'
      && child.termination.retryable === true
