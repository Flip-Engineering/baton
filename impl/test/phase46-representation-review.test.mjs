...
test('RP4/RP6/RP7: packet is bounded, inventory-only, and preserves every missing rung', async () => {
  const review = make(); const result = await review.invoke('representation.review', { treeSha: HEAD }, ctx); for (const field of ['editAuthority', 'verificationAuthority', 'mergeAuthority', 'approvalAuthority', 'publicationAuthority', 'routingMutationAuthority', 'proofAuthority', 'policyAuthoringAuthority']) assert.equal(result.provenance[field], false); const document = JSON.parse(readFileSync(result.refs[0].path, 'utf8')); assert.deepEqual(document.missingStillPlanned, ['live-lsp', 'ssa-pdg-path-solving', 'aliases-heap-implicit-flow', 'exceptions-interprocedural-returns', 'external-ir-translation-validation', 'true-semantic-merge', 'conditional-expression-kernel-egraphs']); assert.equal(document.rows.find((row) => row.rung === 'R3').sources.some((source) => source.path === 'spec/phase54/atlas-cpg-lexical-bindings.md'), true); for (const rung of ['R1', 'R2', 'R3']) { const paths = document.rows.find((row) => row.rung === rung).sources.map((source) => source.path); assert.equal(paths.includes('spec/phase61/graph-backed-representations.md'), true); assert.equal(paths.includes('impl/src/atlas-representation-producer.mjs'), true); } await assert.rejects(make({ limits: { ...limits, maxFileBytes: 32 } }).invoke('representation.review', { treeSha: HEAD }, ctx), (error) => error.code === 'representation_review_oversize'); await assert.rejects(make({ limits: { ...limits, maxRows: 6 } }).invoke('representation.review', { treeSha: HEAD }, ctx), (error) => error.code === 'representation_review_oversize'); await assert.rejects(make({ limits: { ...limits, maxFiles: 26 } }).invoke('representation.review', { treeSha: HEAD }, ctx), (error) => error.code === 'representation_review_oversize');
});

test('RP2/RP5: tree drift, cancellation, claim substitution, and occupied artifacts fail closed', async () => {
  const review = make(); await assert.rejects(review.invoke('representation.review', { treeSha: '0'.repeat(40) }, ctx), (error) => error.code === 'representation_tree_changed'); const abort = new AbortController(); abort.abort(); await assert.rejects(review.invoke('representation.review', { treeSha: HEAD }, { ...ctx, signal: abort.signal }), (error) => error.code === 'cancelled'); const result = await review.invoke('representation.review', { treeSha: HEAD }, ctx); assert.equal((await review.reverify(result, 'representation.review', { treeSha: HEAD }, ctx)).ok, true); assert.equal((await review.reverify({ ...result, op: 'substituted' }, 'representation.review', { treeSha: HEAD }, ctx)).ok, false); assert.equal((await review.reverify({ ...result, payload: [] }, 'representation.review', { treeSha: HEAD }, ctx)).ok, false); assert.equal((await review.reverify({ refs: [{ digest: '0'.repeat(64) }] }, 'representation.review', { treeSha: HEAD }, ctx)).ok, false); writeFileSync(result.refs[0].path, `${readFileSync(result.refs[0].path, 'utf8')} `); await assert.rejects(review.invoke('representation.review', { treeSha: HEAD }, ctx), (error) => error.code === 'artifact_integrity');
});

test('RP6: public createDriver reaches the packet only through the audited ACI registry', async () => {
  const review = make(); const driver = createDriver({ repoRoot: REPO, repoId: 'repo-representation', logDir: root('log'), adapters: {}, capabilities: { 'atlas-representation-review': review }, maxCapabilityBudgetTokens: 30_000, maxCapabilityEnvelopeBytes: 256 * 1024 }); const result = await driver.coordinator.invokeCapability('atlas-representation-review', 'representation.review', { treeSha: HEAD }, ctx); assert.equal(result.payload.length, 7); const origin = 'https://representation.test'; const principal = { userId: 'alice', sessionId: 'representation-web', credentialId: 'representation-cred', authMethod: 'cookie', csrfToken: 'csrf', expiresAt: '2099-01-01T00:00:00.000Z', revoked: false, capabilities: ['control'], repoIds: ['repo-representation'] }; const web = new WebNorthbound({ coordinator: driver.coordinator, coordination: driver.coordination, repoIds: ['repo-representation'], allowedOrigins: [origin] }); const response = await web.execute({ principal, origin, csrfToken: 'csrf', remoteAddress: '127.0.0.1', transport: 'https' }, { schemaVersion: 1, commandId: 'representation-web', idempotencyKey: 'representation-web', command: 'capability_invoke', repoId: 'repo-representation', origin, args: { name: 'atlas-representation-review', op: 'representation.review', action: 'invoke', args: { treeSha: HEAD }, budgetTokens: 20_000 } }); assert.equal(response.status, 200); assert.equal(response.body.result.refs[0].digest, result.refs[0].digest); assert.deepEqual(driver.log.read('hub-capability').filter((event) => event.kind.startsWith('capability.op.')).map((event) => event.kind), ['capability.op.started', 'capability.op.completed', 'capability.op.started', 'capability.op.completed']); assert.equal(driver.close(), true);
});
