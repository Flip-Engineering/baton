...
|---|---|
| Canonical zero-quota suite | **1003/1003 passing** via `npm test` in `impl/`; its lifecycle owner reaps the private suite root, and the real linked-worktree regression covers recursive Baton execution without relying on a directory-shaped `.git` |
...
| Phase 47 Cairn causal integrity/audit/trace implementation gate | `spec/phase47/cairn-causal-integrity-audit.md`; 13 grouped CA tests and 1003/1003 canonical tests cover content/request-bound generic graph events, pre-append promotion validation, true bitemporal views and durable read pins, live/CAS/atomic supersession, canonical authorized contradiction resolution, exact contamination replay, typed live earlier grounding, independently bounded metric audit/live trace, full direct versus authenticated transport claim semantics, repository binding, mode-0600 content addressing, cancellation/result atomicity, restart determinism, and a digested stable-ID retained-goal catalog. Three parallel adversarial reviews supplied the red reproductions. Recursive GLM/concurrent-Grok/exact-route/kill-reap evidence is the next closing gate. |
