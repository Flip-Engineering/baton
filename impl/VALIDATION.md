...
| Atlas CPG seed | `docs/handoff/evidence/phase18-atlas-cpg-2026-07-11.md`; CG1–CG8, 6/6 focused tests, and 707/707 full-suite tests cover source-bound function/statement/identifier/call graphs, containment, control skeleton including branches and terminal exits, lexical reaching definitions, honest local-call resolution/ambiguity, parse health, deployment bounds, resume/tamper refusal, and reverify. Baton built and reverified a substantial graph of its MCP northbound; SSA, path-sensitive PDG, aliases, interprocedural dataflow, taint, deltas, and polyglot R3 remain explicit |
...
8. **Atlas and representation depth.** The real AST/CST structural-delta and proposal-only
   ...
   and SCIP JSON interchange ship. A single-file intraprocedural CPG seed now also ships; direct rewrite apply, full rule configs, live LSP, semantic retrieval, full CPG/dataflow/taint,
