# System Validation — living full-system ledger through Phase 36
...
|---|---|
| Canonical zero-quota suite | **833/833 passing** via `npm test` in `impl/`; its lifecycle owner reaps the private suite root, and the real linked-worktree regression covers recursive Baton execution without relying on a directory-shaped `.git` |
...
8. **Atlas and representation depth.** The real AST/CST structural-delta and proposal-only
   ...
   external evidence/freshness floor, but additional ecosystems, advisory push invalidation, true
   reachability, decisions/SBOMs, and knowledge promotion remain.
