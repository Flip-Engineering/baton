...
|---|---|
| Canonical zero-quota suite | **764/764 passing** via `npm test` in `impl/`; its lifecycle owner reaps the private suite root, and the real linked-worktree regression covers recursive Baton execution without relying on a directory-shaped `.git` |
...
| Structured integration | `docs/handoff/evidence/phase26-structured-merge-2026-07-11.md`; SM1–SM10, 14/14 focused tests, 68/68 surrounding acceptance/worktree tests, and 764/764 canonical tests cover explicit off-main three-way staging, injected bounded Mergiraf-class resolution over isolated single files, canonical path identity before read/write, marker/fallback/unknown/binary/bounds refusal, ambient-Git isolation and hook suppression, exact two-parent candidates, fresh post-merge verification before main update, dirty/advanced-main preservation, result pins, post-effect authority poisoning, replay non-invention, and orphan-stage reconcile. CPG/fingerprint evidence has no merge-authority hook; live Mergiraf remains pending because this host has no binary |
