...
export default async function createMcpServer() {
  ...
  return {
    ...
    application,
    // Application-backed servers default to the nine-tool Run surface.
    // Use `advanced` or `combined` only for an explicit kernel-control deployment.
    surface: 'application',
    principal: {
      userId: 'operator',
      sessionId: 'local-mcp-host',
      capabilities: ['control', 'observe', 'approve', 'emergency_stop', 'adopt_result'],
      repoIds: ['repo-id'],
...
The default application-backed inventory is exactly nine tools: `fleet_run_start`,
...
requires `adopt_result` without merging or publishing. The surface does not advertise deployment shutdown, Run close, worker kill, or
