...
class BatonDeployment {
  ...
  async host(options = {}) {
    ...
    if (!record(advanced)) {
      throw Object.assign(deploymentError(
        'ordinary owner-local resident hosting is not available until its private socket authority is configured',
    ...
    const handle = Object.freeze({ start, close });
  ...
  close() {
    if (!this.#closePromise) {
      this.#closePromise = this.#webHost
        ? this.#webHost.shutdown().then((closed) => closed.application)
