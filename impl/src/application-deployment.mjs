...
export async function openBatonDeployment(rawOptions, createDriver) {
  ...
  closed(advanced, ['adapters', 'capacity', 'deploymentRoot', 'routes', 'verification', 'workflowPolicy'], 'advanced');
  ...
  // The default namespace is an on-disk compatibility boundary. Phase 79/80 added durable
  // Workflow-definition and successor-Plan records that intentionally reject the older v1 event
  // grammar. Keep v1 available only through an explicit advanced recovery root instead of trying
  // to replay it under changed authority semantics or making ordinary openBaton() fail obscurely.
  const deploymentRoot = privateDirectory(advanced.deploymentRoot
    ?? join(repository.common, 'baton', 'application-v2'));
