...
export class GrokAcpCli {
  ...
  async _onClose(session, code, signal) {
    ...
    session.processClose.holdStopConfirmation();
    try {
      if (session.killing) {
      ...
      await session.processClose.close(code, signal, session.providerReady);
      session.closed = true;
      ...
      } else if (!session.killing && !session.setupFailed && session.activeTurn) {
      ...
      session.processClose.releaseStopConfirmation();
    }
  }
