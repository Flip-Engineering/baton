  ...
  *_startupReconstructionPasses() {
  ...
  _drainReaperFor(handle, hold, settled, verifying) {
    ...
    if (settled) {
      ...
      return admitted ? 'drain-reap' : 'exact-close-cleanup';
    }
    return 'drain-kill';
  }

  _drainWaitEntry(workerId, resource, reaper, sinceMs) {
    const entry = {
      resource,
      reaper: reaper ?? null,
  ...
  _drainWaitingOn(targetWorkerIds, dispositions, actor) {
    ...
    for (const workerId of targetWorkerIds) {
      ...
      if (dispositions && !dispositions.has(workerId)) {
        waiting.push(this._drainWaitEntry(workerId, 'disposition', 'run-stop-disposition', null));
      }
      if (!handle) {
      ...
      for (const hold of Object.keys(this._localResourceOwnership(handle))) {
        waiting.push(this._drainWaitEntry(workerId, `local_resources:${hold}`,
          this._drainReaperFor(handle, hold, settled, verifying),
          this._drainWaitSince.get(`${workerId}\0local_resources:${hold}`) ?? null));
      }
      if (handle.processRef && handle.processRef.state !== 'closed') {
        waiting.push(this._drainWaitEntry(workerId, `process:${handle.processRef.state}`, 'drain-kill',
          this._drainWaitSince.get(`${workerId}\0process:${handle.processRef.state}`) ?? null));
      }
      if (handle.pendingApprovalId) waiting.push(this._drainWaitEntry(workerId, `interaction:${handle.pendingApprovalId}`, 'interaction-cancel', null));
      if (handle.pendingQuestionId) waiting.push(this._drainWaitEntry(workerId, `interaction:${handle.pendingQuestionId}`, 'interaction-cancel', null));
      if (waiting.length === 0) continue;
