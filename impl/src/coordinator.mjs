...
import {
  processGroupAlive, processReadyPayload, validProcessClosedPayload, validProcessReadyPayload,
...
function recoveryProcessAbsentPayload(processRef) {
  return {
    schemaVersion: 1,
...
function validRecoveryProcessAbsentPayload(payload) {
  return payload && typeof payload === 'object' && !Array.isArray(payload)
    && Object.keys(payload).sort().join('\0')
      === ['generation', 'pid', 'processGroupId', 'reason', 'schemaVersion'].sort().join('\0')
...
const COORDINATION_MUTATORS = new Set([
  ...
  'settleContextMapCall', 'recordTaskResourceRelease',
]);
...
export class Coordinator {
  ...
  async stopRunTargets(targetWorkerIds, actor = 'orchestrator') {
    ...
    const attempt = async (workerId) => {
      ...
      if (handle.status === 'orphaned' && handle.currentIncarnation !== true
  ...
  async releaseTerminalTaskResources(taskId, workerId, actor = 'policy') {
    ...
    const handle = this._workers.get(workerId);
    if (!task || !handle || handle.taskId !== taskId || task.status !== 'completed') {
    ...
    if (!durable || durable.status !== 'completed' || durable.assignee !== workerId
