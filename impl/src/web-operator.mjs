...
async function reconcile(commandId){for(let attempt=0;attempt<10;attempt+=1){const status=await request('/v1/commands/'+encodeURIComponent(commandId));if(status.command.status!=='admitted'){const outcome=status.command.outcome;if(outcome&&outcome.httpStatus>=400)throw new Error(outcome.body&&outcome.body.error&&outcome.body.error.code||'command_failed');return outcome?outcome.body:status;}await new Promise((resolve)=>setTimeout(resolve,300));}throw new Error('command_still_admitted');}
async function command(name,args,expectedFence){const identity=ids();const envelope={schemaVersion:1,...identity,command:name,args:args||{},repoId:session.identity.repoIds[0],origin:location.origin};if(Number.isInteger(expectedFence))envelope.expectedFence=expectedFence;const response=await request('/v1/commands',{method:'POST',headers:{'content-type':'application/json','x-baton-csrf':readCookie('__Host-baton_csrf')||''},body:JSON.stringify(envelope)});return response.status==='admitted'?reconcile(identity.commandId):response;}
