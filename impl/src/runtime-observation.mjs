...
export function _pausedActTargets(coordinator, recorder, record) {
    ...
    if (task.status !== 'paused' && !(record.reported && task.status === 'working'
      && handle.turnInFlight !== true)) return { ok: false, result: 'not_paused', status: task.status };
...
export async function _reportRunTurn(coordinator, recorder, handle, terminalEvent, report) {
    ...
    while (lineage && !visited.has(lineage.parentRunId)) {
      ...
      if (candidate && ['working', 'blocked'].includes(candidate.status)) { parent = candidate; break; }
