...
const SWARM_CLI_SUMMARIES = Object.freeze({
  ...
  'swarm.view': "Read one swarm's membership, work, shared context, contributions, reviews, caller authority, and available actions; --participant-id scopes the read to one participant's delegation, and --projection names the slice to answer with (full, outline, participants, contributions, attention, guidance, workspace; default full). Each participant row carries its guidance rows, its live checkout custody, the route and scope it was recruited under, its base {observedHead, target, behind} derived from the repository at read time, and its lastRefusal while one stands; `updates` lists the update kinds the caller may send now beside availableActions, each with the permission that admits it, and every projected row carries the seq and ts of the record that wrote it. Participants, contributions, groups, couplings and attention are arrays of rows; work, assignments, reviews and context are keyed objects. The `deployment` summary carries the host capacity derivation with its live leases and visible queue beside the workspace capacity observation and its derived floor, where `capacityPressure` names the crossing.",
