...
const SWARM_CLI_SUMMARIES = Object.freeze({
  ...
  'swarm.guide': "Send guidance to one participant through its native delivery lane. Guidance starts a waiting seat's next turn. The receipt records the sender, priority, delivery state and transport refusal when delivery fails.",
  ...
  'swarm.notify': "Send a message to a participant in this swarm or, with --to-swarm-id, another swarm in the deployment. Native delivery starts a waiting recipient's next turn. The receipt records the sender, recipient, time and delivery result; --receipt reads it back.",
...
const SWARM_GUIDE_DETAILS = ['The receipt carries the guide\'s OWN durable row as',
  ...
  '(delivery.lane), or `refused` with the transport reason. Historical rows can carry `parked`.',
  'Guidance to a waiting seat starts its next turn. `next` names the observation of that turn',
  'boundary (wake class paused).'].join(' ');
