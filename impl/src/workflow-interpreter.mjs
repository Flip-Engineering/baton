...
// #207: the run.start objective cap comes from the frame-economics registry — Decision 8's
// no-re-declare law (the interpreter never re-declares a cataloged lane's byte literal). The
// registry imports only node:crypto, so the lane's W5 node-builtins-only transitive graph holds.
...
// (recorded in docs/reference/evidence/phantom-root-2026-08-15/wave-e/notes-row-admission-align.md):
...
// (run.objective, 4096 bytes, limits.mjs) is the ADMISSION bound enforced at compile/admit below.
// The interpreter does NOT split: a rendered objective over the run cap refuses workflow_spec_invalid
// naming both byte counts — fail-loud at the seam, never a per-member phantom start failure.
...
// machinery would admit — the embedded client's nonempty check, application-client.mjs:11-13,
// measures Buffer.byteLength of the objective it receives) and the run.objective cap. The cap is
// the registry row (FRAME_LIMITS, imported above — Decision 8's no-re-declare law), never a
// re-declared literal; the interpreter does NOT split, so a rendered objective over the cap is an
// unstartable member and the spec refuses at admission.
