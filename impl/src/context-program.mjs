...
// REPL-1 (docs/33 §3.1, issue #21): the ReplManifest is a SECOND manifest shape with its own
// disjoint digest basis. It replaces the Workflow `workflow` coordinate (goal/plan/node/task,
// the `^plan:` gate) with a bare `repl` coordinate (replRole + runId) and carries the header
// `baton.repl_manifest`. Because `kind` is inside the digested body, a repl and a context
// manifest with byte-identical tree/branches have different digests — the collision R33-2
// forbids is structurally impossible. The `tree` and per-branch discipline are shared verbatim
// (same `manifestBranch`, same `ctx:sha256:<digest>` refs) — REPL-1 introduces no new branch
// ref form (`cell:` refs are REPL-3).
const REPL_MANIFEST_FIELDS = Object.freeze([
  'branches', 'kind', 'policyDigest', 'repl', 'repoId', 'schemaVersion', 'tree',
]);
// Deliberately narrower than SAFE_ID (which permits `:`): a `:` inside the suffix would make the
// `worker:` tag boundary ambiguous and break the store-side equality
// `replRole === 'worker:' + auth.principalId`. The narrowing is intentional and pinned (P2-11).
...
export function normalizeReplManifest(value, policyInput = DEFAULT_CONTEXT_PROGRAM_POLICY) {
  ...
  if (replRole !== 'shared'
    && (typeof replRole !== 'string' || !REPL_WORKER_ROLE.test(replRole))) {
...
// Rule A-any: the ONE kind-dispatching entry. Wired symmetrically at exactly the three
// identity/session construction sites that must accept either manifest kind
// (contextProgramInputRefs, contextSessionIdentity, the DurableContextSession constructor) — it
// never widens either normalizer and is nowhere else.
export function normalizeManifestAny(value, policyInput = DEFAULT_CONTEXT_PROGRAM_POLICY) {
  ...
  throw typed('Context manifest kind is unrecognized', 'context_manifest_invalid');
}
...
export class DurableContextSession {
  constructor({ coordination, bench, manifest, principal, execute = null, admitSession = null }) {
    ...
    this.bench = bench;
    // REPL-1 rule 4a/11: the constructor accepts either manifest kind (Workflow context or REPL)
    // and an injected admission function, defaulting to the Workflow `admitContextSession`. The
    // admit-on-construct shape and the `context.session:<manifestDigest>` idempotency key are
    // unchanged — `openReplSession` passes `admitReplManifest`'s sibling `admitReplSession`.
    ...
    this._admitSession = admitSession ?? ((fields, sessionAuth) => (
      coordination.admitContextSession(fields, sessionAuth)));
