...
export class StatelessContextBench {
  constructor({ artifactRoot, sources, environmentDigest, policyDigest }) {
    ...
    this.sources = new Map(Object.entries(sources).map(([ref, source]) => [ref, clone(source,
  ...
  execute({ manifest, program }) {
    ...
    return deepFreeze({
      ...cellCore,
      cellId: `cell:${cellDigest}`,
      state: 'completed',
      providerEffects: 0,
      output,
      outputRef: {
        kind: 'context_value', digest: outputDigest,
        mediaType: 'application/vnd.baton.context-value+json', path,
      },
    });
  }
}
...
export class ContextSession {
  ...
  outline() {
    return deepFreeze({
      ...
      methods: ['outline', 'index', 'search', 'chunk', 'coverage', 'cell', 'help'],
    });
  }
