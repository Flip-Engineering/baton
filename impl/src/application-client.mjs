...
export class BatonRun {
  ...
  async inspect(options = {}) {
    exactOptions(options, new Set([
      'depth', 'section', 'item', 'offset', 'cursor', 'waitMs',
    ]), 'inspect');
  ...
  async help(topic = 'run', depth = 'outline') {
    if (!nonempty(topic)
...
export class BatonRuns {
  ...
  async list() {
    return this.#application.command('runs.list', {}, this.#principal);
  }
  ...
  async attach(runId) {
    ...
    if (view?.runId !== runId || !view?.outline) {
      throw clientError('Run attachment response is invalid', 'application_attach_invalid');
    }
    return new BatonRun(this.#application, this.#principal, runId, view,
...
export class BatonClient {
  ...
  help(topic = 'application', depth = 'outline') {
    ...
    return this.#application.command('application.help', { topic, depth }, this.#principal);
  }
...
export function bindBatonPort(commandPort) {
  ...
  return new BatonClient(port, Object.freeze({ kind: 'bound-command-port' }));
}
