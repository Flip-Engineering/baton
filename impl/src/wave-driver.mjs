...
// status or an explicit claim in its checkpoint. The caller's abort signal is the orchestrator's
  ...
  async function run(options = {}) {
    ...
    try {
      ...
      for (;;) {
        ...
        // L4 steering. A generic pausable turn has no assignment-done field: its projected
        // checkpoint claim describes the completed turn, not completion of the assignment. The
        // driver therefore continues each new checkpoint. Terminal status is the member's done
        // declaration; the caller's abort signal is the orchestrator's stop decision (#572).
        ...
        if (rosterAccounted) {
          ...
          // settle a live member. The caller receives them if it later stops the driver.
          stalls = stalledNow;
          waiting = waitingNow;
        }
