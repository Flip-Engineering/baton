...
// derives from the wave's OWN observed marker-advance cadence — max(2x the max observed gap,
// 8x the poll interval), the same derivation the quiescence quiet window uses (#163 law,
...
const DEFAULT_POLICY = Object.freeze({
  ...
  // #163 law (row-stall-break): the stall window is DERIVED, never a fixed production clock —
  // the fatal window reads the wave's OWN observed marker-advance cadence,
  // max(2x maxObservedGapMs, 8x pollIntervalMs), the same derivation the quiescence quiet
  // window uses (workflow-interpreter.mjs QUIESCENCE_MIN_SILENT_POLLS). `null` = derive; a
  // policy may still PIN an explicit stallTimeoutMs (back-compat — the red suites rely on it).
  stallTimeoutMs: null,
...
function freezePolicy(raw) {
  ...
  // #163 law (row-stall-break): `null`/omitted opts into the DERIVED stall window (the wave's own
  // observed marker-advance cadence); a PINNED stallTimeoutMs is still validated as a positive
  // integer (back-compat — the red suites rely on pinned values).
