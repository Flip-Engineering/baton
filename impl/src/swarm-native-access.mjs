...
export const SWARM_NATIVE_GUIDANCE = [
  ...
  'Use swarm.guide to speak to a participant, swarm.recruit to bring in help when granted, and swarm.watch to await relevant updates. These commands use participant names; no worker, fence, pause, or approval choreography is required. swarm.capture belongs to the root, and the root cannot capture your work until a contribution is recorded — ending a turn without publishing leaves the work unreachable.',
