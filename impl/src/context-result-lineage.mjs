...
const SOURCE_EVIDENCE_FIELDS = Object.freeze([
  'cellId', 'coordinateDigest', 'environmentDigest', 'kind', 'manifestDigest', 'outputLineageDigest',
  'outputLineages', 'outputRef', 'policyDigest', 'programDigest', 'providerEffects',
  'schemaVersion', 'selectedSourceItems', 'sourceBranches', 'sourceCoordinates', 'sourceItems',
]);
...
function artifactRef(value, expectedKind, expectedMediaType, content, label) {
  ...
  if (value.kind !== expectedKind || value.mediaType !== expectedMediaType
    ...
    || value.bytes !== Buffer.byteLength(stable(content))) {
...
function acceptedChild(value, partition, cleanupDigest) {
  ...
  if (value.schemaVersion !== 1 || value.state !== 'accepted'
    ...
    || typeof value.nodeKey !== 'string' || value.nodeKey.length === 0
...
function build(value) {
  ...
  if (!Array.isArray(value.children) || !Array.isArray(value.providerResults)
    ...
    || sourceOutput.items.length !== call.partitions.length) {
  ...
  const outputLineages = call.partitions.map((partition, index) => {
    ...
    if (partition.index !== index
      ...
      || source.index !== index || source.itemDigest !== partition.itemDigest
      || source.coordinateDigest !== partition.coordinateDigest
      || source.lineageDigest !== partition.lineageDigest
      || providerResult.unitId !== partition.partitionId
