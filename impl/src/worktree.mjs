...
function readMeta(repoRoot, taskId) {
  ...
  try {
    ...
    // derives the number.
    if ((stat.mode & 0o077) !== 0 || stat.size > FRAME_LIMITS['worktree.cwd_observation_bytes'].value) return null;
...
const LINKED_WORKTREE_RECORD_BYTES = FRAME_LIMITS['worktree.linked_owner_record_bytes'].value;
  ...
  if (platform === 'darwin') {
    ...
    try {
      result = run('/usr/sbin/lsof', [
        ...
        encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], maxBuffer: FRAME_LIMITS['worktree.cwd_observation_bytes'].value,
      });
    } catch {
      throw linkedLivenessUnobservable(
        worktreePath,
