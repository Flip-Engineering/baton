...
const CAPABILITY = Object.freeze({
  ...
  // Part A rule 3: every reflex tool's capability requirement is just 'observe' — the real
  // orchestrator-authority gate for the four board/two package mutations is the mandatory
  // run-orchestrator lease check inside dispatch, never a capability string.
...
// MCP-SLICE1-INTEGRATION seam (docs/reference/evidence/mcp-reflex-live-2026-07-22/
// mcp-reflex-surface-decisions.md Part A rule 1): REFLEX_TOOL_DEFINITIONS is the fourth tool
// table, additive across seats — slice 1 owns baton_context_eval / baton_decision_list /
// baton_decision_answer, slice 2 (this seat) owns baton_board_* / baton_package_*. If the other
// seat lands first, merge entries into one array; never redeclare REFLEX_TOOL_DEFINITIONS itself.
const REFLEX_BOARD_PACKAGE_TOOL_DEFINITIONS = [
  {
...
const REFLEX_TOOL_DEFINITIONS = Object.freeze(REFLEX_BOARD_PACKAGE_TOOL_DEFINITIONS.map((tool) => Object.freeze({
  ...tool,
  _meta: Object.freeze({ 'baton/registryDigest': APPLICATION_SEMANTIC_REGISTRY.digest }),
  execution: Object.freeze({ taskSupport: 'forbidden' }),
})));
// MCP-SLICE1-INTEGRATION seam: read-only reflex tool names needing typed-error reach through the
// observe-path error gate (Part F rule 12) — slice 1 should add its own read-only reflex tool
// names here too (e.g. baton_decision_list), never redeclare this Set.
const REFLEX_READ_ONLY_TOOLS = new Set(['baton_board_read', 'baton_package_read']);
