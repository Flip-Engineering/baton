...
function normalizeWorkflowFeedback(value) {
  ...
  const findings = input.findings.map((finding) => {
    ...
    if (!['defect', 'risk', 'suggestion', 'question', 'observation'].includes(finding.kind)
...
function workflowNodeBudget(profile, members) {
  const divide = (value) => Math.floor(value / members);
  const budget = {
    tokens: Math.min(profile.nodeBudget.tokens, divide(profile.goalBudget.tokens)),
    usd: Math.min(profile.nodeBudget.usd,
      Math.floor((profile.goalBudget.usd * 1_000_000_000) / members) / 1_000_000_000),
...
export class BatonApplication {
  ...
  _performRunStop(stop) {
    ...
    const operation = (async () => {
      ...
      for (const targetRunId of targetRunIds) await this._abortResultExportDeliveries(targetRunId);
  ...
  async start(rawIntent, rawOwner, rawContext = null) {
    ...
    const nodeFields = intent.composition ? intent.composition.team.map((member) => ({
      ...
      budget: clone(workflowNodeBudget(profile, intent.composition.team.length)),
      routes: {
  ...
  async approve(runId, planDigest, rawApprover) {
    ...
    if (current.plan.nodes.length > 1) this._workflowDefinition(current);
  ...
  async _buildEvidence(current) {
    ...
    if (current.plan?.nodes.length > 1) return this._buildWorkflowEvidence(current, view);
  ...
  _buildWorkflowEvidence(current, view) {
    ...
    const relevantSeqs = [
      ...workflowSeqs, ...taskSeqs, ...artifactSeqs,
  ...
  _workflowDefinition(current) {
    if (current.plan?.nodes.length <= 1) return null;
    const records = this.driver.coordination.events().filter((event) => event.kind === 'driver.recorded'
    ...
    if (event.actor !== APPLICATION_WORKFLOW_RECORD_ACTOR
      ...
      || definitionDigest !== digest(core) || core.schemaVersion !== 1
  ...
  _workflowCandidates(current, projection, definition) {
    ...
    for (const binding of definition.attempts) {
      ...
      candidates.push(deepFreeze({
        ...core, candidateId: `candidate:${digest(core)}`, candidateDigest: digest(core),
      }));
    }
  ...
  async stopWorkflowMember(rawRequest, rawPrincipal) {
    ...
    return this._withRunEffect(rawRequest.runId, async () => {
      ...
      if (!current.plan || current.plan.nodes.length < 2) {
  ...
  async selectWorkflowCandidate(rawRequest, rawPrincipal) {
    ...
    const core = {
      ...
      candidate: {
        ...
        resultSha: candidate.resultSha, evidenceDigest: candidate.evidenceDigest,
      },
  ...
  async _buildWorkflowView(current, observer, options = {}) {
    ...
    for (const binding of definition.attempts) {
      ...
      if (terminalResult) terminalResults.push(terminalResult);
      const requested = {
    ...
    const attention = [...workerAttention, ...selectionAttention].slice(0, MAX_ATTENTION);
    ...
    const view = {
      ...
      attention, attentionTruncated: workerAttention.length + selectionAttention.length > attention.length,
      verification: {
      ...
      result: selection ? {
        state: 'selected', candidate: clone(selection.candidate),
      } : candidates.length > 0 ? {
        state: 'selection_required', candidateCount: candidates.length,
      } : null,
      integration: null, export: null,
      ownership: phase === 'stopped' ? { workers: 0, workerIds: [], closed: false }
        : { workers: ownedWorkers.length, workerIds: ownedWorkers.map((handle) => handle.id).sort(), closed: false },
      evidence: [],
      narrative: terminalCauseNarrative(terminalCause)
  ...
  async _buildView(current, observer, options = {}) {
    ...
    if (current.plan?.nodes.length > 1) return this._buildWorkflowView(current, observer, options);
  ...
  _semanticActions(current, view, principal) {
    ...
    for (const candidate of view.nextActions ?? []) {
      if (['adopt_result', 'select_candidate', 'send_feedback', 'stop_member', 'semantic_review', 'integrate', 'export_result', 'retry_verification', 'resume_work'].includes(candidate.kind)
  ...
  async command(name, args, rawPrincipal, rawContext = null) {
    ...
    if (context?.sessionAuthority && !recursiveReadCommands.has(name) && !recursiveEffectCommands.has(name)) {
  ...
  async stop(runId, rawReason, rawPrincipal, rawContext = null) {
    ...
    return this._withRunEffect(request.runId, () => this._stop(request, principal, context));
  }
