...
export class BatonApplication {
  ...
  _validateContextEffectPlan(current) {
    ...
    const unitIds = new Set(call?.units?.map((unit) => unit.unitId) ?? []);
    ...
    if (!call || call.kind !== 'baton.context_effect_call'
      ...
      || bindings.length !== call.units.length
  ...
  async _reconcileContextCalls(current) {
    ...
    for (const call of calls) {
      ...
      const termination = failed ? {
        ...
        summary: `One or more Context ${generic ? 'effect' : 'map'} children failed before acceptance.`,
      } : null;
  ...
  async _proposeContextReduce(current, inputs, caller) {
    ...
    if (sourceCall.kind !== 'baton.context_map_call') {
      throw applicationError(
    ...
    if (artifacts.evidence?.schemaVersion !== 3 || !Array.isArray(lineages)
  ...
  async _performContextAction(current, action, inputs, caller, signal = null) {
    ...
    if (['context_map', 'context_reduce'].includes(action.kind)) {
      ...
      await this._authorize('run.act', caller, current.goal.runId, {
        ...
        ...(inputs.cellId ? { cellId: inputs.cellId } : { callId: inputs.callId }),
      });
