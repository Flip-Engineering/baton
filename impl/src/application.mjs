...
export class BatonApplication {
  constructor(options) {
    ...
    if (options.context !== undefined) {
      exactObject(options.context, ['openSession', 'principal'], 'application_config_invalid',
  ...
  _contextState(current) {
    ...
    const state = currentSessions.length === 0
    ...
    return deepFreeze({
      sessions, cells, currentSessions, currentCells,
      projection: {
        state,
        ...
        summary: currentSessions.length > 0
  ...
  async act(rawRequest, rawPrincipal, rawContext = null) {
    ...
    if (action.kind.startsWith('context_')) {
      ...
      return this.inspect({
        runId: request.runId, depth: 'item', section: 'context', item: cell.cellId,
      }, principal, context);
    }
