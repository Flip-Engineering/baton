...
export class BatonApplication {
  ...
  _progressTiming(current, view) {
    ...
    const startedMs = Date.parse(current.goal.definedAt);
    ...
    const last = meaningful.at(-1) ?? { ts: current.goal.definedAt };
    ...
    return deepFreeze({
      startedAt: current.goal.definedAt,
      observedAt,
  ...
  async listRuns(rawPrincipal) {
    ...
    if (ordered.length > MAX_RUN_LIST_ITEMS) {
