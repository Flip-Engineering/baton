...
// degraded } — survived is counted from the work-rest set (a rested member reads projection
// state accepted), and every terminal non-survivor is receipted in cell.lost with its
// per-member cause. A member the projection never dispatched (no task) is receipted
// cell_member_lost — a spawn that never started is a loss with its own name, never a silent
// absence. The aggregate is pure in (declaration, projection, task reads): no clock, no
// counter. Terminal minting reads it: lost beyond the quorum allowance fails cell_below_quorum,
// any loss under strict fails cell_exact_breach, and quorum <= survived < size rests cell.degraded.
const CELL_LOST_PROJECTION_STATES = new Set(['failed', 'cancelled', 'stopped', 'denied']);
function deriveCellAggregate(cell, nodes, taskOf) {
  ...
  for (const node of list) {
    ...
    if (!node?.taskId || !task) {
      // The member never started: no task to rest or fail, so the aggregate names the loss.
  ...
  async _buildView(current, observer, options = {}) {
    ...
    const cellAggregate = cellDeclaration
      ? deriveCellAggregate(cellDeclaration, projection.nodes,
        (taskId) => this.driver.coordination.task(taskId))
      : null;
