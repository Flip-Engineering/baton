...
function createWaveHandle({ repoRoot, members, state, waveId = null }) {
  ...
  async function materialize(entry, excludeShas = []) {
  ...
  async function settle({ timeoutMs = 60_000 } = {}) {
    ...
    // computed once per call so the concurrent re-resolution below stays deterministic. A member's
    ...
    const observed = await Promise.all([...state.members].map(async ([role, entry]) => {
      ...
      try {
        ...
        outcome.resultSha = await materialize(entry, attributed.get(role) ?? []);
      } catch (error) {
