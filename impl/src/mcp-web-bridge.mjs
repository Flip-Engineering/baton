...
export class BatonWebApplicationFacade {
  ...
  async authorizeReplay(name, _args, principal, context) {
  ...
  async command(name, args, principal, context) {
    ...
    await this._attestSession(principal);
    const idempotencyKey = `mcp-web-${digest(MUTATIONS.has(name) ? {
      repoId: this.repoId, principalId: principal.principalId,
      sessionId: principal.sessionId, command: name, args,
    } : { repoId: this.repoId, key: context.idempotencyKey })}`;
