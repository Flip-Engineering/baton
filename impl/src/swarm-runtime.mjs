...
export class SwarmRuntime {
   ...
   * (`swarm-turn-report:swarmId:participantId:workerId:turnSeq`), so a faulted append is repaired
  ...
  _reconcileTurnReportedRows(swarm, actor) {
    ...
    for (const event of this.store.eventsView()) {
      ...
      const turn = `${payload.participantId}:${payload.workerId ?? ''}:${payload.turnSeq ?? ''}`;
    ...
    return owed.map(({ payload, turn }) => {
      const event = this.store.recordDriver('swarm.root_attention_owed', {
        ...
        ask: typeof payload.report === 'string' && payload.report.length > 0 ? payload.report : null,
        next: { command: 'swarm.view', swarmId: swarm.swarmId },
