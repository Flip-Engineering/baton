 ...
 * process-group ownership. A close starts one bounded reap. An inconclusive result deliberately
 * clears only the guarded attempt, so a later authorized stop can re-signal and re-probe the same
 * generation/PID/group. Concurrent callers join the same attempt and confirmed ownership seals
 * the latch permanently.
 */
