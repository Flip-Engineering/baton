...
export class KimiAcpCli {
  constructor(options = {}) {
    ...
    this._args = options.args ? [...options.args] : ['acp'];
    ...
    if (!['default', 'auto', 'yolo'].includes(this._permissionMode)) {
  ...
  card() {
    ...
    return {
      ...
      permissions: { mode: this._permissionMode, boundary: 'Baton-owned worktree and private runtime' },
      verbs: {
  ...
  async _onClose(session, outcome) {
    ...
    if (session.setupFailed) return;
    if (session.timeoutFailure) { this._emitCrash(session, session.timeoutFailure); return; }
