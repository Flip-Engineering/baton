...
export class RepositoryContextRuntime {
  ...
  constructor(options) {
    ...
    this.environmentDigest = contextValueDigest({
      ...
      runtime: 'repository-json-cas-v2', stateMode: this.policy.stateMode,
  ...
  async openSession({ authority, principal, signal = null }) {
    ...
    if (!manifest) {
      const produced = await this._executeOwned('source', {
        ...
        scopes: node.pathScope, policy: this.policy,
      }, signal);
      ...
      const attestationCore = {
        ...
        scopeDigest: contextValueDigest([...node.pathScope].sort()),
        branch: 'repository',
