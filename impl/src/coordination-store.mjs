...
export class CoordinationStore {
  ...
  _validateContextEffectCallAdmissionPayload(payload, event, integrity = false) {
    ...
    if (payload.expectedPlanDigest !== expectedPlanDigest
      ...
      || normalizedPlan.predecessor?.version !== predecessor.version
      || normalizedPlan.predecessor?.digest !== predecessor.digest
      || normalizedPlan.nodes.length !== call.units.length
    ...
    if (units.size !== call.units.length
      || call.units.some(({ unitId }) => !units.has(unitId))) {
      return fail('Context effect-call Plan does not cover the exact unit set',
        'context_call_plan_invalid');
    }
    ...
    if (sourceEvent.actor !== 'application:workflow-registry'
      ...
      || sourceDefinition.schemaVersion !== 3 || successorDefinition.schemaVersion !== 3) {
    ...
    const catalogRole = workflowCatalogRole(sourceDefinition, call.role);
    const expectedGeneration = sourceDefinition.lineage.generation + 1;
    const expectedRoot = sourceDefinition.lineage.generation > 1
      ? sourceDefinition.lineage.rootDefinitionDigest : sourceDefinition.definitionDigest;
    if (!catalogRole || authority.roleCatalogDigest !== sourceDefinition.roleCatalog.catalogDigest
      || authority.profileDigest !== sourceDefinition.profileDigest
      || successorDefinition.profileDigest !== sourceDefinition.profileDigest
      || successorDefinition.workflowPolicyDigest !== sourceDefinition.workflowPolicyDigest
      || canonicalDigest(successorDefinition.workItem)
        !== canonicalDigest(sourceDefinition.workItem)
      || canonicalDigest(successorDefinition.roleCatalog)
      ...
      || successorDefinition.lineage.rootDefinitionDigest !== expectedRoot
      || successorDefinition.attempts.length !== call.units.length) {
    ...
    if (attemptedUnits.size !== call.units.length) {
      return fail('Context effect-call Workflow Attempts do not cover its exact units',
        'context_call_definition_invalid');
    }
    ...
    try {
      if (call.operator === 'map') {
        ...
        if (canonicalDigest(expectedSource) !== canonicalDigest(call.source)
          || artifacts.evidence?.schemaVersion !== 3
          || artifacts.output?.items?.length !== call.source.itemCount
          || lineages?.length !== call.source.itemCount
  ...
  _validateContextMapProviderResults(call, children, cleanup, values, integrity = false) {
    ...
    if (!plan) {
      return fail('Context map provider-result Plan is unavailable');
    }
    const normalized = [];
  ...
  contextCall(callId) {
    ...
    const executionUnits = generic ? admitted.units : admitted.partitions;
  ...
  contextCallArtifacts(callId) {
    ...
    if (call.state === 'completed') {
      if (![2, 3].includes(evidence?.schemaVersion)) {
  ...
  contextCompletedCallSource(callId) {
    ...
    const artifacts = this.contextCallArtifacts(callId);
    ...
    try {
      return clone(normalizeContextEffectSource({
