...
import {
  validateContextProviderResultCapsule, validateContextProviderResultRef,
...
export class CoordinationStore {
  ...
  _resetProjection() {
    ...
    this._fleetDrains = new Map(); this._runStops = new Map(); this._runStopByTarget = new Map(); this._runResultAdoptions = new Map(); this._runResultExports = new Map();
  ...
  _runStopContextTargets(targetRunIds) {
    ...
    const targetContextCallIds = [...this._contextCalls.values()]
      .filter((call) => targetRunSet.has(call.source.runId)
        && call.state !== 'stopped')
  ...
  _contextMapSettlementChildren(call, integrity = false, cleanup = null) {
    const failChild = (message, code = 'context_map_call_not_terminal') => (
      this._contextFailure(message, integrity ? 'context_map_call_integrity' : code, integrity)
    );
    ...
    if (!plan || approval?.disposition !== 'approved') {
      return failChild('Context map settlement requires its exact approved successor Plan');
    }
    const terminalCause = (task) => {
      ...
      if (!transitioned && !revoked) {
        return failChild('Context map child terminal identity is invalid',
      ...
      if (coordinate !== null && coordinate !== undefined) {
        ...
        if (!mapped || mapped.seq >= terminal.seq || mapped.payload.worker !== task.assignee
          ...
          || digest(source) !== mapped.payload.digest) {
          return failChild('Context map child terminal evidence is invalid',
            'context_map_child_terminal_invalid');
        }
      }
      ...
      const defaultSummary = task.status === 'cancelled'
        ? 'Context map child was cancelled before acceptance.'
        : 'Context map child failed before acceptance.';
    ...
    const rows = [];
    for (const partition of call.partitions) {
      const node = plan.nodes.find((candidate) => (
        candidate.contextCall?.partition?.partitionId === partition.partitionId
      ));
      ...
      if (!node || !dispatch || !task || !TERMINAL.has(task.status)
        || dispatch.binding?.planDigest !== plan.digest
        || dispatch.binding?.nodeKey !== node.key) {
        return failChild('Context map settlement has a missing or nonterminal child');
      }
      ...
      if (task.status === 'completed' && (!commit?.refs?.sha || !verification)) {
        return failChild('Completed Context map child lacks exact gate artifacts',
          'context_map_child_artifact_invalid');
      }
      if (task.status !== 'completed' && activeArtifacts.length > 0) {
        return failChild('Unaccepted Context map child retains accepted gate artifacts',
          'context_map_child_artifact_invalid');
      }
      ...
      const release = cleanup?.targets?.find((target) => (
        target.partitionId === partition.partitionId
      )) ?? null;
      const core = {
        schemaVersion: 1,
  ...
  _normalizeContextMapCleanupReceipt(call, children, value, integrity = false) {
    const fail = (message) => this._contextFailure(message,
      integrity ? 'context_map_call_settlement_integrity' : 'context_map_cleanup_invalid',
      integrity);
    const fields = [
    ...
    if (targets.some((target) => !target || typeof target.workerId !== 'string'
      ...
      return fail('Context map cleanup target lacks an exact durable resource release');
    }
    if (!value || typeof value !== 'object' || Array.isArray(value)
      || Object.keys(value).sort().join(',') !== fields.sort().join(',')
      || value.schemaVersion !== 1 || value.callId !== call.callId
      ...
      return fail('Context map cleanup receipt does not prove its exact descendant union');
    }
    const core = {
    ...
    if (value.cleanupDigest !== canonicalDigest(core)) {
      return fail('Context map cleanup receipt identity changed');
    }
    return freeze({ ...core, cleanupDigest: value.cleanupDigest });
  }

  _normalizeContextMapProviderResults(call, children, cleanup, value, integrity = false) {
    const fail = (message) => this._contextFailure(message,
    ...
    if (!Array.isArray(value) || value.length !== accepted.length) {
      return fail('Context provider result ref set does not match accepted children');
    }
    const plan = [...this._plans.values()].find((candidate) => (
      candidate.digest === call.expectedPlanDigest
    ));
    if (!plan) return fail('Context provider result ref set lost its successor Plan');
    const normalized = [];
    for (let index = 0; index < value.length; index += 1) {
      ...
      let providerResult; let capsuleRef; let capsule; let source;
      try {
        providerResult = validateContextProviderResultRef(value[index]);
        capsuleRef = normalizeContextArtifactRef(
          providerResult.capsuleRef, 'context_provider_result', this._contextProgramPolicy,
        );
        capsule = validateContextProviderResultCapsule(
          this._contextReferenceRead(capsuleRef),
        );
        source = this._contextReferenceRead(capsule.sourceRef);
      } catch (error) {
        return fail(error?.message ?? 'Context provider result CAS is unavailable');
      }
      const sourceItems = Array.isArray(source) ? source : [source];
      if (!node || canonicalDigest(node) !== child.nodeDigest || !commit
        || providerResult.callId !== call.callId
        ...
        || canonicalDigest(providerResult.capsuleRef) !== canonicalDigest(capsuleRef)
        || contextValueDigest(capsule) !== capsuleRef.digest
        || contextValueDigest(source) !== capsule.sourceRef.digest
        || sourceItems.length !== capsule.sourceRef.itemCount
        ...
        || capsule.result.retainedResultRef !== commit.refs.retainedResultRef
        || canonicalDigest(capsule.result.pathScope) !== canonicalDigest(node.pathScope)) {
        return fail('Context provider result authority changed');
      }
      normalized.push(providerResult);
    }
  ...
  _validateContextMapCallSettlementPayload(payload, event, integrity = false) {
    ...
    let outputRef; let evidenceRef; let children; let cleanup; let providerResults;
    let state; let termination;
    try {
      ...
      providerResults = this._normalizeContextMapProviderResults(
        call, children, cleanup, payload.result.providerResults, integrity,
      );
      if (payload.result.providerResultDigest !== canonicalDigest(providerResults)) {
        throw new TypeError('Context provider result ref set identity changed');
      }
      if (state === 'completed') {
    ...
    const result = freeze({
      ...
      providerResults, providerResultDigest: canonicalDigest(providerResults),
      cleanup, outputRef, evidenceRef,
    ...
    if (!integrity) {
      ...
      const outputValid = state === 'failed' ? output === null
        : output?.schemaVersion === 1 && output.kind === 'baton.context_value'
          ...
          && output.sourceItems === providerResults.length
      ...
      if (!outputValid || !evidence || evidence.schemaVersion !== 2
        ...
        || evidence.providerResultDigest !== result.providerResultDigest
  ...
  _applyGoalPlanEvent(event) {
    ...
    try {
      if (event.kind === 'goal.version_defined') {
        ...
        this._validateContextMapPlanProposal(plan, true);
  ...
  _apply(event) {
    ...
    } else if (event.kind === 'context.call_admitted') admittedRunId = p?.call?.source?.runId ?? null;
    else if (event.kind === 'context.call_settled') {
      admittedRunId = this._contextCalls.get(p?.callId)?.source?.runId ?? null;
    }
    ...
    if (['goal.version_defined', 'plan.version_proposed', 'plan.approval_decided', 'plan.node_dispatched', 'plan.node_budget_settled'].includes(event.kind)) {
      ...
      const validated = this._validateContextMapCallAdmissionPayload(p, event, true);
      this._contextCalls.set(validated.call.callId, freeze({
        ...clone(validated.call), authority: clone(validated.authority),
      ...
      const validated = this._validateContextMapCallSettlementPayload(p, event, true);
      const artifactRows = [
        ['context-value', validated.result.outputRef],
        ['context-call-evidence', validated.result.evidenceRef],
        ...validated.result.providerResults.map((providerResult) => (
          ['context-provider-result', providerResult.capsuleRef]
        )),
      ].filter(([, ref]) => ref !== null);
      for (const [prefix, ref] of artifactRows) {
        const id = `${prefix}:${ref.digest}`;
  ...
  contextCall(callId) {
    ...
    const children = plan ? admitted.partitions.map((partition) => {
      const node = plan.nodes.find((candidate) => (
        candidate.contextCall?.partition?.partitionId === partition.partitionId
      ));
      const dispatch = node ? this._planDispatches.get(this._planNodeKey(
    ...
    const stopped = this._runStopByTarget.has(admitted.source.runId)
      || this._runStops.has(admitted.source.runId)
      || this._contextSessions.get(admitted.source.sessionId)?.state === 'stopped'
      || this._contextCells.get(admitted.source.cellId)?.state === 'stopped';
  ...
  contextCalls({ runId = null } = {}) {
    ...
    return [...this._contextCalls.keys()].map((callId) => this.contextCall(callId))
      .filter((call) => runId === null || call.source.runId === runId)
      .sort((left, right) => left.admittedEvent - right.admittedEvent);
  }
  contextCallSettlementChildren(callId, cleanupReceipt = null) {
    const call = this._contextCalls.get(callId);
    if (!call) throw new CoordinationRefusal('Context map call is unavailable',
      'context_map_call_not_found');
    const children = this._contextMapSettlementChildren(call, false);
    if (cleanupReceipt === null) return clone(children);
    const cleanup = this._normalizeContextMapCleanupReceipt(
      call, children, cleanupReceipt, false,
    );
    return clone(this._contextMapSettlementChildren(call, false, cleanup));
  }
  contextCallArtifacts(callId) {
    ...
    if (!['completed', 'failed'].includes(call.state) || !call.result) {
      throw new CoordinationRefusal('Context map call has no terminal artifacts',
        'context_map_call_not_completed');
    }
    const providerResults = this._normalizeContextMapProviderResults(
  ...
  settleContextMapCall(fields, auth) {
    ...
    try {
      ...
      throw new CoordinationRefusal(error.message, 'context_map_call_settlement_invalid');
    }
    const baseChildren = this._contextMapSettlementChildren(call, false);
    const cleanup = this._normalizeContextMapCleanupReceipt(
      call, baseChildren, fields?.cleanup, false,
    );
    ...
    const prior = this._byKey.get(auth?.key);
    const suppliedProviderResults = fields?.result?.providerResults
      ?? (prior ? call.result?.providerResults : undefined);
    const providerResults = suppliedProviderResults === undefined
      ? null : clone(suppliedProviderResults);
    const result = {
    ...
    const payload = {
      schemaVersion: 1, authority, callId: call.callId,
      expectedVersion: fields?.expectedVersion, newVersion: fields?.expectedVersion + 1,
      result, settlementDigest: canonicalDigest(settlementCore),
    };
    if (prior) {
      if (prior.kind !== 'context.call_settled' || prior.actor !== auth.actor
        || canonicalDigest(prior.payload) !== canonicalDigest(payload)) {
        throw new CoordinationRefusal('Context map settlement key is bound differently',
          'context_map_call_settlement_conflict');
      }
      const projected = this.contextCall(call.callId);
      if (!projected || projected.settledEvent !== prior.seq) {
        throw new CoordinationIntegrityError('Context map settlement projection is absent',
          'context_map_call_settlement_integrity');
      }
      return freeze({ ok: true, result: 'idempotent', event: clone(prior), call: projected });
    }
    ...
    this._validateContextMapCallSettlementPayload(payload, prospective, false);
    const event = this._append('context.call_settled', payload, auth, prospective.ts);
    const projected = this.contextCall(call.callId);
    if (projected?.settledEvent !== event.seq) {
      throw new CoordinationIntegrityError('Context map call settlement did not materialize',
        'context_map_call_settlement_integrity');
    }
  ...
  proposePlan(fields, auth) {
    ...
    this._validateContextMapPlanProposal(plan, false);
