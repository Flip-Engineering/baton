...
export class CoordinationStore {
  ...
  _resetProjection() {
    ...
    this._contextSessions = new Map(); this._contextCells = new Map();
  ...
  _runStopContextTargets(targetRunIds) {
    ...
    return { targetContextSessionIds, targetContextCellIds };
  }

  _runStopTargets(runId, throughSeq = this._events.length, includeContext = true) {
  ...
  _validateRunStopAdmission(p, event, integrity = false) {
    ...
    const contextFields = version === 2 ? ['targetContextSessionIds', 'targetContextCellIds'] : [];
    ...
    if (!p || Object.keys(p).sort().join(',') !== fields.sort().join(',') || ![1, 2].includes(version)
    ...
    const targets = this._runStopTargets(
      p.runId, this._runLineagePolicy ? p.throughSeq : undefined, version === 2,
    );
  ...
  _validateRunStopCompletion(p, event, integrity = false) {
    ...
    if (!p || Object.keys(p).sort().join(',') !== ['receipt', 'runId', 'schemaVersion'].join(',')
      || ![1, 2].includes(p.schemaVersion) || !validRunId(p.runId)) fail('run stop completion is invalid');
    ...
    if (stop.schemaVersion === 2) {
      ...
      if (!context || Object.keys(context).sort().join(',') !== contextFields.sort().join(',')
        ...
        || remainingSessionCount !== 0 || remainingCellCount !== 0) {
  ...
  _normalizeContextSourceAttestation(value, { deployment, manifest, node, branch, source = null },
    ...
    if (!value || typeof value !== 'object' || Array.isArray(value)
      ...
      || value.scopeDigest !== contextValueDigest([...node.pathScope].sort())
    ...
    if (source !== null) {
      ...
      for (const item of source) {
        ...
        const scopeAllows = pathInScopes(item.path, node.pathScope);
  ...
  admitRunStop(fields, auth) {
    ...
    const schemaVersion = targets.targetContextSessionIds?.length > 0
