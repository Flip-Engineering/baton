...
export class CoordinationStore {
  ...
  _runStopContextTargets(targetRunIds) {
    ...
    const targetContextCallIds = [...this._contextCalls.values()]
      .filter((call) => targetRunSet.has(call.source.runId)
        && call.state !== 'completed' && call.state !== 'stopped')
  ...
  _validateRunStopCompletion(p, event, integrity = false) {
    ...
    if (stop.schemaVersion >= 2) {
      ...
      const remainingCallCount = callStates.filter((state) => state !== 'stopped').length;
    ...
    if (!integrity) {
      ...
      if (!Array.isArray(verified.output.items)
        ...
        || call.partitions.some((partition) => (
          ...
          || partition.coordinateDigest !== cell.result.coordinateDigest
        ))) {
  ...
  _contextMapSettlementChildren(call, integrity = false, cleanup = null) {
    ...
    for (const partition of call.partitions) {
      ...
      if (task.status !== 'completed') {
        return failChild('Context map settlement cannot overclaim a failed or cancelled child',
          'context_map_child_failed');
      }
  ...
  _validateTaskResourceReleasePayload(payload, event, integrity = false) {
    ...
    if (!task || task.status !== 'completed' || task.version !== payload.taskVersion
    ...
    if (release.process.state === 'not_started') {
      ...
      if (!terminal || terminal.kind !== release.process.terminalKind
  ...
  _validateContextMapCallSettlementPayload(payload, event, integrity = false) {
    ...
    const result = freeze({
      state: 'completed', providerEffects: call.partitions.length,
      children, childDigest: canonicalDigest(children), cleanup, outputRef, evidenceRef,
    });
    if (!integrity) {
      let output; let evidence;
      try {
        output = this._contextReferenceRead(outputRef);
      ...
      if (!output || output.schemaVersion !== 1 || output.kind !== 'baton.context_value'
  ...
  _apply(event) {
    ...
    if (['goal.version_defined', 'plan.version_proposed', 'plan.approval_decided', 'plan.node_dispatched', 'plan.node_budget_settled'].includes(event.kind)) {
      ...
      this._contextCalls.set(validated.call.callId, freeze({
        ...clone(validated.call), state: 'completed', version: p.newVersion,
        result: clone(validated.result), settlementDigest: p.settlementDigest,
        settledEvent: event.seq, settledAt: event.ts,
      }));
      ...
      if (p.schemaVersion >= 3) {
        for (const callId of p.targetContextCallIds) {
          ...
          if (!call || call.state === 'completed' || call.state === 'stopped') {
  ...
  contextCall(callId) {
    ...
    const dispatches = plan ? [...this._planDispatches.values()].filter((dispatch) => (
      dispatch.binding?.planId === plan.planId && dispatch.binding?.planVersion === plan.version
        && dispatch.binding?.planDigest === plan.digest
    )) : [];
    const children = dispatches.map((dispatch) => {
      const task = this._tasks.get(dispatch.taskId);
      const node = plan?.nodes.find((candidate) => candidate.key === dispatch.binding?.nodeKey);
      return {
        nodeKey: dispatch.binding.nodeKey, taskId: dispatch.taskId,
    ...
    }).sort((left, right) => left.nodeKey < right.nodeKey ? -1 : left.nodeKey > right.nodeKey ? 1 : 0);
    ...
    const state = admitted.state === 'completed' ? 'completed'
          ...
          : approval.disposition === 'rejected' ? 'denied'
            : children.length === 0 ? 'approved'
              : children.every((child) => TERMINAL.has(child.state))
                ? children.every((child) => child.state === 'completed')
                  ? 'settlement_ready' : 'failed'
                : 'running';
    return clone({
      ...admitted, state,
      plan: plan ? {
  ...
  contextCallArtifacts(callId) {
    ...
    if (call.state !== 'completed' || !call.result) {
      throw new CoordinationRefusal('Context map call has no completed artifacts',
        'context_map_call_not_completed');
    }
    let output; let evidence;
    try {
      output = this._contextReferenceRead(call.result.outputRef);
    ...
    if (canonicalDigest(output?.items) !== canonicalDigest(call.result.children)
      || evidence?.callId !== call.callId || evidence?.callDigest !== call.callDigest
  ...
  _validateContextCompletionArtifacts(cell, result, output, evidence, integrity = false) {
    ...
    if (contextValueDigest(output) !== result.outputRef.digest
      ...
      || evidence.schemaVersion !== 1 || evidence.kind !== 'baton.context_cell_evidence'
  ...
  settleContextMapCall(fields, auth) {
    ...
    const result = {
      state: 'completed', providerEffects: children.length,
