...
export const SWARM_DRIVER_EVENT_PAYLOAD_SCHEMAS = Object.freeze({
  ...
  'swarm.root_attention_owed': Object.freeze({
    ...
    fields: Object.freeze({
      ...
      owed: { type: 'string', description: 'which trigger fired: review_owed (no other active seat holds the review permission), needs_root (a needsFromOthers item whose text is addressed to the root), continuation_failed (a successor could not start), or turn_reported (a turn ended with no parent seat to receive its report, so the root is owed the look). Spelled `owed`, never `kind` (the name the reporting half consumes): the ledger\'s driver container records `{kind, ...payload}`, so a payload field named `kind` would overwrite the row\'s own operational identity', expectation: 'one of review_owed, needs_root, turn_reported, continuation_failed', example: 'needs_root' },
