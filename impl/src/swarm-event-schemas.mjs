...
export const SWARM_DRIVER_EVENT_PAYLOAD_SCHEMAS = Object.freeze({
  ...
  'wake.root_delivered': Object.freeze({
    ...
    fields: Object.freeze({
      ...
      swarmId: STRING('the swarm the wake belongs to, or null for a non-swarm Run', { type: 'string|null' }),
      runId: STRING('the non-swarm Run the wake belongs to', { type: 'string|null', required: false }),
      workerId: STRING('the non-swarm worker the wake belongs to', { type: 'string|null', required: false }),
  ...
  'swarm.root_attention_owed': Object.freeze({
    ...
    fields: Object.freeze({
      ...
      owed: { type: 'string', description: 'which trigger fired: review_owed (no other active seat holds the review permission), needs_root (a needsFromOthers item whose text is addressed to the root), worker_idle (a retained worker needs a continue-or-stop decision under host pressure), turn_reported (a seat\'s turn-end report has no live orchestrator — #572; the row rides this kind and carries no contributionId). Spelled `owed`, never `kind` (the name the reporting half consumes): the ledger\'s driver container records `{kind, ...payload}`, so a payload field named `kind` would overwrite the row\'s own operational identity', expectation: 'one of review_owed, needs_root, turn_reported, worker_idle', example: 'needs_root' },
