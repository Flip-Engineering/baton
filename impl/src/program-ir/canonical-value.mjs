...
function normalizeChecked(value, authority, byteLimit) {
  ...
  const visit = (item, depth, label) => {
    nodes += 1;
    if (nodes > authority.maxProgramBytes) fail('Program canonical value exceeds its node authority');
...
function serializeCanonical(value) {
  ...
  return `{${Object.keys(value).map((key) => `${JSON.stringify(key)}:${serializeCanonical(value[key])}`).join(',')}}`;
}
...
class StrictJsonParser {
  ...
  value(depth) {
    this.nodes += 1;
    if (this.nodes > this.authority.maxProgramBytes) fail('Raw Program JSON exceeds its node authority');
  ...
  array(depth) {
    ...
    if (this.text[this.offset] === ']') { this.offset += 1; return result; }
    while (true) {
      if (result.length >= this.authority.maxProgramBytes) fail('Raw Program JSON array exceeds its authority');
