...
// ---------------------------------------------------------------------------
// Issue #69 (D2/R8′) — the cited REPL objects render family. An orchestrator-authored context
// object a member's brief cites renders as `## Cited REPL objects` under the closed
// UNTRUSTED_REPL_OBJECT frame, one `[repl/untrusted]` bullet per cited object. Bounding the cited
// set (the item/byte rows, the digest-cited spill) belongs to the seam that resolves the
// citations; this family owns the frame and the single-line leaf, exactly as the attention family
// leaves its own bounding to the push projection.
// ---------------------------------------------------------------------------

export const CITED_REPL_SECTION = '## Cited REPL objects';

export const REPL_ITEM_PREFIX = '[repl/untrusted]';
...
/** The cited object's leaf text: the bounded head the resolving seam composed. */
export function replObjectLeafText(entry) {
  if (!entry || typeof entry !== 'object') return '';
  if (typeof entry.head?.text === 'string') return entry.head.text;
  return typeof entry.head === 'string' ? entry.head : '';
}

export function replObjectItemLine(entry) {
  return `- ${REPL_ITEM_PREFIX} ${namedField(entry?.citation)}: ${neutralizeCarriedBody(replObjectLeafText(entry))}`;
}

/** The `## Cited REPL objects` section, or null when the citation set is empty (D2's
 * absence-on-empty pin). */
export function renderCitedReplObjectsSection(replObjects) {
  const entries = Array.isArray(replObjects) ? replObjects : [];
  if (entries.length === 0) return null;
  return [CITED_REPL_SECTION, UNTRUSTED_REPL_OBJECT_FRAME, ...entries.map(replObjectItemLine)].join('\n');
}
