...
export function renderPrompt(brief) {
  ...
  const dispatchGuidance = brief.contextInput
    ? (advertisesBatonTool
      ? 'This task is already dispatched by Baton. Use the attached immutable Context directly. Orchestration actions may use only the Baton control surface explicitly listed in this Brief.'
      : 'This task is already dispatched and supervised by Baton. Treat the attached immutable Context as authoritative. Do not search for or launch another Baton CLI, MCP server, or Run; use one only when this Brief explicitly advertises it.')
  ...
  const lines = [
    ...
    brief.verification?.command ? (brief.contextInput
      ? `A reviewer independently enforces \`${brief.verification.command}\` (must exit ${brief.verification.expectExit}). Run it only when the requested work needs code verification; do not substitute it for analyzing the attached Context.`
      : `A reviewer will independently run this to check your work: \`${brief.verification.command}\` (must exit ${brief.verification.expectExit}). Make that pass.`) : '',
  ];
