...
class CliAdapter {
  ...
  async _onClose(session, code, signal) {
    ...
    processClose.holdStopConfirmation();
    try {
      if (session.stopping) {
        const terminalCause = session.timeoutFailure ? 'timeout' : session.wireFailure ? 'wire_frame_oversize' : null;
      ...
      await processClose.close(code, signal, false);
      ...
      processClose.releaseStopConfirmation();
    }
  }
