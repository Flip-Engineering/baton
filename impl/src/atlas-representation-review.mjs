...
export class AtlasRepresentationReview {
  constructor(opts = {}) {
    ...
    if (typeof opts.repoRoot !== 'string' || typeof opts.artifactRoot !== 'string' || !limits || Object.keys(limits).sort().join(',') !== fields.sort().join(',') || Object.values(limits).some((value) => !Number.isSafeInteger(value) || value <= 0) || limits.maxRows < ROWS.length || limits.maxFiles < ROWS.reduce((sum, row) => sum + row.paths.length, 0)) throw new TypeError('representation review requires repository, artifact root, and exact positive limits');
  ...
  async reverify(claim, op, args, ctx) { const rebuilt = await this.invoke(op, args, ctx); return Object.freeze({ ok: rebuilt.refs[0].digest === claim?.refs?.[0]?.digest, observedDigest: rebuilt.refs[0].digest }); }
}
