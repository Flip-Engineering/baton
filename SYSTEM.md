...
- **Down:** the *brief* (the task: goal, what's in scope, how it'll be checked, budget — written in each vendor's preferred style), a *nudge* (a note delivered at the next natural pause), a *steer* (redirect now).
...
### 5.1 Trust — re-verify before believing "done"
When a worker says it passed, the coordinator **re-runs the check itself, in a fresh copy of the repo the worker never touched**, and believes only what it observes. A worker can't fool the driver by leaving a doctored test behind, because the check runs somewhere clean against a spec *you* pinned (not one the worker rewrote). This is why the driver can safely auto-merge, move to the next task, or reroute — it's earned the right to trust the result. For high-stakes code, this extends to a **ladder of deeper checks** (types → tests → property tests → fuzzing → math proof), where the driver picks the cheapest rung that fits the task's risk — most tasks stop at "run the tests." Deeper: `docs/capabilities/math-proof.md` (renamed in spirit to *evidence*, honest about where proof genuinely applies).
...
- **Debugging** that returns a *structured explanation of why something failed*, plus shareable record-and-replay recordings, instead of a giant step-by-step log. `docs/capabilities/debug-interp.md`.
...
- **Things that will move under us:** vendors are building pieces of this themselves and models keep improving, so build *deep* only the parts that stay valuable as models get better — dependable cross-vendor control and trustworthy verification — and keep the rest thin and swappable.
