...
```
   ...
   │  Codex       Claude Code       GLM (Claude+Z.ai)       Grok Build (ACP)            │
   └───────────────────────────────────────────────────────────────────────────────────┘
```
...
The ordinary embedded, authenticated Web, and MCP surfaces share this application bus:
...
selection, not integration or publication. `run.stop` is Run-scoped; `application.shutdown` is host-only and fleet-wide. Web/MCP expose
...
| Graph-backed `Representation` and semantic-delta producers | Tools/Memory/Trust | Shipped Phase 61 for fixed R1 structural delta, R2 SCIP snapshot, and R3 bounded CPG delta; exact source-card/environment binding, immediate reverify, content-addressed receipt, atomic Cairn lineage, replay/reconcile, and direct/web/MCP parity. Deeper precision and R4–R7 remain gated. | `spec/phase61/graph-backed-representations.md`, `impl/test/phase61-*` |
...
| Advisory refresh guard + exact TTL invalidation | Safety/Memory | Shipped Phase 39; adverse provider ingress ships in Phase 43, positive clearance remains later | `spec/phase39/advisory-ttl-invalidation.md`, `docs/capabilities/orientation-reuse.md` |
...
| Supervised startup native-session auto-rejoin | Core/Operate/Safety | Shipped Phase 45; opt-in bounded readiness barrier, exact context/session/model/effort handshake, eligible worktree/runtime preservation, degraded orphan honesty, and async close/reap | `spec/phase45/supervised-session-auto-rejoin.md`, `docs/handoff/evidence/phase45-session-auto-rejoin-2026-07-12.md` |
| Attested complete representation review packet | Tools/Trust | Shipped Phase 46; exact committed-source inventory for R1 AST/CST through R7 Decisions, full-claim reverify, independent ceilings, authenticated ACI reach, and explicit retained gaps | `spec/phase46/attested-representation-review.md`, `docs/handoff/evidence/phase46-representation-review-2026-07-12.md` |
| Cairn causal integrity and attested audit/trace | Memory/Trust/Operate | Phase 47 shipped locally; true bitemporal observation history, content-bound generic graph events, typed live causal audit, explicit contradiction CAS/resolution/contamination, bounded live trace, repository/transport-bound ACI, durable request/result idempotency, descriptor-first artifact bounds, publication cancellation, and a digested full-goal retention catalog; exact GLM/Codex dogfood is fresh-verified while the current Grok retry is auth-red before provider spawn | `spec/phase47/cairn-causal-integrity-audit.md`, `impl/test/phase47-cairn-causal-audit.test.mjs` |
| Cairn audit-gated bounded recall | Memory/Trust/Operate | Phase 48 shipped locally; deterministic integer lexical/graph ranking, complete unresolved-contradiction bundles, compact append-before-return receipts, historical `ReadBy`/contamination identity, pull-only untrusted framing, direct/web/MCP replay and reverify, recomputable request integrity, and registry-derived pre-effect ACI output gates; exact Codex and project-key GLM reviews are fresh-verified while the current two-Grok retry remains auth-red before provider spawn and fully reaped | `spec/phase48/cairn-bounded-recall.md`, `impl/test/phase48-cairn-recall.test.mjs` |
| Cairn audit-gated selective promotion | Memory/Trust/Operate | Phase 49 shipped locally; one pinned coordination prefix deterministically promotes a closed taxonomy of operator/orchestrator Decisions, policy Counterexamples, and independently verified cited observed Scratch Findings in one replay-validated atomic batch, with safe fixed projections, exact causal edges, all-or-nothing ceilings, transport-bound direct/web/MCP invoke and reverify, ACI pre-effect output gates, and full no-op/idempotency/restart semantics; Phase 50 separately closes derived-Scratch release and correction without widening this taxonomy | `spec/phase49/cairn-selective-promotion.md`, `impl/test/phase49-cairn-promotion.test.mjs` |
| Cairn Scratch correction and independent-oracle release | Memory/Trust/Operate | Phase 50 shipped locally; hub-derived Scratch facts with derived grounding can be reviewed through an explicit independent harness/model/effort route pinned to the fact's immutable tree, and accepted fact-bound provenance can release, supersede, or retract the closed Scratch Finding class through one audited prefix-CAS event with exact contamination, token-bound direct/web/MCP authority, ACI pre-effect result gates, restart replay, and non-integrable oracle tasks; 14 focused contracts and 1038/1038 canonical tests are green; post-fix project-key GLM is fresh-verified and kill-confirmed while current Codex initialization and two concurrent Grok attempts are honestly pre-PID red with complete ownership restoration | `spec/phase50/cairn-scratch-correction-oracle.md`, `impl/test/phase50-cairn-scratch-correction.test.mjs` |
| Exact provider process lifecycle and reap | Core/Operate/Safety | Phase 51 shipped locally; coordinator-selected generation plus exact PID/group/source binds process start, provider readiness, close, replay, recovery, interrupt, kill, forced/poison retry, cleanup, and bounded authenticated status across Claude/GLM/Codex/Grok/one-shot; 63 focused contracts and 1103/1103 canonical tests are green; recursive exact Codex/GLM/two-Grok routes prove all start/close and cleanup while the current Grok readiness matrix remains honestly authentication-red | `spec/phase51/pre-ready-process-lifecycle.md`, `impl/test/phase51-process-lifecycle.test.mjs` |
| Cairn verified recall-outcome attribution | Memory/Trust/Operate | Phase 52 shipped locally; a pinned audited scan selects task-scoped Phase 48 receipts that precede exact mapped hub verification and compatible terminal state, records one compact replay-validated association batch, and reports honest coverage/pass-after/fail-after/contamination metrics with `causationClaimed:false`; callers cannot nominate outcomes, workers cannot rate memory, direct/web/MCP share token-bound authority and pre-effect output gates, and no ranking/confidence/routing mutation occurs; 9 grouped contracts and 1112/1112 canonical tests are green, project-key GLM fresh-verifies PASS, and every recursive process/ownership surface reaps while current Grok readiness remains honestly auth-red | `spec/phase52/cairn-recall-outcome-attribution.md`, `impl/test/phase52-cairn-recall-assessment.test.mjs` |
...
| Attach-only native recovery transaction | Core/Operate/Safety | Phase 60 shipped locally for the covered deterministic transaction: exact identity attach before any continuation, durable refinement and continuation intent, accepted/refused/unknown dispatch dispositions, no automatic redelivery, and exact stop/reap ownership. Provider-backed recovery proof and remaining adapter/crash rows stay open. | `spec/phase60/attach-only-native-recovery.md`, `impl/test/phase60-coordination-recovery.test.mjs` |
| Graph-backed Atlas representation production | Tools/Memory/Trust | Phase 61 shipped locally for fixed R1/R2/R3 mappings with source-contract hardening, immediate reverify, stable identity, atomic causal lineage, bounded receipts/results, completed-replay integrity, and direct/web/MCP parity. It does not claim semantic truth, compiler IR, whole-repo CPG, true semantic merge, or proof. | `spec/phase61/graph-backed-representations.md`, `impl/test/phase61-*` |
