...
1. **P0.1 — objective-first review preset.** `baton.review(objective, { reviewers })` and
   `baton review OBJECTIVE --exact ... --exact ...` require two to sixteen complete exact
   harness/model/effort tuples. They deterministically assign `reviewer-1` through `reviewer-N` and
...
3. **P0.3 — connected deployment truth.** The resident application card exposes the same sanitized,
   ...
   `doctor()` and exact `route({ harness, model, effort })`; `baton route
