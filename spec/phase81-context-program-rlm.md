...
Layer A is locally implemented and green: closed immutable tree-bound ContextManifests; strict
JSON-only content identity; a closed canonical Context Program AST; deterministic pure
source/outline/index/search/slice/chunk/filter/project/sort/unique/join/collect/coverage/finish;
typed refusal of map/reduce/review/verify outside Workflow authority; stateless memoized cells;
content-addressed output with collision validation and path-free public refs; source substitution,
missing artifact, missing key, oversized source, policy mismatch, and prospective join-work
refusal; plus a compact ContextSession outline/index/cell/evidence/help cascade. The focused suite
is green at 7/7. The canonical-order plus Context Program regression gate is 18/18, and the full
implementation suite is 2,018/2,018 green on 2026-07-18.

Layer B is not implemented and is not claimed: no durable ledger cell admission or manifest
producer, no exact per-item/range/node provenance, no `context_recursive` application strategy, no
map-to-WorkItem/Wave compiler, no reduce/review Candidate, no cell-generation recovery/late-result
fence, no CP6 termination projection, no stop-union extension, no transport parity, and no four-arm
utility evaluation. The next executable slice is one dependency-ordered successor: durable cell
admission and exact evidence first, then a single depth-one map Wave with exact role routing,
restart identity, generation fencing, and stop/reap shipped together.

Live Baton-on-Baton evidence used exact Codex `gpt-5.6-sol`/low and native Kimi Code
`kimi-code/k3`/high in one parallel Wave. Both produced freshly verified independent Candidates.
Typed feedback selected the exact Kimi Candidate, appended and distinctly approved Plan v2, and
launched a fresh Kimi revision worktree from its exact retained SHA; the revised Candidate freshly
verified. Workflow stop and deployment close returned zero workers, caller status/index were
unchanged, and route truth retained requested/resolved/observed distinctions. A precursor
interrupted Kimi run was reopened from its ledger and reaped both processes/worktrees to
`remainingCount: 0` without fabricating Kimi success. Dogfood also exposed and repaired the
default state-version collision, repeated-signal runner cleanup, strict JSON collision risk,
host-path projection, cell recomputation, and untyped missing-key/prospective-join defects. It still
exposes weak ordinary in-flight progress, incomplete harness-internal descendant visibility, and
short-lived Kimi subscription readiness as product friction.
