...
const workflow = await baton.workflow(objective, {
  ...
  team: {
    explorer: { harness: 'kimi-code', model: 'kimi-code/k3', effort: 'high' },
    critic: { harness: 'codex', model: 'gpt-5.6-sol', effort: 'low' },
    synthesizer: { harness: 'codex', model: 'gpt-5.6-sol', effort: 'high' },
  },
});
...
```json
{
  ...
  "repoId": "repo-baton",
  "tree": { "sha": "...", "source": "workflow_plan" },
  "workflow": { "runId": "...", "goalId": "...", "planId": "..." },
  "branches": [
    { "name": "repository", "ref": "ctx:...", "summary": "..." },
    { "name": "atlas", "ref": "ctx:...", "summary": "..." },
    { "name": "coordination", "ref": "ctx:...", "summary": "..." },
    { "name": "evidence", "ref": "ctx:...", "summary": "..." }
  ],
  "policyDigest": "...",
  "digest": "..."
}
...
The manifest binds one tree. Cross-tree facts or artifacts remain readable only with conspicuous
...
`map` never calls a provider directly. The hub validates the role against the approved Workflow
definition, derives one WorkItem, one Wave, and one child Attempt per addressed partition, and
resolves each exact harness/model/effort tuple from the orchestrator-approved role map. Child
...
3. `map` compilation to one bounded WorkItem/Wave and exact role-routed Attempts.
4. Durable child attachment, synthesis/review, coverage, progress, termination, replay, and stop.
5. Direct/CLI/Web/browser/MCP parity plus direct-vs-context-vs-RLM evaluation.
6. Only then consider an isolated code backend, deeper recursion, or opt-in persistent kernels.
