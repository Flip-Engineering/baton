...
This is a pure contract, not durable generic admission: existing map calls are not dual-written,
and current map settlement lacks result-output lineage, so no present call is reduce-eligible.
...
Phase 84 map-v2 projection now also ship without dual ledger authority. Next make that envelope the
sole authority of a new durable admission schema, add exact map-result output lineage, and only then
admit reduce; add
projection-failure terminalization and workerless non-admission; add retry generations; consolidate
pure evaluation through the immutable builder; then add reduce, transport parity, recovery/stop
