...
`resultSourceDigest` binds the exact CAS `sourceRef` and its content identity. `capsuleDigest` binds
...
2. `phase85-context-result-capsule-red.test.mjs`: actual report/delta projection from exact accepted
   artifacts or retained commits after cleanup; private CAS materialization into a later Brief;
