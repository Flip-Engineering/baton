...
coordinates. This is still projection, not acceptance authority: durable map settlement must next
rederive the successful child from coordination and attach the exact capsule refs atomically. Until
that gate lands, reduce remains closed and no standalone caller-supplied digest is treated as proof
that a child was accepted or released.
...
1. `phase85-context-lineage-red.test.mjs`: item lineage through source, outline, index, search,
   ...
   failures.
2. `phase85-context-result-capsule-red.test.mjs`: the retained-commit projection core now ships;
   continue with coordination-derived accepted-child attachment and actual report/delta projection
   from exact accepted artifacts or retained commits after cleanup; private CAS materialization into a later Brief;
