...
and generic stop/reap closes exact ownership. Next add generic successful/failed settlement and
projection-failure terminalization plus workerless non-admission; then retry generations,
`context_eval`, transport parity, deeper recovery/stop reconciliation, and the full recursive live
proof.
