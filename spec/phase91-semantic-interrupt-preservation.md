...
Phase 91 makes ordinary Run interrupt a turn-preservation operation. It ends only the exact
currently admitted provider turn. It does not cancel or fail the Plan task, close Run dispatch,
release the Run's worktree, replace the worker, weaken its route, or relinquish its native session.
...
6. provider terminal governance is valid, the exact turn is sealed, and no provider policy or
   telemetry failure is active;
...
session identity and `transportOpen:true` in deterministic payload tests. This is conditional
governance evidence: a live transport may claim preservation only when its observed terminal event,
provider seal, card, and current local authority all satisfy the receipt gate. A transport that
cannot provide these facts must not expose semantic interrupt, or its confirmation settles without
preservation and is failed/quarantined.
...
physical stop-before-successor ordering and exact cleanup, but is not itself evidence that the
