...
Resume and reverify are public only when implemented and receive the same budget/cancellation policy.
...
Deployments inject a closed registry, token ceiling, result-envelope ceiling, and trusted repository root. Baton invents neither artifact roots nor per-capability resource bounds. An empty registry is valid and visible.
