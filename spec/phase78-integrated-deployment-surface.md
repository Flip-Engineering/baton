...
Claude Code `kimi-k3[1m]`, optional Claude Opus, and optional GLM `glm-5.2`. No older GLM model is a
current default or suggested route.
...
and exact kill/reap is demonstrated. GLM live work waits for its stated provider rate-limit reset
and uses only `glm-5.2`.
