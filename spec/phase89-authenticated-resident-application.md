...
`BatonDeployment.host({ advanced })` is only an authority-preserving integration/test seam: it
requires an injected authenticated HTTPS server and loopback binding. Ordinary `host()` remains
fail-closed. It is not evidence for the resident lease, private credential/session issuance,
owner-local socket, atomic publication, restart fencing, or crash recovery required by this phase.
...
- replace the signature-erasing port compatibility shim with one explicit two-argument command
  port throughout;
...
- build the ordinary private resident host, lease/incarnation/publication authority, Run-scoped
  streams, CLI/browser convergence, and restart dogfood required by gates 3–13.

Evidence: `docs/reference/evidence/phase89-resident-application-dogfood-live-2026-07-18/`.
