...
`application.shutdown` is fleet-wide; it is intentionally not called `run.close`. Safe CLI/serve lifecycle, recovery, structured
...
another worker primitive: the safe primary CLI/serve host, materialized result export, Run-level
